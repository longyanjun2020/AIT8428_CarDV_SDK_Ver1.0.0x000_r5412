...
#define AHC_PRINT_RET_WARNING(force_stop,ret_val) \
		{printc(BG_BLUE("%s,L:%d warning:0x%x!")"\r\n",__func__,__LINE__, ret_val); if(force_stop) {while(1);}}
...
#define COMMON_KEY_B_VOL					"B_VOL"// lyj
...
#define COMMON_KEY_REARCAM_MIRROR 			"RearCamMirror"
...
typedef struct _SetAllFlag
{
	INT8 SaveTheFM;
	INT8 SaveTheAM;
	INT8 SaveTheBluetooth;
	INT8 SaveTheAUX;
	INT8 SaveTheUSB;
	INT8 SaveTheBrightness;
	INT8 SaveTheLightBar;
	INT8 SaveTheRGB;
	INT8 upDwonUB;
	INT8 Speedmode;
	INT8 SlowBecome;
	INT8 onceFlag;
	//INT8 SlowBecomeCountUnite;
	
}SetAllFlag;
extern SetAllFlag SelfAllFlag;
