...
static UINT16                   m_wReviewTime      		= 0;///< ms
static UINT16                   m_wWindowsTime     		= 0;///< ms
...
extern MMP_BOOL MMP_IsDscLdcSupport(void);
...
AHC_BOOL AHC_ConfigEXIF_RTC(void)
{
    ...
    AHC_UTILITY_Int2Str(uwYear   , StrYear);
    AHC_UTILITY_Int2Str(uwMonth  , StrMonth);
    AHC_UTILITY_Int2Str(uwDay    , StrDay);
    AHC_UTILITY_Int2Str(uwHour   , StrHour);
    AHC_UTILITY_Int2Str(uwMinute , StrMinute);
    AHC_UTILITY_Int2Str(uwSecond , StrSecond);
...
AHC_BOOL AHC_SetImageSize(UINT8 byMode, UINT16 wSizeX, UINT16 wSizeY)
{
    ...
    case VIDEO_CAPTURE_MODE:
        ...
        if (CAM_CHECK_PRM(PRM_CAM_TV_DECODER)) {
            MMPS_3GPRECD_CustomedEncResol(MMPS_3GPRECD_FILESTREAM_NORMAL, MMP_SCAL_FITMODE_OPTIMAL, wSizeX, wSizeY);
        }
        else {
            MMPS_3GPRECD_CustomedEncResol(MMPS_3GPRECD_FILESTREAM_NORMAL, MMP_SCAL_FITMODE_OUT, wSizeX, wSizeY);
        }
#if 0
        if (bRestartPreview == MMP_TRUE) {
            AHC_SetMode(SysMode);
...
AHC_BOOL AIHC_SetCapturePreviewMode(AHC_BOOL bRawPreview, AHC_BOOL bMultiReturn)
{
    ...
    if (bMultiReturn == AHC_FALSE)
    {
        ...
        if (MMP_IsPrmCamExist()) {
            MMPS_Sensor_Initialize(gsAhcPrmSensor, AHC_SNR_GetPresetSnrMode(), MMP_SNR_DSC_PRW_MODE);
        }
        ...
        // To Check SNR Flip and set dir
        {
            ...
            AHC_SetKitDirection(LCDStatus, AHC_TRUE, AHC_GetSensorStatus(), ubSnrFlipEn);
        }
        
        /* Start TV Decoder Preview */
        if (CAM_CHECK_PRM(PRM_CAM_TV_DECODER))
        {
            UINT32 ulTVDecTypeTimeout = 100;
            static MMP_SNR_TVDEC_SRC_TYPE sTVDecSrc = MMP_SNR_TVDEC_SRC_NO_READY;
        ...
        sRet = MMPS_DSC_InitPreview(gsAhcPrmSensor);
        ...
        MMPS_DSC_EnablePreviewDisplay(gsAhcPrmSensor, MMP_TRUE, MMP_FALSE);
        MMPD_Display_SetWinActive(MMP_DISPLAY_WIN_OVERLAY , MMP_FALSE);
    }
    else {
        UINT16 uwReviewTime;
        ...
        MMPS_DSC_RestorePreview(gsAhcPrmSensor, uwWindowsTime);
    }
...
AHC_BOOL AIHC_ConfigCaptureTimeStamp(UINT32 TimeStampOp)
{
	...
	if(MenuSettingConfig()->uiIMGSize == IMAGE_SIZE_VGA)
	{
	        AHC_OSDChangeDimetion(DATESTAMP_PRIMARY_OSD_ID, PRIMARY_DATESTAMP_WIDTH-400, PRIMARY_DATESTAMP_HEIGHT);
	}
	else
	{
	        AHC_OSDChangeDimetion(DATESTAMP_PRIMARY_OSD_ID, PRIMARY_DATESTAMP_WIDTH, PRIMARY_DATESTAMP_HEIGHT);
	}		
	#endif
    ...
    AHC_STICKER_TransDateToString(&rtcTime, Date, DateConfig, FormatConfig);
    ...
    if (PositionConfig == AHC_ACC_TIMESTAMP_BOTTOM_RIGHT) {
        ...
        StickerY    = PrimaryJpegH - AHC_CAPTURE_STICKER_HEIGHT-50;
    }
    ...
    {
    	AHC_OSDDrawText(DATESTAMP_PRIMARY_OSD_ID, (UINT8*)Date, 50, 0, STRLEN(Date));
    }
    ...
    if (TimeStampOp & AHC_ACC_TIMESTAMP_LOGOEX_MASK)
    {
		...
		AHC_OSDDrawText(DATESTAMP_PRIMARY_OSD_ID, (UINT8*)szProductName, uiPosX, uiPosY, STRLEN(szProductName));
		uiPosY += 20;
		#else
        {
            UINT32 modeNameAddr;
            ...
            {
    			AHC_OSDDrawText(DATESTAMP_PRIMARY_OSD_ID, (UINT8 *) modeNameAddr, uiPosX, 0, STRLEN((char *) modeNameAddr));
            }
		}
    ...
    AHC_STICKER_DrawTextEdge(PrimaryBufAddr, PrimaryBufW, AHC_CAPTURE_STICKER_HEIGHT, 0xFF, 0x00, 0x01);
	#endif
...
AHC_BOOL AIHC_SetCaptureMode(UINT8 uiMode)
{
    ...
    /* Set Large resolution capture information*/
	MMPS_DSC_SetLargeCaptureInfo(2);
    ...
    if (bRet == AHC_TRUE) 
    {
        if (uiMode != AHC_MODE_CONTINUOUS_CAPTURE)
        {
            ...
            printc(FG_RED("--Error-- We don't support Format Free for Capture mode now!!"));
            while(1);
            MMPS_FORMATFREE_EnableWrite( 1 ); // Note: In case to write Serial Flash, please disable it.
            AHC_FF_SetFileNameSlot( bJpgFileName , DCF_CAM_FRONT);
            // Note: MMPS_FORMATFREE_EnableWrite(0) should be set at a suitable place, ex: outside of AIHC_SetCaptureMode().
            #endif
		}
        ...
        if (TimeStampOp & AHC_ACC_TIMESTAMP_ENABLE_MASK)
        {
            ...
            UINT32      *uwDigZoomRatio;

            AHC_Menu_SettingGetCB((char *)COMMON_KEY_STILL_CAP_SIZE, &ulStillCapSize);

            MMPS_Sensor_GetCurCaptScalInputRes(gsAhcPrmSensor, &ulScalInW, &ulScalInH);
			DBG_AutoTestPrint(ATEST_ACT_CAPTURE_0x0005, ATEST_STS_START_0x0001, 0, 0, gbAhcDbgBrk);
			if (gbAhcDbgBrk) AHC_GetCameraDigitalZoomRatio(uwDigZoomRatio);
  		    DBG_AutoTestPrint(ATEST_ACT_CAPTURE_0x0005, ATEST_STS_ZOOM_0x0008, 0, *uwDigZoomRatio, gbAhcDbgBrk);
            ...
            if (0)//(ZoomCtrl_GetStillDigitalZoomLevel() || (ulStillCapSize < maxImageSize))
                ...
                #ifdef _OEM_CAPTURE_STICKER_
                DrawCapture_TimeStamp(TimeStampOp);
                #else
        ...
        if (uiMode == AHC_MODE_DRAFT_CAPTURE) {
            ...
            MMPS_Sensor_GetCurPrevScalInputRes(gsAhcPrmSensor, &CaptureWidth, &CaptureHeight);
            DBG_AutoTestPrint(ATEST_ACT_CAPTURE_0x0005, ATEST_STS_RSOL_SIZE_0x0004, CaptureWidth, CaptureHeight, gbAhcDbgBrk);
            ...
            sRet = MMPS_DSC_CaptureTakeJpeg(gsAhcPrmSensor, &captureinfo);
            if (sRet != MMP_ERR_NONE) { AHC_PRINT_RET_ERROR(gbAhcDbgBrk,sRet); return AHC_FALSE;} 
            ...
            sRet = MMPS_DSC_GetRawData(gsAhcPrmSensor, 0, ((bSaveJPG) ? AHC_FALSE : AHC_TRUE), bRawFileName, STRLEN(bRawFileName), &ulRawWidth, &ulRawHeight);
            if (sRet != MMP_ERR_NONE) { AHC_PRINT_RET_ERROR(gbAhcDbgBrk,sRet); return AHC_FALSE;} 
			...
			MMPS_DSC_SetContiShotMaxDramAddr(MMPS_System_GetMemHeapEnd());
			#else
            ...
            for (i = 1; i < MultiShotNum; i++) {
                ...
                MMPS_DSC_CaptureTakeJpeg(gsAhcPrmSensor, &captureinfo);
            }
	...
	DBG_AutoTestPrint(ATEST_ACT_CAPTURE_0x0005, ATEST_STS_END_0x0003, 0, sRet, gbAhcDbgBrk);	
    return AHC_TRUE;
}
