...
extern DCF_REGION_ID    m_sDCFRegionID	= DCF_REGION_ID_4;
/**
...
static void* AIHC_DcfMalloc(UINT32 size) {
    ...
    addr = (m_ulDCFTempBaseAddr) + size + mem_offset;
    
    ptr = (UINT8*)( m_ulDCFTempBaseAddr + mem_offset);
                    
    //m_ulDCFTempBaseAddr += (size + mem_offset); //Rogers@201102: When Dcf mount 6's timers, the system will data about. 

    #ifdef AHC_MENU_SETTING_TEMP_BUFFER_SIZE
    MMPS_System_GetPreviewFrameStart(&mem_offset);
    
    if (mem_offset) {
        if (((UINT32) ptr) >= (mem_offset + AHC_MENU_SETTING_TEMP_BUFFER_SIZE)) {
            printc(FG_RED("DCF buffer overlap to FileSystem Buffer !!!\r\n"));
        }
    }
    #endif
...
static AHC_BOOL AIHC_DCF_ParseFile(	DCF_DIR_NODE *psDirNode, 
									...
									DCF_DIR_MOUNT_TYPE type )
{
    INT8                chFileName[MAX_FILE_NAME_SIZE];
    INT8				chFileFreeChar[MAX_FILE_FREE_CHAR_NUMBER + 1];
...
static AHC_BOOL AIHC_DCF_ParseDir(INT8* pchFullDirName, UINT16 uwDirNumber, AHC_BOOL bTop)
{
    ...
    AHC_FS_FILETIME		FileTimeStru;
    UINT32              EntrySize;
    INT8                chDirName[MAX_FILE_NAME_SIZE];
    
    uiDirLength	= STRLEN(pchFullDirName);

	if(AHC_DCF_DirOpen(pchFullDirName, uiDirLength, &uiDirID) != MMP_ERR_NONE){
		AHC_DCF_DirClose(uiDirID);
		return AHC_FALSE;
...
static AHC_BOOL AIHC_DCF_ConfirmDCFxDB(INT8* chFullRootName)
{
	...
	if( AHC_DCF_DirOpen(m_chDCFxDBName, uiNameLength, &uiDirID) != MMP_ERR_NONE){
		AHC_DCF_DirCreate(m_chDCFxDBName, uiNameLength);
	}else{
		MMPS_FS_DirClose (uiDirID);
	}
	...
	*/
	...
	if(AHC_DCF_FileOpen(m_chDCFxDBName, uiNameLength, "rb", STRLEN("rb"), &m_uiDCFxFileID) == AHC_ERR_NONE){
		...
		*/
		...
		AHC_DCF_FileGetSize(m_uiDCFxFileID, &uiFileSize);
...
AHC_BOOL AIHC_DcfMountDB(UINT32 uiMaxMem)
{
    ...
    */
    ...
    #if (DCF_DB_COUNT >= 5) 
    if(m_bEnableDCF[DCF_DB_TYPE_5TH_DB] == AHC_TRUE){
    
        m_uiDBIndex = DCF_DB_TYPE_5TH_DB;
        AIHC_DCF_MountDB(DCF_DB_TYPE_5TH_DB);
        m_DcfDB[DCF_DB_TYPE_5TH_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*DCF_CLUSTER_NUMERATOR_DB4/DCF_CLUSTER_DENOMINATOR;
        m_bMountDCF[DCF_DB_TYPE_5TH_DB] = AHC_TRUE;
    }
    #endif///< (DCF_DB_COUNT >= 5) 
    
    #if (DCF_DB_COUNT >= 4) 
    if(m_bEnableDCF[DCF_DB_TYPE_4TH_DB] == AHC_TRUE){
    
        m_uiDBIndex = DCF_DB_TYPE_4TH_DB;
        AIHC_DCF_MountDB(DCF_DB_TYPE_4TH_DB);
        m_DcfDB[DCF_DB_TYPE_4TH_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*DCF_CLUSTER_NUMERATOR_DB3/DCF_CLUSTER_DENOMINATOR;
        m_bMountDCF[DCF_DB_TYPE_4TH_DB] = AHC_TRUE;
    }
    #endif///< (DCF_DB_COUNT >= 4) 
    
    #if (DCF_DB_COUNT >= 3)
    if(m_bEnableDCF[DCF_DB_TYPE_3RD_DB] == AHC_TRUE){
    
        m_uiDBIndex = DCF_DB_TYPE_3RD_DB;   
        AIHC_DCF_MountDB(DCF_DB_TYPE_3RD_DB);
        m_DcfDB[DCF_DB_TYPE_3RD_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*DCF_CLUSTER_NUMERATOR_DB2/DCF_CLUSTER_DENOMINATOR;
        m_bMountDCF[DCF_DB_TYPE_3RD_DB] = AHC_TRUE;
    }
    #endif///< (DCF_DB_COUNT >= 3) 

    #if (DCF_DB_COUNT >= 2) 
    if(m_bEnableDCF[DCF_DB_TYPE_2ND_DB] == AHC_TRUE){
    
        m_uiDBIndex = DCF_DB_TYPE_2ND_DB;   
        AIHC_DCF_MountDB(DCF_DB_TYPE_2ND_DB);
        m_DcfDB[DCF_DB_TYPE_2ND_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*DCF_CLUSTER_NUMERATOR_DB1/DCF_CLUSTER_DENOMINATOR;
        m_bMountDCF[DCF_DB_TYPE_2ND_DB] = AHC_TRUE;
    }
    #endif///< (DCF_DB_COUNT >= 2) 
    
    if(m_bEnableDCF[DCF_DB_TYPE_1ST_DB] == AHC_TRUE){
    
        m_uiDBIndex = DCF_DB_TYPE_1ST_DB;   
        AIHC_DCF_MountDB(DCF_DB_TYPE_1ST_DB);
        m_bMountDCF[DCF_DB_TYPE_1ST_DB] = AHC_TRUE;
        m_DcfDB[DCF_DB_TYPE_1ST_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*DCF_CLUSTER_NUMERATOR_DB0/DCF_CLUSTER_DENOMINATOR;
    }
	/**
...
{   
    ...
    case DCF_DB_TYPE_1ST_DB:
        m_bEnableDCF[DCF_DB_TYPE_1ST_DB] = bEnable;
        break;
        
    #if (DCF_DB_COUNT >= 2)
    case DCF_DB_TYPE_2ND_DB:
        m_bEnableDCF[DCF_DB_TYPE_2ND_DB] = bEnable;
        break;
    #endif ///< (DCF_DB_COUNT >= 2)
    
    #if (DCF_DB_COUNT >= 3)
    case DCF_DB_TYPE_3RD_DB:
        m_bEnableDCF[DCF_DB_TYPE_3RD_DB] = bEnable;
		 break;
    #endif///< (DCF_DB_COUNT >= 3)
    
    #if (DCF_DB_COUNT >= 4)
    case DCF_DB_TYPE_4TH_DB:
        m_bEnableDCF[DCF_DB_TYPE_4TH_DB] = bEnable;
        break;
    #endif
    ...
    if( uwDirKey ){
        ...
        if ( AIHC_DCF_FindDir(NULL, uwDirKey, &psDirNode) == AHC_TRUE) {
        	...
        	*/
		    ...
		    if(AHC_DCF_DirCreate(chFullDirName, uiDirLength) == MMP_ERR_NONE){
                           ...
                           UINT8 *pDestFileName)
{
	...
	case DCF_FILE_RENAME:
	{
		...
		*/
        ...
        if(AHC_DCF_FileDirGetAttribute(chSrcFileName, FileNameSize, &sFileAttribute) != MMP_ERR_NONE){
        	AIHC_DCF_UNLOCKPAGE();
        	AIHC_DCF_UNLOCKDB();
        	return AHC_FALSE;
        }
...
AHC_BOOL AHC_DCF_AdjustAllocation(DCF_STORAGE_ALLOC eType)
{
    ...
    if(m_bEnableDCF[DCF_DB_TYPE_4TH_DB] == AHC_TRUE){
        ...
        m_DcfDB[DCF_DB_TYPE_4TH_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*uiNumeratorDB3/uiDenominator;
    ...
    if(m_bEnableDCF[DCF_DB_TYPE_3RD_DB] == AHC_TRUE){
        ...
        m_DcfDB[DCF_DB_TYPE_3RD_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*uiNumeratorDB2/uiDenominator;
    ...
    if(m_bEnableDCF[DCF_DB_TYPE_2ND_DB] == AHC_TRUE){
        ...
        m_DcfDB[DCF_DB_TYPE_2ND_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*uiNumeratorDB1/uiDenominator;
    ...
    if(m_bEnableDCF[DCF_DB_TYPE_1ST_DB] == AHC_TRUE){
        ...
        m_DcfDB[DCF_DB_TYPE_1ST_DB].uiClustersTH = m_sMediaInfo.ulTotalClusters*uiNumeratorDB0/uiDenominator;
    }
