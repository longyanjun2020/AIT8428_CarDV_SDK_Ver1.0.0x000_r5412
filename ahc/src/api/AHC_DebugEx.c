...
void AIHC_DBG_PrintTCB(OS_TCB* pTCB)
{
	...
	if (pulStackData >= pulBottom) {
		while( pulStackData < pulTop )
		{
		    ...
		    i++;
		    ulSKSize ++;
		    if (ulSKSize > ulSKSize) {
		    	break;
		    }
		} 
	}
	else {
		printc("pulStackData > OSTCBStkBottom\r\n");
	}
...
INT16U AHC_DBG_GetCurPrioEx(void)
{
    ...
    return OSTCBCur->OSTCBPrio + isISR;
}
...
void AHC_PrintUITask(void)
{
    ...
    if( OSTCBPrioTbl[54] != 0 &&
        OSTCBPrioTbl[54] != (OS_TCB *)1 )
    {
        AIHC_DBG_PrintTCB(OSTCBPrioTbl[54]);
    }
}
