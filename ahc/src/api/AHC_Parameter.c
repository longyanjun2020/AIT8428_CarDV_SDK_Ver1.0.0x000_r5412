...
AHC_BOOL AHC_SetParam(AHC_PARAM_ID wParamID, UINT32 wValue)
{
	...
	switch(wParamID) {
		...
		#if ENABLE_STROBE
		// calculated 
		case PARAM_ID_STROBE_CALCULATED:
			glAhcParameter.bCalculated = (AHC_BOOL)wValue;
			break;
		case PARAM_ID_STROBE_CALCULATED_NUMBER:
			glAhcParameter.ulCalculatedNumber = wValue;
			break;
		// preflash	
		case PARAM_ID_STROBE_PREFLASH:
			glAhcParameter.bPreFlash = (AHC_BOOL)wValue;
			break;
		case PARAM_ID_STROBE_PREFLASH_NUMBER:
			glAhcParameter.ulPreFlashNumber = wValue;
			break;
		case PARAM_ID_STROBE_WAIT_PREFLASH_TIME:
			glAhcParameter.ulWaitPreFlashTime = wValue;
			break;
		case PARAM_ID_STROBE_PREFLASH_TIME:
			glAhcParameter.ulPreFlashTime = wValue;
			break;
		case PARAM_ID_STROBE_TIME_BETWEEN_TWO_FLASH:
			glAhcParameter.ulTimeBetweenTwoFlash = wValue;
			break;
		// main flash
		case PARAM_ID_STROBE_MAINFLASH:
			glAhcParameter.bMainFlash = (AHC_BOOL)wValue;
			break;
		case PARAM_ID_STROBE_WAIT_MAINFLASH_TIME:
			glAhcParameter.ulWaitMainFlashTime = wValue;
			break;
		case PARAM_ID_STROBE_MAINFLASH_TIME:
			glAhcParameter.ulMainFlashTime = wValue;
			break;
		#endif
		// m shutter
		#if (ENABLE_MECHANICAL_SHUTTER)
		case PARAM_ID_MECHANICAL_SHUTTER:
			glAhcParameter.bMechanicalShutter = (AHC_BOOL)wValue;
			break;
		case PARAM_ID_MECHANICAL_SHUTTER_TIME:
			glAhcParameter.ulMechanicalShutterTime = wValue;
			break;
		#endif
...
AHC_BOOL AHC_GetParam(AHC_PARAM_ID wParamID, UINT32 *wValue)
{
	...
	switch(wParamID) {
		...
		// calculated 
		case PARAM_ID_STROBE_CALCULATED:
			*wValue = glAhcParameter.bCalculated;
			break;
		case PARAM_ID_STROBE_CALCULATED_NUMBER:
			*wValue = glAhcParameter.ulCalculatedNumber;
			break;
		// pre flash	
		case PARAM_ID_STROBE_PREFLASH:
			*wValue = glAhcParameter.bPreFlash;
			break;
		case PARAM_ID_STROBE_PREFLASH_NUMBER:
			*wValue = glAhcParameter.ulPreFlashNumber;
			break;
		case PARAM_ID_STROBE_WAIT_PREFLASH_TIME:
			*wValue = glAhcParameter.ulWaitPreFlashTime;
			break;
		case PARAM_ID_STROBE_PREFLASH_TIME:
			*wValue = glAhcParameter.ulPreFlashTime;
			break;
		case PARAM_ID_STROBE_TIME_BETWEEN_TWO_FLASH:
			*wValue = glAhcParameter.ulTimeBetweenTwoFlash;
			break;
		// mainflash	
		case PARAM_ID_STROBE_MAINFLASH:
			*wValue = glAhcParameter.bMainFlash;
			break;
		case PARAM_ID_STROBE_WAIT_MAINFLASH_TIME:
			*wValue = glAhcParameter.ulWaitMainFlashTime;
			break;
		case PARAM_ID_STROBE_MAINFLASH_TIME:
			*wValue = glAhcParameter.ulMainFlashTime;
			break;
		#endif
		
		#if (ENABLE_MECHANICAL_SHUTTER)
		case PARAM_ID_MECHANICAL_SHUTTER:
			*wValue = glAhcParameter.bMechanicalShutter;
			break;
		case PARAM_ID_MECHANICAL_SHUTTER_TIME:
			*wValue = glAhcParameter.ulMechanicalShutterTime;
			break;
		#endif
...
#if(AIT_INT_USE==1)	
	ExportMenuAtom((char*)"SD:0:\\usercfg.txt", TXT_COMMENT | TXT_MENU_ALL | TXT_VERSION);
#endif
...
{	
	...
	// Old way, for debug
	// DramToFile("SF:1:\\MenuSetting.bin",  (INT8*)MenuSettingConfig(),sizeof(MenuInfo));
	ImportMenuInfo(MenuSettingConfig());
...
#if(AIT_INT_USE==1)
	ExportMenuAtom("SD:0:\\MenuConfig.txt", TXT_COMMENT | TXT_VERSION);
	DefaultMenuSettingInit();
	ImportMenuInfo(DefaultMenuSettingConfig());
	ExportMenuAtom("SD:0:\\DefMenuConfig.txt", TXT_COMMENT | TXT_VERSION);
#endif	
	return AHC_TRUE;
}
