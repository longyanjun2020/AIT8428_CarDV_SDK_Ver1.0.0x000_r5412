...
extern MMP_BOOL SDUpdateIsFWExisted(void);
...
AHC_BOOL AHC_CheckMedia_FormatFree( UINT8 byMediaID )
{
	...
	bFWUpdate = SDUpdateIsFWExisted();
    
    if (err !=MMP_ERR_NONE && bFWUpdate == AHC_FALSE){ //If no need to update FW, dis-mount Media
...
AHC_BOOL AHC_CheckStorageMediaAccess(UINT8 byMediaID, MMP_BYTE *bpVolume)
{
	...
	if ( (byMediaID == AHC_MEDIA_MMC)||(byMediaID == AHC_MEDIA_MMC1) ) {
		...
		if(err){
			...
			printc("bpTestFileName FileOpen fail,skip SD card check \r\n");
		}
		else {
			err = AHC_FS_FileClose(ulTestFileID);
			if(err){         
				printc("bpTestFileName FileClose fail \r\n");	
            }
            else {
    			MMPD_FS_CacheClear();
    			...
    			if(err) {				
    				printc("bpTestFileName FileGetInfo fail \r\n");	 
    				printc("AHC_MountStorageMedia fail \r\n");	
    			}
    			else {
    				err = AHC_FS_FileRemove( bpTestFileName,STRLEN(bpTestFileName) );
    				if(err) {
    					printc("bpTestFileName FileRemove fail \r\n");				
                    }
                    else {
                        bResult = AHC_TRUE;
    					//printc(FG_YELLOW("AHC_CheckStorageMediaAccess OK!!!\r\n"));				
                    }
    			}
            }
		}	
...
AHC_BOOL AHC_MountStorageMediaWithoutDCF(UINT8 byMediaID)
{
    ...
    */
    ...
    {
        printc(FG_RED("AHC_CheckStorageMediaAccess Failed!!!\r\n"));
	...
	if (err == MMP_ERR_NONE) {
        ...
        if(AHC_TRUE == mount_status){
            ...
            if((AHC_MEDIA_MMC == byMediaID)|| (AHC_MEDIA_MMC1 == byMediaID)){
                ...
                #if 0//(VR_SEAMLESS) && (SUPPORT_STORAGE_ALLOCATION)
                printc("Clear SD space \r\n");
                /*
                check the storage allocation of DTDCF.
                */
                if(AHC_MEDIA_MMC == byMediaID){
                	INT32 ubStorageMode = 0;
                	UINT8 ubCheckMode = STORATE_CHECK_FOLDER_SIZE;
                	
                
                	if(AHC_CheckStorageAllocate(ubCheckMode) == AHC_FALSE) {
...
AHC_BOOL AHC_IsSDInserted(void)
{
    ...
    static AHC_BOOL bFirst = AHC_TRUE;
    MMP_UBYTE tempValue;
    ...
    //Skip below to avoid GPIO semaphore waiting in driver.
    if( bFirst == AHC_TRUE )
    {
...
AHC_BOOL AHC_Media_GetSmartCardInfo(PSMART_CARD_STATUS ptSmartCardInfo)
{
	...
	if(err){
		printc("Get LONGSYS Smart Card Info Err : %#x\n", err);
		return AHC_FALSE;
	}
	...
	{
		printc("Get Smart Info ERROR or It's not Longsys Smart Card\r\n");
		return AHC_FALSE;
	}
}
#endif
