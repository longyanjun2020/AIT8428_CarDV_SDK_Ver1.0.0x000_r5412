...
#define THUMBNAIL_BUFFER_NUMBER     AHC_THUMB_BUF_NUM
...
#define	BEGIN_LAYER(l)	{	extern AHC_OSD_INSTANCE *m_OSD[]; \
							if (m_OSD[l] && (m_OSD[l]->gui.gui_semi < 0xFE)) { \
								if (AHC_OS_AcquireSem(m_OSD[l]->gui.gui_semi, 1000) != OS_NO_ERR) \
								    {printc("ERROR:%s %d Lock fail, OSD-ID[%d]!!\r\n", __func__, __LINE__, l);} \
							} \
							else {printc("ERROR:%s %d GUI layer error\r\n", __func__, __LINE__);} \
						}
...
typedef struct _AHC_GUI_CONTEXT{
    ...
    GUI_COLOR				gui_bkclr;
    UINT8					gui_pen;
    //AHC_OSD_FILP_DRAW_MODE	gui_drawmode;
...
typedef struct _AHC_OSD_INSTANCE {
    ...
    UINT8   				AlphaWeight[8];	///< The alpha blending weight for all layers
