...
AHC_BOOL AIHC_Thumb_DrawFocus(UINT16 uwFocusIndex, AHC_BOOL bClear)
{
    ...
    #if (AHC_BROWSER_DRAW_FOCUS_THICK_BORDER)
    ...
    if( m_ubPlaybackRearCam == AHC_TRUE && (VIDRECD_MULTI_TRACK == 0 || (AHC_UF_GetCurrentDBFlag() & DCFDT_DB_FLAG_HAS_REAR_CAM))){ 
    ...
    if (bClear) {
        ...
        if(ubClearFlag)
        {
			...
			AHC_OSDSetColor(m_ulDrawingThumbOSD, RGB565_COLOR_GRAY); //20150313 Mark to keep the border in thumbnail mode. 
			#else
...
AHC_BOOL AHC_Thumb_DrawPage(AHC_BOOL bForce)
{
    ...
    UINT32						tmpThumbPosY;
    #endif
#if (EN_WATCH_THUMB_COST == 1)
    UINT32						t1,t2; //remove for warning
#endif
    GUI_COLOR                   CurrentGUIColor,CurrentBkColor;
...
#endif
    AIHC_Thumb_CalculateRatio( ulVRThumbJpgW, ulVRThumbJpgH, TargetThumbWidth, TargetThumbHeight, &usTmpW, &usTmpH);
			...
			*/
...
#if (SUPPORT_VR_THUMBNAIL)
            ubHitThumbJpgInFile = MMP_FALSE;													  

            if ((uiDecodeIdx != ulCurIdx)) {
                ulCurIdx = uiDecodeIdx;
                error = MMPS_VIDPLAY_GetThumbnail(FilePathName,
                            STRLEN(FilePathName),
                            PRE_ALLOC_VR_THUMBNAIL_SIZE,
                            (MMP_UBYTE**)&TargetThumbAddr,
                            &TargetThumbSize);

                ubHitThumbJpgInFile = (MMP_ERR_NONE == error) ? MMP_TRUE : MMP_FALSE;
                
                if (error) {
                    RTNA_DBG_Str0(FG_YELLOW("ThumbErr="));
                    RTNA_DBG_Long0(error);
                    RTNA_DBG_Str0("\r\n");
                }                    
            }	

            
            if ( ubFileType == DCF_OBG_JPG || ubHitThumbJpgInFile) 
#else
...
#endif			
            {
                ...
                if (!ubHitThumbJpgInFile) {
                    ...
                    jpeginfo.ulJpegBufAddr          = TargetThumbAddr;
                    jpeginfo.ulJpegBufSize          = TargetThumbSize;
                    jpeginfo.usPrimaryWidth         = ulVRThumbJpgW;
                    jpeginfo.usPrimaryHeight        = ulVRThumbJpgH;
                    //jpeginfo.usThumbWidth         = usTmpW;
                    //jpeginfo.usThumbHeight        = usTmpH;
                }
#endif //#if (SUPPORT_VR_THUMBNAIL == 0)
      			#if (AHC_BROWSER_DRAW_GRAY_BORDER)
                {
      			...
      			#endif //#if (AHC_BROWSER_DRAW_GRAY_BORDER)
                ...
                if ( ( OSDColor == OSD_COLOR_RGB565 ) || ( OSDColor == OSD_COLOR_RGB888 ) ) 
                {
                    ...
                    if ( error == MMP_ERR_NONE) {
                        ...
                        if (ubHitThumbJpgInFile) {
                            uiDecodeIdx--;
                            k--;
                            RTNA_DBG_Str0(FG_YELLOW("[Warning]DecThumbnailErr Hdl!\r\n"));
                        }
                        else
#endif						
						{
                ...
                if (ubHitThumbJpgInFile) {
                    DBG_AutoTestPrint(ATEST_ACT_BROW_VID_0x0006,
                                        ...
                                        jpeginfo.usPrimaryWidth,
                                        jpeginfo.usPrimaryHeight,
                                        gbAhcDbgBrk);
...
#endif                    
                {
                    DBG_AutoTestPrint(ATEST_ACT_BROW_PHOTO_0x0007, ATEST_STS_RSOL_SIZE_0x0004, jpeginfo.usPrimaryWidth, jpeginfo.usPrimaryHeight, gbAhcDbgBrk);
                    DBG_AutoTestPrint(ATEST_ACT_BROW_PHOTO_0x0007, ATEST_STS_END_0x0003, 0, error, gbAhcDbgBrk);
                }
			#endif
			
				#if(UVC_HOST_VIDEO_ENABLE)
                #if(AHC_BROWSER_DRAW_DUAL_FILES)
                {
                	UINT8    status;
                    AHC_BOOL	bFileExist;       
                	
                	//AHC_UF_IsRearCamFileExistbyIndex(uiDecodeIdx,&bFileExist);
                	AHC_UF_CheckFileStatus(uiDecodeIdx, &status);
                	if(status&DCF_CAM_REAR_EXIST)
                		bFileExist = AHC_TRUE;
                	else
                		bFileExist = AHC_FALSE;
					//bFileExist = (status&DCF_CAM_REAR_EXIST != 0) ? AHC_TRUE : AHC_FALSE; 
                	if(bFileExist)
                	{
                	    AHC_OSDSetColor(m_ulDrawingThumbOSD, GUI_BLACK);
                		AHC_OSDDrawFillRect(m_ulDrawingThumbOSD,
                                            ThumbPosX - 2 + TargetThumbWidth*3/4,
                                            ThumbPosY + 2,
                                            ThumbPosX - 2 + TargetThumbWidth,
                                            ThumbPosY + 2 + TargetThumbHeight/3);
                        
                	    AHC_OSDSetColor(m_ulDrawingThumbOSD, GUI_WHITE);
                		AHC_OSDDrawFillRect(m_ulDrawingThumbOSD,
                                            ThumbPosX - 1 + TargetThumbWidth*3/4,
                                            ThumbPosY + 3,
                                            ThumbPosX - 3 + TargetThumbWidth,
                                            ThumbPosY + 1 + TargetThumbHeight/3);
                	    AHC_OSDSetBkColor(m_ulDrawingThumbOSD,GUI_WHITE);                    
                        AHC_OSDSetColor(m_ulDrawingThumbOSD,GUI_BLACK);
						AHC_OSDSetFont(m_ulDrawingThumbOSD,&GUI_Font16B_1);
						AHC_OSDDrawText(m_ulDrawingThumbOSD, (UINT8*)"D", ThumbPosX + 3 + TargetThumbWidth*3/4, ThumbPosY + 3, 1);
                	}
                }
                #endif	
                #endif
...
//#endif													  
                    ...
                    #if(AHC_BROWSER_DRAW_DUAL_FILES)
                    {
                    	...
                    	//AHC_UF_IsRearCamFileExistbyIndex(uiDecodeIdx,&bFileExist);
                    	AHC_UF_CheckFileCamStatus(uiDecodeIdx, &status);
                    	if(status&DCF_CAM_REAR_EXIST)
                    		bFileExist = AHC_TRUE;
                    	else
                    		bFileExist = AHC_FALSE;
...
#if (EN_WATCH_THUMB_COST == 1)	
    AHC_OS_GetTime(&t2);
    printc(FG_YELLOW("THUMBNAIL_COST: %d %d\r\n"), t1, (t2-t1));
#endif
    //restore color
    AHC_OSDSetColor(m_ulDrawingThumbOSD, CurrentGUIColor);
    AHC_OSDSetBkColor(m_ulDrawingThumbOSD, CurrentBkColor);
