...
void AIHC_SetGUIMemEndAddr(UINT32 addr) 
{
	#if (SWITCH_MODE_FREEZE_WIN)
	MMPF_Display_SetTempMemHeapEnd((MMP_ULONG)addr);
	#endif
	MMPS_System_SetMemHeapEnd((MMP_ULONG)addr);
	printc(BG_BLUE("[INFO]END OF OSD-BUFF 0X%08X\r\n"),addr);
}
...
//    AHC_OSDClearBuffer(0);
    ...
    MMPS_Display_SetWinPriority(WMSG_LAYER_WINDOW_ID, OSD_LAYER_WINDOW_ID, UPPER_IMAGE_WINDOW_ID, LOWER_IMAGE_WINDOW_ID);
	...
	#if	defined(MALLOC_OSDBUFF_DOWN_GROWTH)&&(MALLOC_OSDBUFF_DOWN_GROWTH)
	AIHC_SetGUIMemEndAddr(m_ulOSDTempBaseAddr);
	#endif
...
AHC_BOOL AHC_OSDCreateBuffer(UINT16 uwDisplayID, UINT16 uwWidth, UINT16 uwHeight, AHC_OSD_COLOR_FMT uiColorFormat)
{
    ...
    printc(FG_RED("ERR_OSD: duplicate ratio exceed 6X!\r\n"));
	#else //TBD
	m_OSD[uwDisplayID]->DuplicateW 				= OSD_DISPLAY_SCALE_LCD - 1;
