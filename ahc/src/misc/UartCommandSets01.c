...
#include "mmpf_usbh_uvc.h" //For compiler warning.
#include "mmps_3gprecd.h"
...
UINT16 guwKeyCnt   = 0x0000;
UINT32 gulLastKCmd = 0x00000000;
#endif
...
extern SD_UPDATER_ERR SDWrWpArea(void);
extern SD_UPDATER_ERR SDDumpWpAreaBak(MMP_BYTE *bFileName);
extern MMP_ERR MMPF_SF_SetProtWpArea(void);
extern MMP_ERR MMPF_SF_ClrProtArea(void);
#endif

extern MMP_BOOL ubStartPrintVifCnt;
...
void UartCmd_KeyDef(char* szParam)
{
    ...
    if (!gubMmpDbgBk) {
	    ...
	    guwKeyCnt++;        
		gulLastKCmd = (UINT32)((ulKeyDef<<16)+ guwKeyCnt);
	    RTNA_DBG_Str0("@KEY:");
	    RTNA_DBG_Long0(gulLastKCmd);
	 	RTNA_DBG_Str0("\r\n");
	}
#endif    
...
void UartOSDSetSemiTransparent( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDSetTransparentEnable( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDSetAlphaBlendingEnable( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDSetRotate( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDSetMirror( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDInit( char* szParam )
{
    ...
    bStatus = AHC_OSDInit(	iWidth,
    						...
    						iColorFormat,AHC_TRUE, OSD_ROTATE_DRAW_NONE);

    printc( "status = %d\n", bStatus );
}

void UartOSDUninit( char* szParam )
{
    AHC_BOOL bStatus;
...
void UartOSDCreateBuffer( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int iWidth;
...
void UartOSDSetCurrentDisplay( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDRefresh( char* szParam )
{
    AHC_BOOL bStatus;
...
void UartOSDSetSetActive( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int enable;
...
void UartOSDSetClearBuffer( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDDestroyBuffer( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDSetColor( char* szParam )
{
    int iCount;
    int iColor;
...
void UartOSDDrawPixel( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDDrawRect( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDDrawFillRoundedRect( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDDrawFillRect( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDDrawInvertRect( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    int id;
...
void UartOSDDrawText( char* szParam )
{
    AHC_BOOL bStatus;
    int iCount;
    char str[64]={0};
...
void UartOSDDrawJpeg( char* szParam )
{
#if 0
    AHC_BOOL bStatus;
    int iCount;
...
void UartFormatSD( char* szParam )
{
	MMPF_FS_Format( "SD:0:" );
}

void UartCounterGet( char* szParam )
{
    MMP_ULONG  ulCounter;
...
void UartCounterInit( char* szParam )
{
    MMP_ULONG  ulCounter;
...
void UartChangeUSBRearCamType( char* szParam )
{
	...
	printc( "UartChangeUSBRearCamType(%d)\r\n", id);
#endif
}

#if (SUPPORT_TIMING_MARK == 1)
void UartTimingMarkInit( char* szParam )
{   
...
void UartTimingMark( char* szParam )
{    
	MMPF_TimingMarkF( 0x01, "[TEST] %d ", iTiming); //Mark TBD TIMING_GROUP_BOOT
	iTiming++;	
}

void UartTimingMarkDump( char* szParam )
{
	MMPF_TimingStop();
...
//------------------------------------------------------------------------------
#if (CPU_ID == CPU_A)
extern MMP_BOOL ADAS_CTL_GetAdasInitParam(ADAS_init_params *pADAS_InitParams);
extern void ADAS_CTL_SetADASOutputLog(MMP_BOOL bADASOutputLog);
#endif
...
void UartCmd_GetRaw(char* szParam)
{
    ...
    if( bySensorIndex == 0 )
    {
        ...
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
		...
		uiAddress = 0x80006B14;
    	uiValue = 0x00;//0x0300 0000//AIT_REG_D   RAWPROC_S_ADDR_PLANE0;// 0x14
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
		uiAddress = 0x80006B15;
    	uiValue = 0x00;//0x0300 0000
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
		uiAddress = 0x80006B16;
    	uiValue = 0x00;//0x0300 0000
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
		uiAddress = 0x80006B17;
    	uiValue = 0x03;//0x0300 0000
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
...
void UartCmd_GetRawHDR(char* szParam)
{
    MMP_UBYTE bySensorIndex = 0;
	MMP_ULONG uiAddress;
	MMP_ULONG uiHDR_L_Address;
	MMP_ULONG uiHDR_S_Address;
    MMP_UBYTE uiValue;
    MMP_UBYTE uiValue1,uiValue2,uiValue3;
    MMP_USHORT W,H;
    char* fileName_l = "SD:\\hdr_l.raw"; //Add Reg: 0x80006B14~7 (p0)
    char* fileName_s = "SD:\\hdr_s.raw"; //Add Reg: 0x80006BA0~3 (p1)
    ...
    if( bySensorIndex == 0 )
    {
    	uiAddress = 0x80006022; //Reg: Sensor_0 Video output data enable
    	uiValue = 0x00;
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
        
        uiAddress = 0x800060B0; //Reg: Sensor_0 Raw data output enable
    	uiValue = 0x01;
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
        
        // hdr_l raw data add  //Add Reg: 0x80006B14~7 (p0)
        uiAddress = 0x80006B17;
    	uiValue3 = *(MMP_UBYTE*)uiAddress;
    	uiAddress = 0x80006B16;
    	uiValue2 = *(MMP_UBYTE*)uiAddress;
		uiAddress = 0x80006B15;
    	uiValue1 = *(MMP_UBYTE*)uiAddress;
    	uiAddress = 0x80006B14;
    	uiValue = *(MMP_UBYTE*)uiAddress;
    	uiHDR_L_Address= ((MMP_ULONG)uiValue3 << 24) | ((MMP_ULONG)uiValue2 << 16) | ((MMP_ULONG)uiValue1 << 8) | uiValue; 
        
        // hdr_s raw data add  //Add Reg: 0x80006BA0~3 (p1)
        uiAddress = 0x80006BA3;
    	uiValue3 = *(MMP_UBYTE*)uiAddress;
    	uiAddress = 0x80006BA2;
    	uiValue2 = *(MMP_UBYTE*)uiAddress;
		uiAddress = 0x80006BA1;
    	uiValue1 = *(MMP_UBYTE*)uiAddress;
    	uiAddress = 0x80006BA0;
    	uiValue = *(MMP_UBYTE*)uiAddress;
    	uiHDR_S_Address= ((MMP_ULONG)uiValue3 << 24) | ((MMP_ULONG)uiValue2 << 16) | ((MMP_ULONG)uiValue1 << 8) | uiValue; 
        
        uiAddress = 0x80006B19; //Reg: Raw Store Source Selection
    	uiValue = 0x00;
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;
        
        uiAddress = 0x80006B10; //Reg: Raw Mode Select 
    	uiValue = 0x0A;
        *(volatile MMP_UBYTE*)(uiAddress) = uiValue;   
	...
	DramToFile((INT8*)fileName_l, (INT8*)uiHDR_L_Address, (UINT32)W*H);	
	DramToFile((INT8*)fileName_s, (INT8*)uiHDR_S_Address, (UINT32)W*H);	
	
	printc("UartCmd_GetRaw: Done!!\r\n");
}

void UartCmd_GetRearCamBuf(char* szParam)
{
	MMP_ULONG uiAddress;
    MMP_UBYTE uiValue;
    MMP_UBYTE uiValue1;
	...
	MMP_ULONG Address;
    MMP_USHORT W,H;
    char* fileName = "SD:\\RearCam.raw";
      
    uiAddress = 0x80002D03;
	uiValue3 = *(MMP_UBYTE*)uiAddress;
	uiAddress = 0x80002D02;
	uiValue2 = *(MMP_UBYTE*)uiAddress;
	uiAddress = 0x80002D01;
	uiValue1 = *(MMP_UBYTE*)uiAddress;
	uiAddress = 0x80002D00;
	uiValue = *(MMP_UBYTE*)uiAddress;
	Address= ((MMP_USHORT)uiValue3 << 24) | ((MMP_USHORT)uiValue2 << 16) | ((MMP_USHORT)uiValue1 << 8) | uiValue; 
	
    uiAddress = 0x80002D51;
	uiValue1 = *(MMP_UBYTE*)uiAddress;
	uiAddress = 0x80002D50;
	uiValue = *(MMP_UBYTE*)uiAddress;
	W= ((MMP_USHORT)uiValue1 << 8) | uiValue; 
   
    uiAddress = 0x80002D55;
	uiValue1 = *(MMP_UBYTE*)uiAddress;
	uiAddress = 0x80002D54;
	uiValue = *(MMP_UBYTE*)uiAddress;
	H= ((MMP_USHORT)uiValue1 << 8) | uiValue; 
   
    printc("UartCmd_GetRearCamBuf.....\r\n");
	printc("W = %d\r\n", W);
    printc("H = %d\r\n", H);
    
	DramToFile((INT8*)fileName, (INT8*)Address, (UINT32)W*H*3/2);	
    
	printc("UartCmd_GetRearCamBuf: Done!!\r\n");
}
...
void UartCmd_SNR_Flip(char* szParam)
{
	...
	AHC_SNR_SetFlipDir(PRM_SENSOR, tar);
...
{
	unsigned char soundtype,delay;
	
	sscanfl( szParam, "%d %d",&soundtype,&delay);
	...
	if(delay > 0)
		AHC_OS_SleepMs(delay);
	
	AHC_PlaySoundEffect(soundtype);//AHC_SOUNDEFFECT_BUTTON
}
...
void UartCmd_TouchPower_Operation(char* szParam)
{
	UINT32 reg;

	#define APK_TOUCH_KEY_PWR	(MMP_GPIO_MAX)//MMP_GPIO23
...
void UartCmdSetDebugLevel( char* szParam )
{
...
void UartCmdGetDebugBreak( char* szParam )
{
	...
	RTNA_DBG_Str0(FG_YELLOW("@gubMmpDbgBk="));
	DBG_D0(gubMmpDbgBk,1);
	RTNA_DBG_Str0("\r\n");	
	RTNA_DBG_Str0(FG_YELLOW("@ubAhcDbgBrkFg="));
    DBG_D0(ubAhcDbgBrkFg,1);
	RTNA_DBG_Str0("\r\n");
}

void UartCmdDeadAbort( char* szParam )
{
	*(volatile MMP_UBYTE *)0xCD99ee1 = 0;
}

void UartCmdDeadAbs( char* szParam )
{
	*(volatile MMP_UBYTE *)0x80006902 = 0xFF;
}

void UartCmdSetEmergAction( char* szParam )
{
	...
	if ((sMergerStatus == VIDENC_FW_STATUS_NONE) || (sMergerStatus == VIDENC_FW_STATUS_STOP))
	{
    	switch (uiMode) {
    	...
    	MMPS_3GPRECD_SetEmergActionType(uiMode);
	}
	else
	{
        printc("In recording, failed to reset action.\r\n");
	}
    #endif
}
#if (EN_WP_AREA_CONFIG == 1)
void UartCmdDumpWpArea (char* szParam )
{
    AHC_FS_FileRemove(SD_WP_BAK_BIN_NAME,  STRLEN(SD_WP_BAK_BIN_NAME));
    SDDumpWpAreaBak(SD_WP_BAK_BIN_NAME);
}

void UartCmdWrWpArea (char* szParam )
{
    SDWrWpArea();
}

void UartCmdReadWpSts (char* szParam )
{
    MMP_UBYTE ubStatus1, ubStatus2, ubStatus3;
    
    MMPF_SF_ReadStatus(1, &ubStatus1);
    MMPF_SF_ReadStatus(2, &ubStatus2);
    MMPF_SF_ReadStatus(3, &ubStatus3);

	RTNA_DBG_Str0("Rd WpSts:");
	RTNA_DBG_Byte0(ubStatus1);
	RTNA_DBG_Str0(":");
	RTNA_DBG_Byte0(ubStatus2);
	RTNA_DBG_Str0(":");
	RTNA_DBG_Byte0(ubStatus3);
	RTNA_DBG_Str0("\r\n");
}

void UartCmdReadSecuritySts (char* szParam )
{
    MMP_UBYTE ubStatus;
    
    //MMPF_SF_ReadSecurityReg(&ubStatus);

	RTNA_DBG_Str0("Rd ScurSts:");
	RTNA_DBG_Byte0(ubStatus);
	RTNA_DBG_Str0("\r\n");
}

void UartCmdWriteWpSts (char* szParam )
{
    MMPF_SF_SetProtWpArea();
}

void UartCmdClrWp (char* szParam )
{
    MMPF_SF_ClrProtArea();
}
#endif
...
#if (EN_AUTO_TEST_LOG == 1)
void UartCmdClrAutoTestLog( char* szParam )
{  
	RTNA_DBG_Str0(FG_YELLOW("@ClrAutoTestLog!\r\n"));
	MMPS_System_ClrAutoTestLog();
}

void UartCmdDumpAutoTestLog( char* szParam )
{
    MMPS_System_DumpAutoTestLog();
}

void UartCmdGetLastKCmd( char* szParam )
{
	RTNA_DBG_Str0("@LKEY:");
	RTNA_DBG_Long0(gulLastKCmd);
	RTNA_DBG_Str0("\r\n");
...
void UartCmdSetUserDefCmdNum( char* szParam )
{
    MMP_ULONG uiValue;

	sscanfl( szParam, "%d", &uiValue );
    printc(FG_YELLOW("@UserDefCmdL=%d\r\n"), uiValue);

    MMPS_System_SetUserDefCmdNum(uiValue);
}

int Menu_Atoms_ConfigSize(void);
int Menu_Atoms_Config(int i);
void UartCmdDumpMenuSetting( char* szParam )
{
	MMP_ULONG i;
    MMP_ULONG ubResId, ubRateId;
    MMP_ULONG ulCapW, ulCapH;

    printc(FG_YELLOW("$GetMenuInfo size:%d"), Menu_Atoms_ConfigSize());  
    for( i=0; i< Menu_Atoms_ConfigSize(); i++) { 
        if (!(i%16)) {         
            RTNA_DBG_Str(0, "\r\n");
            RTNA_DBG_Byte(0, i);
            RTNA_DBG_Str(0, "|");
        }
        RTNA_DBG_Byte(0, Menu_Atoms_Config(i));        
    }        
    RTNA_DBG_Str(0, "\r\n");

    AIHC_VIDEO_GetMovieCfgEx(0, AHC_VIDEO_RESOLUTION, &ubResId);
    AIHC_VIDEO_GetMovieCfgEx(0, AHC_FRAME_RATE, &ubRateId);
    CaptureFunc_SetResolution(MenuSettingConfig()->uiIMGSize, AHC_TRUE);
    AHC_GetImageSize(STILL_CAPTURE_MODE, &ulCapW, &ulCapH);
    
    printc(FG_YELLOW("$Vrecres(id)=%d\r\n"), ubResId);
    printc(FG_YELLOW("$Frate(id)=%d\r\n"), ubRateId);
    printc(FG_YELLOW("$Capsize(id)=%d,%d,%d\r\n"), MenuSettingConfig()->uiIMGSize, ulCapW, ulCapH);            
}

void UartCmdDumpSnrType( char* szParam ) 
{
    MMP_BYTE  ubDefType[64];
    
    if (CAM_CHECK_SCD(SCD_CAM_NONE))
        strcpy(ubDefType, DEF2STR(SCD_CAM_NONE));
    else if (CAM_CHECK_SCD(SCD_CAM_BAYER_SENSOR))
    	strcpy(ubDefType, DEF2STR(SCD_CAM_BAYER_SENSOR));
    else if (CAM_CHECK_SCD(SCD_CAM_TV_DECODER))
    	strcpy(ubDefType, DEF2STR(REAR_CAM_TYPE_TV_DECODER));
    else if (CAM_CHECK_SCD(SCD_CAM_YUV_SENSOR))
    	strcpy(ubDefType, DEF2STR(SCD_CAM_YUV_SENSOR));
	else
		strcpy(ubDefType, "Unknow sensor interface!");
    ...
    if (CAM_CHECK_SCD(SCD_CAM_TV_DECODER)) {
        ...
        if (CAM_CHECK_USB(USB_CAM_AIT))
            strcpy(ubDefType, DEF2STR(REAR_CAM_TYPE_AIT));
        else if (CAM_CHECK_USB(USB_CAM_NONE))
            strcpy(ubDefType, DEF2STR(REAR_CAM_TYPE_NONE));
    	else if (CAM_CHECK_USB(USB_CAM_SONIX_MJPEG))
            strcpy(ubDefType, DEF2STR(REAR_CAM_TYPE_SONIX_MJPEG));
        else if (CAM_CHECK_USB(USB_CAM_SONIX_MJPEG2H264))
        	strcpy(ubDefType, DEF2STR(REAR_CAM_TYPE_SONIX_MJPEG2H264));
        else
        	strcpy(ubDefType, "Unknow sensor interface!");\
    ...
    strcpy(ubDefType, DEF2STR(SENSOR_IF_PARALLEL));
#elif (SENSOR_IF == SENSOR_IF_MIPI_1_LANE)
    strcpy(ubDefType, DEF2STR(SENSOR_IF_MIPI_1_LANE));
...
#if (PRM_SENSOR_VIF_ID == MMPF_VIF_MDL_ID0)
    strcpy(ubDefType, DEF2STR(MMPF_VIF_MDL_ID0));
#else
    strcpy(ubDefType, DEF2STR(MMPF_VIF_MDL_ID1));
#endif
    printc(FG_PURPLE("%s\r\n"), ubDefType);


#if (BIND_SENSOR_AR0330 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_AR0330));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_AR0330_OTPM == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_AR0330_OTPM));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_AR0331 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_AR0331));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_AR0835 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_AR0835));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_AR1820 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_AR1820));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_IMX175 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_IMX175));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_IMX214 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_IMX214));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_IMX322 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_IMX322));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);        
#endif
#if (BIND_SENSOR_IMX326 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_IMX326));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);        
#endif
#if (BIND_SENSOR_OV2710 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_OV2710));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_OV2710_MIPI == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_OV2710_MIPI));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_OV4689 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_OV4689));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);       
#endif
#if (BIND_SENSOR_OV5653 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_OV5653));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_OV9712 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_OV9712));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_OV10822 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_OV10822));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_DM5150 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_DM5150));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_BIT1603 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_BIT1603));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_CJC5150 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_CJC5150));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_GM7150 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_GM7150));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_CP8210 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_CP8210));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_H42_MIPI == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_H42_MIPI));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_OV9732_MIPI == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_OV9732_MIPI));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_JXF02 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_JXF02));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_JXF22 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_JXF22));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_PS5226 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_PS5226));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_BRV0200 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_BRV0200));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
#if (BIND_SENSOR_BRV0500 == 1)
    strcpy(ubDefType, DEF2STR(BIND_SENSOR_BRV0500));
    printc(FG_PURPLE("$%s\r\n"), ubDefType);
#endif
}
#endif //#if (EN_AUTO_TEST_LOG == 1)
    ...
    case 7: //TV
        ...
        //Set full OSD by uwDisplayID = 0            
            AHC_OSDInit(   uwWidth/2,
            				...
            				PRIMARY_DATESTAMP_HEIGHT,
            				THUMB_DATESTAMP_WIDTH,
            				THUMB_DATESTAMP_HEIGHT,OSD_COLOR_ARGB32,AHC_TRUE, OSD_ROTATE_DRAW_NONE);
...
//	#endif
	...
	switch(i)
	{
		...
		case 2: 
		    {
                ...
                if (EDOGCtrl_GetAddSpeedCamPOI()) {
                	...
                	while(j) {      
                        if (EDOGCtrl_AddPOI_Manual(hGps)) {                        
                    ...
                    printc("Please enable AddPOI Manual first!\r\n");
                }							
		    } 
		    break; 
		case 3:
		...
		case 7:
    		...
    		float dflat = 25.055269;
    		float dflon = 121.617302;
    		double dwlat = 25.055269;
    		double dwlon = 121.617302;		
    		if (!EDOGCtrl_GetPoiDataType) {
    			EDOGCtrl_UpdatePositionInfo_double(dwlat, dwlon);
    		}
    		else {
    			EDOGCtrl_UpdatePositionInfo_float(dflat, dflon);
    		}
		...
		case 15:
    		...
    		break;
		case 16:
            EDOGCtrl_SetEnEdogLog(MMP_TRUE);
...
void UartCmd_PrintVifCnt(char* szParam)
{
	if (ubStartPrintVifCnt == MMP_TRUE)
		ubStartPrintVifCnt = MMP_FALSE;
	else
		ubStartPrintVifCnt = MMP_TRUE;
}
...
int TouchPanel_Init0(void)
{
	...
	{
		AHC_GPIO_SetOutputMode(MMP_GPIO50, AHC_TRUE);	
		AHC_GPIO_SetData(MMP_GPIO50, AHC_TRUE);
	}
	#if 1 // lyj 20181026
...
void UART_TouchPanel1(char* szParam)
{
    ...
    //TouchPanel_Init(); // lyj 20181026
}

void UART_TouchPanel2(char* szParam)
{
    // MMPF_I2cm_ReadBurstData(&m_TouchPanelI2c, NULL, usReadData, 0x64); // lyj 20181026
}
...
void UART_GPIO(char* szParam)
{
    ...
    AHC_GPIO_SetOutputMode(gpio, output);   

    if( output )
    {
		AHC_GPIO_SetData(gpio, value);        
        printc("Set GPIO%d = %d\n", gpio, value);
...
void UartCmd_SetRecordGain(char* szParam)
{
	...
	AHC_VIDEO_GetRecVolumeParam(&ubOldDgain, &ubOldAgain);
	...
	AHC_VIDEO_SetRecVolumeParam(RDgain, RAgain);
}
...
void UartCmd_GetRecordGain(char* szParam)
{
	...
	AHC_VIDEO_GetRecVolume(&iDGain, &iLAGain, &iRAGain);
}
...
UARTCOMMAND sUartCommandSample2[] =
{
	...
	{"rearcam"                     , "                ",   "UartChangeRearCamType    ",UartChangeUSBRearCamType     },
    #if (SUPPORT_TIMING_MARK == 1)
    ...
    #if(MIMF_CUSTOMIZED_SORT_TYPE)  
	{"dump_mimf"               , "output MIMF Result","output MIMF Result         ",UartCmd_MIMF_OutputResult   },
	#endif
...
void UartCmd_VidCapture( char* szParam )
{
    VideoFunc_Shutter();
}

void UartCmd_MountSD( char* szParam )
{
#if 1
...
{   
	...
	RTNA_DBG_Str0("LivePCMTransferCB: UnReadSampleCnt=");
	RTNA_DBG_Long0(ulUnReadSampleCnt);
	RTNA_DBG_Str0(" pPCMOutRing: ");
	RTNA_DBG_Long0((MMP_ULONG)pPCMOutRing);
	RTNA_DBG_Str0("\r\n");
...
void UartCmd_LiveAudio( char* szParam )
{
    ...
    switch( uiItem ) 
    {
        case 1:
            ...
            MMPS_AUDIO_InitLiveRecord(MMP_AUDIO_PCM_ENCODE, 0, ulAdcSampleRate, (MMP_LivePCMCB)LivePCMTransferCB);
            break;
...
extern MMP_USHORT m_usISPTempDataSize;
extern unsigned char CaliDataBuf[];

MMP_BYTE bISPFilePathTmp[MAX_FILE_NAME_SIZE]={0};

void UartCmd_DumpISPCaliFile( char* szParam )
{
    MMP_ULONG ulActualSize=0;
    MMP_ULONG ulFileId;
	INT8       StrIdx[4];
	INT8       i=0;
    for(i=0 ; i < ISP_CALI_FILE_NUM ;i++) {

        MEMSET(bISPFilePathTmp, 0, sizeof(bISPFilePathTmp));
	    STRCPY(bISPFilePathTmp, "SF:");
	    STRCAT(bISPFilePathTmp, ISP_CALI_PARTITION);
        STRCAT(bISPFilePathTmp,":\\");
        STRCAT(bISPFilePathTmp, ISP_CALI_FILENAME_PREFIX);
        AHC_UTILITY_Int2Str(i, StrIdx);
        STRCAT(bISPFilePathTmp, StrIdx);
        STRCAT(bISPFilePathTmp, ISP_CALI_FILENAME_EXT);

        if(MMP_ERR_NONE != MMPS_FS_FileOpen(bISPFilePathTmp, strlen(bISPFilePathTmp), "r+b", strlen("r+b"), &ulFileId))
	    {
            MMPS_FS_FileClose(ulFileId);
		    printc("MMPS_FS_FileOpen err\r\n");
	    }else {
            MEMSET(bISPFilePathTmp, 0, sizeof(bISPFilePathTmp));
            STRCPY(bISPFilePathTmp, "SD");
            STRCAT(bISPFilePathTmp,":\\");
            STRCAT(bISPFilePathTmp, ISP_CALI_FILENAME_PREFIX);
            AHC_UTILITY_Int2Str(i, StrIdx);
            STRCAT(bISPFilePathTmp, StrIdx);
            STRCAT(bISPFilePathTmp, ISP_CALI_FILENAME_EXT);
            printc("%s\r\n", bISPFilePathTmp);
            printc("UartCmd_DumpISPCaliFile  i = %d CaliDataBuf 0x%x   m_usISPTempDataSize 0x%x\r\n", i, CaliDataBuf, m_usISPTempDataSize);
    	    MMPS_FS_FileRead(ulFileId, CaliDataBuf, m_usISPTempDataSize, &ulActualSize);
    	    DramToFile(bISPFilePathTmp, (INT8*)CaliDataBuf, (UINT32)m_usISPTempDataSize);
            MMPS_FS_FileClose(ulFileId);
        }
    }
}
...
MMP_ULONG testfileNUM;
char gFileName[255];

void UartCmd_DumpSIFInfo( char* szParam )
{
    UINT32 uiAddr = 0;
    UINT32 uiSize = 0;    
    MMP_ULONG ulDmaAddr;
    ...
    sscanfl( szParam, "%x %x", &uiAddr, &uiSize);
	printc("SIF: %x, %x, %x\r\n", uiAddr, uiSize, ulDmaAddr);
	if (MMPF_SF_FastReadData(uiAddr, ulDmaAddr, uiSize) == MMP_ERR_NONE) {
		MMP_ULONG fileID = 0;
		MMP_ULONG ulwriteSize = 0;
		sprintf(gFileName, "SD:\\dumpsf%d.bin", testfileNUM);
		MMPF_FS_FOpen(gFileName, "wb", &fileID);
		MMPF_FS_FWrite(fileID, (void *)ulDmaAddr, uiSize, &ulwriteSize);
		MMPF_FS_FClose(fileID);
		testfileNUM ++;
	}
}
#if (DEBUG_UART_TO_FILE)
void UartCmd_DebugEnable( char* szParam )
{
	MMP_BOOL bSDEnable, bSFEnable, bKeyInfoEnable;
    sscanfl( szParam, "%x %x %x", &bSFEnable, &bSDEnable, &bKeyInfoEnable);
	printc("Debug: SF:%x, SD:%x, Key:%x\r\n", bSFEnable, bSDEnable, bKeyInfoEnable);
	MMPF_Debug_Enable(bSFEnable, bSDEnable, bKeyInfoEnable);
}
#endif
extern MMP_BYTE byDspyWrOfst[6];
void UartCmd_DisplayIQ( char* szParam )
{
    UINT16 uiParam1 = 0;
    UINT16 uiParam2 = 0;
    UINT16 uiParam3 = 0;
    ...
    sscanfl( szParam, "%d %d %d %d %d %d", &uiParam1, &uiParam2, &uiParam3, &uiParam4, &uiParam5, &uiParam6);

    printc("--UartCmd_DisplayIQ-- %d %d %d %d %d %d\r\n", uiParam1, uiParam2, uiParam3, uiParam4, uiParam5, uiParam6);

    byDspyWrOfst[0] = uiParam1; 
    byDspyWrOfst[1] = uiParam2; 
    byDspyWrOfst[2] = uiParam3;
    byDspyWrOfst[3] = uiParam4; 
    byDspyWrOfst[4] = uiParam5; 
    byDspyWrOfst[5] = uiParam6; 
   
}

#define LCD_BUILT_IN_PATTERN_TEST 0
#if (LCD_BUILT_IN_PATTERN_TEST == 1)
#include "ex_display.h"

MMP_BOOL bEnterLCDTestMode=MMP_FALSE;
void UartCmd_LCDTestMode(char* szParam)
{
	MMP_DSC_JPEG_INFO           jpeginfo;
	MMP_USHORT           StartX, StartY, DispWidth, DispHeight;
    ...
    MMP_USHORT clrs=2;
    MMP_ULONG img=0x3000000;
    UINT16 uiParam1 = 0;

    sscanfl( szParam, "%d", &uiParam1);
    if(!bEnterLCDTestMode) {
	    MMPS_3GPRECD_StopRecord();
	    MMPS_3GPRECD_PreviewStop(0);
	}
    ...
    winattribute.usWidth            = DispWidth;
    winattribute.usHeight           = DispHeight;           
    ...
    dispattribute.usDisplayWidth 	= DispWidth;
	dispattribute.usDisplayHeight 	= DispHeight;
    ...
    MMPS_Display_SetOutputPanel(MMP_DISPLAY_PRM_CTL, MMP_DISPLAY_SEL_MAIN_LCD);    
	MMPS_Display_SetWinPriority(MMP_DISPLAY_WIN_PIP, MMP_DISPLAY_WIN_MAIN, MMP_DISPLAY_WIN_OVERLAY, MMP_DISPLAY_WIN_MAX);


    DisplayDrawGray((MMP_ULONG)img, DispWidth, DispHeight, CLR_SCREEN);	
    DisplayDrawGray((MMP_ULONG)img, DispWidth, DispHeight, uiParam1); //patternIdx++);
    if(uiParam1 != 8) {
        clrs=1;
    }else {
        clrs=2;
    ...
    MMPS_Display_SetWindowAttrToDisp(MMP_DISPLAY_WIN_PIP, winattribute, dispattribute);
    MMPS_Display_SetWinActive(MMP_DISPLAY_WIN_PIP, 1);
}

void UartCmd_LCDTestSetPixel(char* szParam)
{
    UINT8 uiParam1, uiParam2, uiParam3;

    sscanfl( szParam, "%d %d %d", &uiParam1, &uiParam2, &uiParam3);   
    DisplaySetSubPixelVal(uiParam1, uiParam2, uiParam3);
}
#endif

#if (SUPPORT_LONGSYS_SMART_CARD)
void UartCmd_DumpSmartCardInfo( char* szParam )
{
	SMART_CARD_STATUS tCardInfo;
	
	if (AHC_Media_GetSmartCardInfo(&tCardInfo)){
		printc("Get Smart Info :\n");
		printc("dwDevSMARTTag      :%#x\n", tCardInfo.dwDevSMARTTag);
		printc("dwDevSMARTVersions :%#x\n", tCardInfo.dwDevSMARTVersions);
		printc("dwXorValue         :%#x\n", tCardInfo.dwXorValue);
		printc("dwSizeOfDevSMART   :%d\n", tCardInfo.dwSizeOfDevSMART);
		printc("dwOriginalBadBlock :%d\n", tCardInfo.dwOriginalBadBlock);
		printc("dwIncreaseBadBlock :%d\n", tCardInfo.dwIncreaseBadBlock);
		printc("lwWriteAllSectNum  :%ld\n", tCardInfo.lwWriteAllSectNum);
		printc("dwReplaceBlockLeft :%d\n", tCardInfo.dwReplaceBlockLeft);
		printc("dwDegreOfWear      :%d\n", tCardInfo.dwDegreOfWear);
		printc("dwSectorTotal      :%d\n", tCardInfo.dwSectorTotal);
		printc("dwRemainLifeTime   :%d\n", tCardInfo.dwRemainLifeTime);
		printc("dwRemainWrGBNum    :%d\n", tCardInfo.dwRemainWrGBNum);
		printc("dwLifeTimeTotal    :%d\n", tCardInfo.dwLifeTimeTotal);		
		printc("\n");
	}	
}
#endif

void UartCmd_System_ResetHModule(char * pszParam)
{
	UINT16 uiParam1 = 0;
    UINT16 uiParam2 = 0;
    sscanfl( pszParam, "%d %d", &uiParam1, &uiParam2);

	MMPD_System_ResetHModule(uiParam1, 	uiParam2);	
}

void UartCmd_USBTestPattern( char* szParam )
{
    extern MMP_ULONG   glTestModeFeature;
    extern void UsbEp0IntHandler(void);

    glTestModeFeature = 0x0400;
    UsbEp0IntHandler();
    printc("Send USB test pattern\r\n");
}

void UartCmd_DumpAHLMessage( char* szParam )
{
    AHC_DumpAHLMessage();
}
...
UARTCOMMAND sUartCommandSample3[] =
{
	...
	{ "touchpwr", 	        "touch",   			" power on/off", UartCmd_TouchPower_Operation},
    ...
    // recording related UART command
	{ "sea",                "",      "Set emergency recording action type",   UartCmdSetEmergAction},                

	// calibration related UART command ------------------------------
	// add by casio
	{ "getraw2",           	    "",      " Get raw data", 				    UartCmd_GetRaw},
	{ "getrawhdr",           	"",      " Get hdr raw data", 				UartCmd_GetRawHDR},
	{ "GetRearBuf",           	"",      " Get Rear Cam data", 				UartCmd_GetRearCamBuf},	
#if (EN_WP_AREA_CONFIG == 1)
	{ "dumpwp",                 "",      " Dump WP area data",              UartCmdDumpWpArea },
	{ "wrwp",                   "",      " Wr WP area",                     UartCmdWrWpArea },	
	{ "rdsc",                   "",      " Read Security status",           UartCmdReadSecuritySts },
	{ "rdwp",                   "",      " Read Wp status",                 UartCmdReadWpSts },	
	{ "setwp",                  "",      " Set write protect",              UartCmdWriteWpSts },	
	{ "clrwp",                  "",      " Clr write protect",              UartCmdClrWp },	
#endif
	{ "setdbbrk",               "",      " Set dbg break",                  UartCmdSetDebugBreak },
	{ "getdbbrk",               "",      " Get dbg break",                  UartCmdGetDebugBreak },
#if (DEBUG_UART_TO_FILE)
	{ "godead1",                "",      " Go dead by abort",               UartCmdDeadAbort},                
	{ "godead2",                "",      " Go dead by clock",               UartCmdDeadAbs},                
#endif	
#if (EN_AUTO_TEST_LOG == 1)	
	{ "clrlog",                 "",      " Clr dbg log",                    UartCmdClrAutoTestLog },
	{ "dumplog",                "",      " Dump dbg log",                   UartCmdDumpAutoTestLog },
    { "setdefcmdl",             "",      " Set UserDef CmdL",               UartCmdSetUserDefCmdNum },
    { "dpmenu",                 "",      " Dump Menu Setting",              UartCmdDumpMenuSetting },
    { "dpsnrtype",              "",      " Dump sensor type",               UartCmdDumpSnrType },
    { "getlastkey",             "",      " get last key cmd",               UartCmdGetLastKCmd },
#endif
...
//    { "nfc",           "",                     " NFC test",         MonitorNfcFunc },
    { "vifcnt",                 "",                         "Enable/Disable print VIF count",       UartCmd_PrintVifCnt },
#if (WIFI_PORT == 1)
    ...
    { "dspyiq",            "",                  " Set panel IQ",               UartCmd_DisplayIQ },     // 150925 AIT Add ’{’˚∆¡Ô@ IQ
#if  (defined(SUPPORT_ISP_CALIBRATION) && (SUPPORT_ISP_CALIBRATION == 1))			
    { "dumpISP",      "",                     " Dump ISP Calibration data",          UartCmd_DumpISPCaliFile },
#endif
#if (LCD_BUILT_IN_PATTERN_TEST == 1)
    { "lcdtest",      "",                     " Enter LCD pattern test mode",          UartCmd_LCDTestMode },
    { "lcdsetp",      "",                     " Enter LCD set pixel value",          UartCmd_LCDTestSetPixel },

#endif
	{ "sifread",          "<SF addr>,<SIZE>",   " SIF read Test",             UartCmd_DumpSIFInfo },
#if (SUPPORT_LONGSYS_SMART_CARD)
	{ "SmartInfo",    "",   	"Dump Longsys Smart Card Info",             UartCmd_DumpSmartCardInfo },
#endif
#if (DEBUG_UART_TO_FILE)
	{ "dbgEn",      "<SF enable>,<SD enable>,<Key info enable.",   " Debug dynamic enable Test",             UartCmd_DebugEnable },
#endif
    { "resethm", "", "Reset Hardware Modulw", UartCmd_System_ResetHModule},
    { "cap",	"",								"Video Capture", UartCmd_VidCapture},	
    { "msd",	"",								"Mount SD", UartCmd_MountSD},
	{ "usbt", 	"",								"Send USB test pattern",      	UartCmd_USBTestPattern },
	{ "ahcmsg", 	"",							"Dump AHC Messages in queue",   UartCmd_DumpAHLMessage },
	{0,0,0,0}
};
