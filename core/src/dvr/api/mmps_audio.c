...
MMP_ERR MMPS_AUDIO_SetPlayVolumeDb(MMP_BYTE ulInputDb)
{
	...
	#if (AUDIO_SET_DB == 0x1)	 
    // CarDV++
    if (m_audioOutPath == MMPS_AUDIO_OUT_AFE_HP) {
        if (ulInputDb > MMPS_Audio_GetConfig()->ubAudioOutVolHPTable[30][0]) 
            ulInputDb = MMPS_Audio_GetConfig()->ubAudioOutVolHPTable[30][0];
    }
    else if (m_audioOutPath == MMPS_AUDIO_OUT_AFE_LINE) {
        if (ulInputDb > MMPS_Audio_GetConfig()->ubAudioOutVolLineTable[30][0])
            ulInputDb = MMPS_Audio_GetConfig()->ubAudioOutVolLineTable[30][0];
    }
    else if ((m_audioOutPath & MMPS_AUDIO_OUT_AFE_HP) && (m_audioOutPath & MMPS_AUDIO_OUT_AFE_LINE)) {
        if (ulInputDb > MMPS_Audio_GetConfig()->ubAudioOutVolLineTable[30][0])
            ulInputDb = MMPS_Audio_GetConfig()->ubAudioOutVolLineTable[30][0];
    }
    // CarDV--
...
MMP_ERR MMPS_AUDIO_SetPlayVolume(MMP_USHORT usLevel, MMP_BOOL bSWGainOnly)
{
    ...
    if (bSWGainOnly) {
        if (m_audioOutPath == MMPS_AUDIO_OUT_AFE_HP)
            ubSWGain = MMPS_Audio_GetConfig()->ubAudioOutVolHPTable[usLevel][0];
        else if (m_audioOutPath == MMPS_AUDIO_OUT_AFE_LINE)
        ...
        if (m_audioOutPath == MMPS_AUDIO_OUT_AFE_HP) {
            ubSWGain = MMPS_Audio_GetConfig()->ubAudioOutVolHPTable[usLevel][0];
            ubDGain = MMPS_Audio_GetConfig()->ubAudioOutVolHPTable[usLevel][1];
            ubAGain = MMPS_Audio_GetConfig()->ubAudioOutVolHPTable[usLevel][2];
        }
        else if (m_audioOutPath == MMPS_AUDIO_OUT_AFE_LINE) {
            ubSWGain = MMPS_Audio_GetConfig()->ubAudioOutVolLineTable[usLevel][0];
