...
*/
static  MMP_SNR_MODE    m_CurSensorMode[VIF_SENSOR_MAX_NUM];

/**@brief The current preview mode.

Use @ref MMPS_Sensor_Initialize to set it.
*/
static MMP_UBYTE        m_ubCurPreviewMode[VIF_SENSOR_MAX_NUM] = {0, 0};//same as gsCurPreviewMode 
...
MMP_ERR MMPS_Sensor_Initialize(MMP_UBYTE ubSnrSel, MMP_UBYTE ubPreviewMode, MMP_SNR_MODE sSnrMode)
{
    ...
    if (ubPreviewMode >= MMPS_Sensor_GetPreviewResNum(ubSnrSel)) {
        RTNA_DBG_Str(0, "Warning!! Sensor mode is out of all sensor modes, use default mode!!\r\n");
        ubPreviewMode = MMPS_Sensor_GetDefPreviewMode(ubSnrSel);
    }
    ...
    error = MMPD_Sensor_Initialize(ubSnrSel, ubPreviewMode);
...
{
    m_ubCurPreviewMode[ubSnrSel] = ubPreviewMode;//gsCurPreviewMode
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPS_Sensor_SetNightMode(MMP_ULONG ulMode, MMP_ULONG ulMinFPS)
{
    return MMPD_Sensor_SetNightMode(ulMode, ulMinFPS);       
}
...
{
    if (MMPS_Sensor_IsInitialized(gsAhcPrmSensor))
...
#if 0
void _____TV_Decoder_Functions_____(){}
#endif

//------------------------------------------------------------------------------
//  Function    : MMPS_Sensor_GetTVDecSrcType
//  Description :
//------------------------------------------------------------------------------
void MMPS_Sensor_GetTVDecSrcType(MMP_SNR_TVDEC_SRC_TYPE *sSnrTVSrc)
{
    MMPF_Sensor_GetTVDecSrcType(sSnrTVSrc);
}

//------------------------------------------------------------------------------
//  Function    : MMPS_Sensor_CheckTVDecSrcType
//  Description :
//------------------------------------------------------------------------------        
void MMPS_Sensor_CheckTVDecSrcType(MMP_UBYTE ubSnrSel, MMP_SNR_TVDEC_SRC_TYPE *sSnrTVSrc)
{
    MMPF_Sensor_CheckTVDecSrcType(ubSnrSel, sSnrTVSrc);
}

//------------------------------------------------------------------------------
//  Function    : MMPS_Sensor_EnableTVDecSrcTypeDetection
//  Description :
//------------------------------------------------------------------------------   
void MMPS_Sensor_EnableTVDecSrcTypeDetection(MMP_BOOL bEnable)
{
    MMPF_Sensor_EnableTVDecSrcTypeDetection(bEnable);
}
