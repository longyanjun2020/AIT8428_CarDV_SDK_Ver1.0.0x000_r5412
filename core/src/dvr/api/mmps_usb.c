...
//==============================================================================
static MMP_ERR MMPS_USB_SetMemoryMap(MMPS_USB_OP_MODE ubMode)
{
    MMP_ULONG buf_start, cur_buf;

    /* Allocate buffer from end address of firmware */
    MMPD_System_GetFWEndAddr(&buf_start);
    cur_buf = buf_start;
    ...
    case MMPS_USB_DETECT_MODE:
        MMPD_USB_SetMSDCBuf(cur_buf, MSDC_RDWR_BUF_SIZE);
        cur_buf += MSDC_RDWR_BUF_SIZE;
        MMPD_USB_SetCtlBuf(cur_buf, MSDC_CTL_BUF_SIZE);
        cur_buf += MSDC_CTL_BUF_SIZE;
        #if (MSDC_WR_FLOW_TEST_EN == 1)
        MMPD_USB_SetMSDCWriteBuf(buf_start, MMPS_USB_W_DATA_BUFF_SIZE);
        cur_buf += MMPS_USB_W_DATA_BUFF_SIZE * MMPS_USB_W_DATA_BUFF_DEPTH;
        #endif
        break;

    #if (SUPPORT_PCSYNC_FUNC)
    case MMPS_USB_PCSYNC_MODE:
        MMPD_USB_SetCtlBuf(cur_buf, PCSYNC_CTL_BUF_SIZE);
        cur_buf += PCSYNC_CTL_BUF_SIZE;
        MMPD_USB_SetPCSYNC_InBuf(cur_buf, PCSYNC_IN_BUF_SIZE);
        cur_buf += PCSYNC_IN_BUF_SIZE;
        MMPD_USB_SetPCSYNC_OutBuf(cur_buf, PCSYNC_OUT_BUF_SIZE);
        cur_buf += PCSYNC_OUT_BUF_SIZE;
        MMPD_USB_SetPCSYNC_HandShakeBuf(cur_buf, PCSYNC_HANDSHAKE_BUF_SIZE);
        cur_buf += PCSYNC_HANDSHAKE_BUF_SIZE;
        MMPD_FS_SetFileNameDmaAddr(cur_buf, PCSYNC_NAME_BUF_SIZE,
                        cur_buf + PCSYNC_NAME_BUF_SIZE, PCSYNC_PARAM_BUF_SIZE);
        cur_buf += PCSYNC_NAME_BUF_SIZE;
        cur_buf += PCSYNC_PARAM_BUF_SIZE;
        break;
    #endif
    ...
    case MMPS_USB_MTP_MODE:
        MMPD_FS_SetFileNameDmaAddr(cur_buf, MTP_NAME_BUF_SIZE,
                            cur_buf + MTP_NAME_BUF_SIZE, MTP_PARAM_BUF_SIZE);
        cur_buf += MTP_NAME_BUF_SIZE;   //UTF8: name_buf_size;
        cur_buf += MTP_PARAM_BUF_SIZE;  //8*1024: parameter_buf_size;
        MMPD_USB_SetCtlBuf(cur_buf, MTP_CTL_BUF_SIZE);
        cur_buf += MTP_CTL_BUF_SIZE;
        MMPD_USB_SetPCSYNC_InBuf(cur_buf, MTP_PCSYNC_IN_BUF_SIZE);
        cur_buf += MTP_PCSYNC_IN_BUF_SIZE;
        MMPD_USB_SetPCSYNC_OutBuf(cur_buf, MTP_PCSYNC_OUT_BUF_SIZE);
        cur_buf += MTP_PCSYNC_OUT_BUF_SIZE;
        MMPD_USB_SetPCSYNC_HandShakeBuf(cur_buf, PCSYNC_HANDSHAKE_BUF_SIZE);
        cur_buf += PCSYNC_HANDSHAKE_BUF_SIZE;
        cur_buf = ALIGN512(cur_buf);
        MMPD_USB_SetMTPEPBuf(cur_buf, cur_buf + MTP_EP_BUF_SIZE, MTP_EP_BUF_SIZE,
                            cur_buf + (MTP_EP_BUF_SIZE << 1), MTP_DRM_BUF_SIZE);
        cur_buf += (MTP_EP_BUF_SIZE << 1);
        cur_buf += MTP_DRM_BUF_SIZE;
        break;
    #endif

    default:
        PRINTF(0, "ERROR: unsupport USB mode !!\r\n");
...
void MMPS_USB_DetectAdaptorOrUSB(void)
{
    ...
    while ((gsUsbCurStatus == tempStatus)) {
        MMPF_OS_Sleep_MS(10);       //release cpu
        TimeOutCount++;
...
{
    return MMPD_System_ResetHModule(MMPD_SYS_MDL_USB, MMP_TRUE);
}
...
//------------------------------------------------------------------------------
//  Function    : MMPS_USB_SetRecdFrmRxTimeoutCFG
//  Description :
//------------------------------------------------------------------------------
/**
@brief The function register following configs for error handle.
@param[in] pulRecdFrmRxLostTH The lost reocrd frame count of threshold to be trigger callback.
@param[in] pDevRecdFrmRxLostTHCallback The callback will be executed when record frame lost reach pulFrmRecdRxLostTH.
@return It reports the status of the operation.
*/
MMP_ERR MMPS_USB_SetRecdFrmRxTimeoutCFG(MMP_ULONG pulRecdFrmRxLostTH, void *pDevRecdFrmRxLostTHCallback)
{
	return MMPD_USB_SetRecdFrmRxTimeoutCFG(pulRecdFrmRxLostTH,pDevRecdFrmRxLostTHCallback);
}
