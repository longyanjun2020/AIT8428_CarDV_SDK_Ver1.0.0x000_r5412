...
MMP_ERR MMPS_Display_SetOutputPanel(MMP_DISPLAY_CONTROLLER controller, MMP_DISPLAY_OUTPUT_SEL dispOutSel)
{
    ...
    case MMP_DISPLAY_SEL_CCIR:
        ...
        if ((m_displayCurOutput[controller] == MMP_DISPLAY_SEL_MAIN_LCD && m_displayConfig.mainlcd.lcdtype == MMP_DISPLAY_RGB_LCD) ||
            ...
            MMPD_Display_DisableRGBOutput(controller, m_RGBLCDInterface);
        }
		...
		if ((m_displayConfig.mainlcd.lcdtype == MMP_DISPLAY_P_LCD) ||
			...
			if ((m_displayConfig.mainlcd.lcdtype == MMP_DISPLAY_P_LCD) || 
				...
				MMPD_Display_SetPLCDOutput(controller, m_displayConfig.mainlcd.cssel, &lcdAttr);
                 ...
                 MMPD_Display_SetRGBLCDOutput(controller, &lcdAttr, m_RGBLCDInterface);            }
		}
	...
	case MMP_DISPLAY_SEL_PAL_TV:
		...
		if (dispOutSel == MMP_DISPLAY_SEL_NTSC_TV)
			MMPD_Display_SetTVOutput(controller, &m_displayConfig.ntsctv, m_RGBLCDInterface);
		else
			MMPD_Display_SetTVOutput(controller, &m_displayConfig.paltv, m_RGBLCDInterface);
        ...
        if((m_displayCurOutput[MMP_DISPLAY_PRM_CTL] == MMP_DISPLAY_SEL_MAIN_LCD && m_displayConfig.mainlcd.lcdtype == MMP_DISPLAY_RGB_LCD) ||
           (m_displayCurOutput[MMP_DISPLAY_SCD_CTL] == MMP_DISPLAY_SEL_MAIN_LCD && m_displayConfig.mainlcd.lcdtype == MMP_DISPLAY_RGB_LCD)) {
            if(m_RGBLCDInterface == m_HDMIInterface) {
                MMPD_Display_DisableRGBOutput(controller, m_HDMIInterface);
            }
        }
		MMPD_Display_SetHDMIOutput(controller, &m_displayConfig.hdmi, m_HDMIInterface);
        break;
	}
...
{
	return MMPD_RGBLCD_GetDotClk(ulRGBDotClk);
}
...
//------------------------------------------------------------------------------
//  Function    : MMPS_Display_SetFreezeWinEn
//  Description :
//------------------------------------------------------------------------------
void MMPS_Display_SetFreezeWinEn(MMP_BOOL bEn)
{
	MMPD_Display_SetFreezeWinEn(bEn);
}

//------------------------------------------------------------------------------
//  Function    : MMPS_Display_GetFreezeWinEn
//  Description :
//------------------------------------------------------------------------------
MMP_BOOL MMPS_Display_GetFreezeWinEn(void)
{
	return MMPD_Display_GetFreezeWinEn();
}

//------------------------------------------------------------------------------
//  Function    : MMPS_Display_FreezeWinUpdate
//  Description : Copy window buffer to a temporal buffer, change address pointer on it.
//				  Simulate freezed window.
//------------------------------------------------------------------------------
void MMPS_Display_FreezeWinUpdate(MMP_BOOL PIPEn, MMP_BOOL OVLYEn, MMP_BOOL OSDEn)
{
	MMPD_Display_FreezeWinUpdate(PIPEn, OVLYEn, OSDEn);
}
...
{
    return MMPD_Display_GetWinAttributes(winID, pWinAttr);
}
...
MMP_ERR MMPS_Display_ReadWinBuffer(MMP_DISPLAY_WIN_ID winID, MMP_USHORT *usMemPtr)
{
    ...
    if (winAttr.colordepth == MMP_DISPLAY_WIN_COLORDEPTH_8) {
    	usSize = winAttr.usWidth * winAttr.usHeight * 1;
             ...
             winAttr.colordepth == MMP_DISPLAY_WIN_COLORDEPTH_ARGB3454 ||
             winAttr.colordepth == MMP_DISPLAY_WIN_COLORDEPTH_ARGB4444) {
...
//------------------------------------------------------------------------------
//  Function    : MMPS_Display_LockWindow
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPS_Display_LockWindow(MMP_DISPLAY_WIN_ID winID)
{
    return MMPD_Display_LockWindow(winID);
}

//------------------------------------------------------------------------------
//  Function    : MMPS_Display_UnlockWindow
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPS_Display_UnlockWindow(MMP_DISPLAY_WIN_ID winID)
{
    return MMPD_Display_UnlockWindow(winID);
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//  Function    : MMPS_Display_AdjustWinColorMatrix
//  Description :
//------------------------------------------------------------------------------
...
MMP_ERR MMPS_Display_SetYuvAttribute(MMP_BYTE ubU2U,
                    				 ...
                    				 MMP_BYTE ubYoffset)
{
	return MMPD_Display_SetYuvAttribute(ubU2U, ubV2U, ubU2V, ubV2V, ubYGain, ubYoffset); 
}
