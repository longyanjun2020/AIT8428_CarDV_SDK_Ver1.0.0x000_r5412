                                ...
                                MMP_RAW_COLORFMT 	colorFormat)
{
    ...
    if (bEnable) 
    {
        ...
        MMPF_RAWPROC_SetFetchLineDelay(0x40);	//0x20; // Change to 0x20 for raw fetch zoom. Large than 0x48,"Raw fetch busy !" will prompt.
        ...
        /* Set Sensor ID */
		MMPH_HIF_SetParameterB(GRP_IDX_FLOWCTL, 5, ubSnrSel);		
    ...
    {    
        ...
        if (pathSelect & MMP_RAW_IOPATH_VIF2ISP) {
            ...
            MMPF_VIF_SetRawOutPath(ubVifId, MMP_FALSE, MMP_FALSE);
        }
...
//------------------------------------------------------------------------------
//  Function    : MMPD_RAWPROC_SetStoreBufEnd
//  Description :
//------------------------------------------------------------------------------
/** @brief Set raw store buffer end address
@param[in] rawbuf  bayer buffer address
@return It reports the status of the configuration.
*/
MMP_ERR MMPD_RAWPROC_SetStoreBufEnd(MMP_UBYTE ubRawId, MMP_RAW_STORE_BUF *pRawBuf)
{
    MMP_ULONG 	i;
    MMP_ERR 	ret = MMP_ERR_NONE;
    ...
    if (pRawBuf->ulRawBufCnt > 0) {
    	MMPF_RAWPROC_InitStoreBufferEnd(ubRawId, MMP_RAW_STORE_PLANE0, pRawBuf->ulRawBufAddr[0]);
    	
    	/* For Store YUV420 Format */
    	if (pRawBuf->ulRawBufUAddr[0] != 0 && pRawBuf->ulRawBufVAddr[0] != 0) {
    	    MMPF_RAWPROC_InitStoreBufferEnd(ubRawId, MMP_RAW_STORE_PLANE1, pRawBuf->ulRawBufUAddr[0]);
    	    MMPF_RAWPROC_InitStoreBufferEnd(ubRawId, MMP_RAW_STORE_PLANE2, pRawBuf->ulRawBufVAddr[0]);
        }
    }
    ...
    ret = MMPH_HIF_SendCmd(GRP_IDX_FLOWCTL, HIF_FCTL_CMD_SET_RAW_PREVIEW | SET_RAWSTORE_ENDADDR);
    ...
    if (pRawBuf->ulRawBufUAddr[0] != 0 && pRawBuf->ulRawBufVAddr[0] != 0) 
    {
        ...
        ret = MMPH_HIF_SendCmd(GRP_IDX_FLOWCTL, HIF_FCTL_CMD_SET_RAW_PREVIEW | SET_RAWSTORE_ENDUADDR);

        for (i = 0; i < pRawBuf->ulRawBufCnt; i++) {
            MMPH_HIF_SetParameterL(GRP_IDX_FLOWCTL, (i+2)*4, pRawBuf->ulRawBufVAddr[i]);
        }
        
        ret = MMPH_HIF_SendCmd(GRP_IDX_FLOWCTL, HIF_FCTL_CMD_SET_RAW_PREVIEW | SET_RAWSTORE_ENDVADDR);
    }
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_RAWPROC_SetDeInterlaceBuf(MMP_UBYTE ubRawId, MMP_UBYTE ubBufIdx, MMP_ULONG ulYAddr, MMP_ULONG ulUAddr, MMP_ULONG ulVAddr, MMP_ULONG ulBufCnt)
{
    return MMPF_RAWPROC_SetDeInterlaceBuf(ubRawId, ubBufIdx, ulYAddr, ulUAddr, ulVAddr, ulBufCnt);
}

//------------------------------------------------------------------------------
//  Function    : MMPD_RAWPROC_ResetDeInterlaceBufIdx
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_RAWPROC_ResetDeInterlaceBufIdx(MMP_UBYTE ubRawId)
{
    return MMPF_RAWPROC_ResetDeInterlaceBufIdx(ubRawId);
}
