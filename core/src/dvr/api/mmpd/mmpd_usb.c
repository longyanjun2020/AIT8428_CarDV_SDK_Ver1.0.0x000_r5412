...
{
#if 1
    MMPF_USBPHY_SetVrefTune(Vref);
#else
    MMPH_HIF_WaitSem(GRP_IDX_USB, 0);
    MMPH_HIF_SetParameterL(GRP_IDX_USB, 0, Vref);
    MMPH_HIF_SendCmd(GRP_IDX_USB, HIF_USB_CMD_ADJUST_PHY|HIF_USB_PHY_VREF);
...
{
#if 1
    MMPF_USBPHY_SetBiasCurrentTune(BiasCurrent);
#else
    MMPH_HIF_WaitSem(GRP_IDX_USB, 0);
    MMPH_HIF_SetParameterL(GRP_IDX_USB, 0, BiasCurrent);
    MMPH_HIF_SendCmd(GRP_IDX_USB, HIF_USB_CMD_ADJUST_PHY|HIF_USB_PHY_BIASCURRENT);
    MMPH_HIF_ReleaseSem(GRP_IDX_USB);
#endif    
    return MMP_ERR_NONE;
}
...
{
#if 1
    MMPF_USBPHY_SetSignalTune(TxCur, TxCurPlus, SqLevel);
#else
    MMPH_HIF_WaitSem(GRP_IDX_USB, 0);
    MMPH_HIF_SetParameterL(GRP_IDX_USB, 0, TxCur);
    MMPH_HIF_SetParameterL(GRP_IDX_USB, 4, TxCurPlus);
    MMPH_HIF_SetParameterL(GRP_IDX_USB, 8, SqLevel);
    MMPH_HIF_SendCmd(GRP_IDX_USB, HIF_USB_CMD_ADJUST_PHY|HIF_USB_PHY_SIGNAL);
    MMPH_HIF_ReleaseSem(GRP_IDX_USB);
#endif    
    return MMP_ERR_NONE;
}
...
//------------------------------------------------------------------------------
//  Function    : MMPD_USB_SetRecdFrmRxTimeoutCFG
//  Description :
//------------------------------------------------------------------------------
/**
@brief The function register following configs for error handle.
...
@return It reports the status of the operation.
*/
MMP_ERR MMPD_USB_SetRecdFrmRxTimeoutCFG(MMP_ULONG pulRecdFrmRxLostTH, void *pDevRecdFrmRxLostTHCallback)
{
	return MMPF_USBH_SetRecdFrmRxTimeoutCFG(pulRecdFrmRxLostTH,pDevRecdFrmRxLostTHCallback);
}
...
//------------------------------------------------------------------------------
//  Function    : MMPD_USB_CaptureJpeg
//  Description :
//------------------------------------------------------------------------------
/**
@brief No blocking code. Inform USBH to capture(copy) a jpeg into jpeg buffer.
@param[in] ulJpegAddr: Jpeg Address.
@param[in] ulBufSize : Jpeg Buffer Size.
@param[in] ubDelayCnt: Delay frame count.

@return It reports the status of the operation.
*/
MMP_ERR MMPD_USB_CaptureJpeg(MMP_ULONG ulJpegAddr, MMP_ULONG ulBufSize, MMP_UBYTE ubDelayCnt)
{
    ...
    return MMPF_USBH_StartJpegCpature(ulJpegAddr, ulBufSize, ubDelayCnt);
#else
    return MMP_ERR_NONE;
#endif
}
...
//------------------------------------------------------------------------------
//  Function    : MMPD_USB_QueryJpeg
//  Description :
//------------------------------------------------------------------------------
/**
@brief Blocking code. Query jpeg capture done or not.
@return It reports the status of the operation.
*/
MMP_ERR MMPD_USB_QueryJpeg(MMP_ULONG *ulJpegSize)
{
#if (SUPPORT_DEC_MJPEG_TO_PREVIEW)
    if (MMP_IsSupportDecMjpegToPreview()) {
        return MMPF_USBH_QueryJpeg(ulJpegSize);
    }
    else {
        return MMP_ERR_NONE;
    }
