...
#define MIN_EXTERNAL_COMPRESS_BUF_TH   	(256*1024) 	 // 256KB is enough for MIN 320*240 preview (256*1024=0x0004_0000)
#define MAX_EXTERNAL_COMPRESS_BUF_TH   	(9*256*1024) //(9*256*1024=0x0024_0000)
...
static MMP_PIPE_LINK	m_MJPEGStreamSrcPipe    = {MMP_SCAL_PIPE_0, MMP_ICO_PIPE_0, MMP_IBC_PIPE_0};

extern MMP_USHORT		m_usDecodeMode;
								...
								MMP_ULONG			*ulSize)
{
	...
	case MMPD_DSC_ENC_VR_CAPTURE_COMP_BUFFER:
        ...
        *ulSize = (usWidth * usHeight);
		*ulSize +=  512 *1024;  // welly for VR capture buffer Over
         ...
         */
        if (*ulSize > MAX_EXTERNAL_COMPRESS_BUF_TH && m_DramSettings.DRAMID != DRAM_DDR3) {
...
//------------------------------------------------------------------------------
//  Function    : MMPD_DSC_TakeJpegAfterLdcReady
//  Description : For video still capture (LDC Multi-Slice)
//------------------------------------------------------------------------------
MMP_ERR MMPD_DSC_TakeJpegAfterLdcReady(void)
{
    ...
    mmpstatus = MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_CMD_TAKE_LDC_JPEG);
...
//------------------------------------------------------------------------------
//  Function    : MMPD_DSC_TakeJpegNotTrigSrc
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_DSC_TakeJpegNotTrigSrc(void)
{
    MMP_ERR mmpstatus = MMP_ERR_NONE;
    ...
    mmpstatus = MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_CMD_TAKE_JPEG_NO_TRIG_SRC);
...
//------------------------------------------------------------------------------
//  Function    : MMPD_DSC_SetLargeCaptureStoreAddr
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_DSC_SetLargeCaptureStoreAddr(MMP_ULONG ulAddr)
{
	MMPF_DSC_SetLargeCaptureStoreAddr(ulAddr);
    
    return MMP_ERR_NONE;
}

//------------------------------------------------------------------------------
//  Function    : MMPD_DSC_SetLargeCaptureInfo
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_DSC_SetLargeCaptureInfo(MMP_UBYTE ubDivideNum)
{
	MMPF_DSC_SetLargeCaptureInfo(ubDivideNum);
    return MMP_ERR_NONE;
}
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_DSC_InitGra2MJPEGAttr(	MMP_PIPE_LINK			fctllink,
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_DSC_GetRearCamBufForDualStreaming(MMP_ULONG *pulCompAddr, 	MMP_ULONG *pulCompSize,
...
{
    MMP_ERR err = MMP_ERR_NONE;
#if (USB_EN)&&(SUPPORT_USB_HOST_FUNC)&&(SUPPORT_DEC_MJPEG_TO_ENC_H264)
                                ...
                                MMP_SCAL_PIPEID 		pipeID)
{
	...
	ibcAttr.ulBaseUAddr 	= pBufAttr->ulBaseUAddr;
	ibcAttr.ulBaseVAddr 	= pBufAttr->ulBaseVAddr;
    ibcAttr.colorformat     = MMPF_IBC_MapDispColorToIBCFmt(dispColor);
    ibcAttr.ulLineOffset 	= 0;
	ibcAttr.function 		= MMP_IBC_FX_TOFB;
	ibcAttr.InputSource		= fctllink.icopipeID;
	ibcAttr.bMirrorEnable	= MMP_FALSE;
	...
	// Config Scaler module
    scal_outcolor = MMPF_Scaler_MapDispColorToScalerColor(dispColor);
	...
	if (pJpegInfo->ulJpegBufAddr != 0) {
        ...
        if ((panel->usPanelW * panel->usPanelH) == (480 * 854)) {
            if ((pBufAttr->usWidth * pBufAttr->usHeight) == (320 * 240)) {
                MMPD_DSC_SetDecodeSpeed(MMP_FALSE, 32); // CHECK
            }
            else {
        ...
        // Just workaround it
        if (m_usDecodeMode == DSC_HDMI_DECODE_MODE) {
            MMPD_DSC_SetDecodeSpeed(MMP_FALSE, 7);
        }
        //TL: FrameRdy was not cleared after Checking FrameRdy that casue decode timing overlap (Only HDMI case)
        //So we clear it before decode start.
        MMPD_IBC_ClearFrameReady(fctllink.ibcpipeID);
...
//------------------------------------------------------------------------------
//  Function    : MMPD_DSC_KillExifForMultiShot
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_DSC_KillExifForMultiShot(MMP_USHORT usExifNodeId)
{
    MMP_ERR mmpstatus = MMP_ERR_NONE;
	...
	mmpstatus = MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_CMD_CARD_MODE_EXIF | EXIF_KILL_FOR_MULTISHOT);
	MMPH_HIF_ReleaseSem(GRP_IDX_DSC);
...
MMP_ERR MMPD_DSC_SetThumbFileName(MMP_BYTE ubFilename[], MMP_USHORT usLength)
{
    ...
    MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_CMD_SET_JPEG_MEDIA | JPEG_THUMB_FILE_NAME);
    MMPH_HIF_ReleaseSem(GRP_IDX_DSC);
...
//------------------------------------------------------------------------------
//  Function    : MMPD_DSC_ThumbDram2Card
//  Description :
//------------------------------------------------------------------------------
/** @brief The function saved Jpeg data from DRAM (JPG_EXT_MEM_ST) to card
...
@return It reports the status of the operation.
*/
MMP_ERR MMPD_DSC_ThumbDram2Card(MMP_ULONG ulStreamTyp, MMP_BOOL bFirstWrite, MMP_BOOL bLastWrite)
{	
	MMP_ERR	mmpstatus;
   
    MMPH_HIF_WaitSem(GRP_IDX_DSC, 0);
    MMPH_HIF_SetParameterL(GRP_IDX_DSC, 0,	ulStreamTyp);
    MMPH_HIF_SetParameterL(GRP_IDX_DSC, 4, 	bFirstWrite);  
	MMPH_HIF_SetParameterL(GRP_IDX_DSC, 8, 	bLastWrite); 
	
    mmpstatus = MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_CMD_THUMB_JPEG_SAVE_CARD);
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_MJPEG_SetCompBuf(MMP_USHORT usEncID, MMP_USHORT usModeID, MMP_ULONG ulAddr, MMP_ULONG ulSize, MMP_M_STREAMCB_CODEC codec)
{
    MMP_ERR sRet = MMP_ERR_NONE;
	MMP_USHORT ustype = MMP_DSC_JPEGCMD_SETCOMPBUF;

    // Set EncID and ModeID before set compress buffer.
    sRet = MMPD_MJPEG_SetModeID(usEncID, usModeID);
    if (sRet != MMP_ERR_NONE) { MMP_PRINT_RET_ERROR(0, 0, "", 0); return sRet;}
    ...
    MMPH_HIF_SetParameterW(GRP_IDX_DSC, 12, codec);
    
    sRet = MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_MJPEG_OPERATION | STREAM_JPEG_INFO);
    MMPH_HIF_ReleaseSem(GRP_IDX_DSC);
...
//------------------------------------------------------------------------------
//  Function    : MMPD_Streaming_SetCompBufAttr
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_Streaming_SetCompBufAttr(MMP_USHORT usEncID, 
									  MMP_ULONG (*FrameSizeCalFunc)(MMP_ULONG ulFrameSize),
									  MMP_USHORT usAlignment)
{
	MMP_USHORT ustype = MMP_DSC_JPEGCMD_SETCOMPBUF_ATTR;

    MMPH_HIF_WaitSem(GRP_IDX_DSC, 0);
    MMPH_HIF_SetParameterW(GRP_IDX_DSC, 0, usEncID);
    MMPH_HIF_SetParameterW(GRP_IDX_DSC, 2, ustype);
	MMPH_HIF_SetParameterL(GRP_IDX_DSC, 4, (MMP_ULONG)FrameSizeCalFunc);
	MMPH_HIF_SetParameterW(GRP_IDX_DSC, 8, usAlignment);
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_MJPEG_StreamEnable(MMP_USHORT usEncID, MMP_BOOL bEnable, MMP_BOOL bWaitForSync)
{
...
//------------------------------------------------------------------------------
//  Function    : MMPD_MJPEG_SetLineBuf
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_MJPEG_SetLineBuf(MMP_USHORT usEncID, MMP_ULONG ulAddr, MMP_ULONG ulSize)
{
	MMP_USHORT ustype = MMP_DSC_JPEGCMD_SETLINEBUF;
...
//------------------------------------------------------------------------------
//  Function    : MMPD_MJPEG_SetModeID
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_MJPEG_SetModeID(MMP_USHORT usEncID, MMP_USHORT usModeID)
{
    MMP_USHORT ustype = MMP_DSC_JPEGCMD_SET_MODEID;
...
//------------------------------------------------------------------------------
//  Function    : MMPD_MJPEG_SetDualStreamingEnable
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_MJPEG_SetDualStreamingEnable(MMP_BOOL bFrontCamEn, MMP_BOOL bRearCamEn)
{
#if (HANDLE_JPEG_EVENT_BY_QUEUE)
    if (MMPF_JPEG_GetCtrlByQueueEnable()) {
        return MMPF_MJPEG_SetDualStreamingEnable(bFrontCamEn, bRearCamEn);
    }
    else {
        return MMP_ERR_NONE;
    }
...
#if 0
void _____JPEG_Ctl_Functions_____(){}
#endif

//------------------------------------------------------------------------------
//  Function    : MMPD_JPEG_Ctl_PausePreview
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_JPEG_Ctl_PausePreview(void)
{
#if (SUPPORT_DEC_MJPEG_TO_PREVIEW) && (HANDLE_JPEG_EVENT_BY_QUEUE)
    if (MMPF_JPEG_GetCtrlByQueueEnable()) {
	    MMPH_HIF_WaitSem(GRP_IDX_DSC, 0);
	    MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_CMD_JPEG_CTRL | PAUSE_PREVIEW);
	    MMPH_HIF_ReleaseSem(GRP_IDX_DSC);
	}
#endif
	return MMP_ERR_NONE;
...
//------------------------------------------------------------------------------
//  Function    : MMPD_JPEG_Ctl_ResumePreview
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_JPEG_Ctl_ResumePreview(void)
{
#if (SUPPORT_DEC_MJPEG_TO_PREVIEW) && (HANDLE_JPEG_EVENT_BY_QUEUE)
    if (MMPF_JPEG_GetCtrlByQueueEnable()) {
	    MMPH_HIF_WaitSem(GRP_IDX_DSC, 0);
	    MMPH_HIF_SendCmd(GRP_IDX_DSC, HIF_DSC_CMD_JPEG_CTRL | RESUME_PREVIEW);
	    MMPH_HIF_ReleaseSem(GRP_IDX_DSC);
	}
#endif
	return MMP_ERR_NONE;
