...
static MMP_PIPE_LINK 							m_fctlLinkVidPlayback = {MMP_SCAL_PIPE_0, MMP_ICO_PIPE_0, MMP_IBC_PIPE_0};
...
extern MMP_UBYTE MMPS_3GPRECD_GetVRThumbnailSts(void);
...
//    printc("MMPS_System_GetMemHeapEnd() = 0x%X\r\n", MMPS_System_GetMemHeapEnd());
    #if defined(ALL_FW)
    if (ulCurAddr > MMPS_System_GetMemHeapEnd()) {
        printc("\t= [HeapMemErr] Video play = 0x%x ( > 0x%x )\r\n", ulCurAddr, MMPS_System_GetMemHeapEnd());
                                                ...
                                                MMPS_VIDPLAY_CROPPING_DATA cropping)
{
    ...
    if (usMode == VIDEOPLAY_MODE_CCIR) {
        MMPS_VIDPLAY_SetColorFmt(VID_FMT_YUV422);
    }
    else {
		...
		((usMode == VIDEOPLAY_MODE_NTSC_TV) || (usMode == VIDEOPLAY_MODE_PAL_TV))) || (usMode == VIDEOPLAY_MODE_CCIR)))
    {
		...
		if (usMode == VIDEOPLAY_MODE_CCIR) {
			// always use scaler to do color transform (NV12 to YUV422)		
	   	    ...
	   	    if((*usW < usLCDW) && (*usH < usLCDH)){
				//UpScaling bug -> Use 1:1 GraScal and then use DispScal to fit OutDevice
				if (MMPS_VIDPLAY_GetColorFmt() == VID_FMT_I420) {
		   		    usLCDW = *usW - usRight - usLeft;
			   	    usLCDH = *usH - usBottom - usTop;			
				}
				else if (MMPS_VIDPLAY_GetColorFmt() == VID_FMT_NV12) {
		   		    usLCDW = *usW - usRight - usLeft;
			   	    usLCDH = *usH - usBottom - usTop;
				}
	   	    }
			else if ((*usW > usLCDW && *usH > usLCDH)){			
			}
			else if ((*usW == usLCDW && *usH == usLCDH) || (*usW == 1920 && *usH == 1088)){
                if (MMPS_VIDPLAY_GetColorFmt() == VID_FMT_NV12) {            
	                *usScalerWidth  = 0;
	                *usScalerHeight = 0;            
	                return MMP_FALSE;
	            }
				...
				printc(FG_YELLOW("H/V Scaling not equal!\r\n"));
				if (*usW > usLCDW) {
		   	        usLCDH = *usH - usBottom - usTop;			
				}
				else if (*usH > usLCDH) {
					...
					MMP_PRINT_RET_ERROR(0, 0, "This case could not happened", 0);
				}
	   	    }
   	    }
		if (MMPS_VIDPLAY_GetColorFmt() == VID_FMT_I420) {
			...
			printc(FG_RED("ConfigScaler->YUV422 not config!\r\n"));
		}
		else {
			srcBufAttr.colordepth  = MMP_GRAPHICS_COLORDEPTH_YUV420_INTERLEAVE;		
		}
	    ...
	    //if (!m_bAHCConfig) 
		{
	        if (m_presetconfig.config[m_3gpplayData.usCurMode].display.bFullyFilled) {
	            if (m_presetconfig.config[m_3gpplayData.usCurMode].display.bKeepRatio) {
	                fitrange.fitmode = MMP_SCAL_FITMODE_OUT;
	                //RTNA_DBG_Str0(FG_YELLOW("GRA:FIT_OUT!\r\n"));
	            }
	            else {
	                fitrange.fitmode = MMP_SCAL_FITMODE_OPTIMAL;                       
	                //RTNA_DBG_Str0(FG_YELLOW("GRA:FIT_OPTIMAL!\r\n"));
	            }
	        }
	        else {
	            if (m_presetconfig.config[m_3gpplayData.usCurMode].display.bKeepRatio) {
	                fitrange.fitmode = MMP_SCAL_FITMODE_IN;
	                //RTNA_DBG_Str0(FG_YELLOW("GRA:FIT_IN!\r\n"));                
	            }
	            else {
	                fitrange.fitmode = MMP_SCAL_FITMODE_OPTIMAL;
	                MMP_PRINT_RET_ERROR(0, 0, "[WARRING] GRA:The usage not verify yet! (fitmode=OPTIMAL)!", 0);                 
	            }
	        }
	    }
        fitrange.ulInWidth      = srcBufAttr.usWidth;
        ...
        fitrange.ulInGrabX      = 1 + usLeft;
        fitrange.ulInGrabY      = 1 + usTop;
...
{  
    ...
    if (MMPS_DSC_GetVideo2MJpegEnable())
	{
        ...
        MMPS_DSC_SetVid2MJpegBuf(&curAddr,
							     m_usVid2MJpegWidth,
							     m_usVid2MJpegHeight,
							     MMP_TRUE);
...
//#endif
    ...
    // Parse and change the firmware if needed
    {
        ...
        if (error != MMP_ERR_NONE) {
            RTNA_DBG_Str0(FG_RED("PARSE failed\r\n"));
            return error;
        }
...
#else		
        //AHC_OS_GetTime(&t1);
        m_ubIsPreDecSts = MMP_TRUE;		
    	error = m_sParser.MMPD_VIDPSR_Predecode(MMP_TRUE, bPreDecAudio, bPreDecVideo ,&audio_samplerate);
        m_ubIsPreDecSts = MMP_FALSE;
        //AHC_OS_GetTime(&t2);
...
#define ATOM_SKIP_FILE_POS  	(0x24) 
    ...
    MMP_ULONG   ulFileID;
    MMP_ULONG   ulAtom, ulAtomLen, ulJpgSymbSt;
    MMP_UBYTE   *ubOutBuf = *pubOutBuf;
    ...
    *ulOutSize = 0;
    ...
    if (MMPS_3GPRECD_GetVRThumbnailSts() == MMP_FALSE) {
    ...
    do {
        ...
        case ATOM_SKIP:
            ...
            ulJpgSymbSt = MAKE_TAG_BE(buf[20], buf[21], buf[22], buf[23]);
            if (ulJpgSymbSt == 0xFFD8FFDB) {
                *ulOutSize = MAKE_TAG_BE(buf[16], buf[17], buf[18], buf[19]);
                ulAtomLen = 8 + 12;
                ...
                if (ulAtomLen > PRE_ALLOC_VR_THUMBNAIL_SIZE) {
                    err = MMP_VIDPSR_ERR_CONTENT_UNKNOWN_DATA;
                    goto VIDEO_GETTHUMB_EXIT;
                }                    
            }
        ...
        case ATOM_UDTA:
            ...
            break;
        case ATOM_PANA:
            //RTNA_DBG_Str0(FG_YELLOW("ATOM_PANA Len="));
            //RTNA_DBG_Long0(ulAtomLen);
            //RTNA_DBG_Str0("\r\n");
    ...
    if (bThumbnailFound) {
        if (*ulOutSize <= ulOutBufSize) {
            MMPS_FS_FileSeek(ulFileID, ulCurFilePos, MMPS_FS_SEEK_SET);
            err = MMPS_FS_FileRead(ulFileID, ubOutBuf, *ulOutSize, &ulBufRemain);
        }
        else {
            RTNA_DBG_Str0(FG_RED("[ERR] ThblSizes OV:"));
            RTNA_DBG_Long0(*ulOutSize);
            RTNA_DBG_Str0("\r\n");
        }
    }
		                        	...
		                        	MMP_ULONG				*pulJpegSize)
{
    ...
    if (GetFrameType == MMPS_3GP_GETFRAME_FIRST) {
        ...
        if (m_3gpplayData.bFileOpened == MMP_FALSE) {
            ...
            MMPD_VIDPLAYER_SetGetFrameMode(MMP_TRUE);
            m_ubIsThumbNail = MMP_TRUE;
            error = MMPS_VIDPLAY_Open(pFileName, ulFileNameLength, 0, MMP_TRUE, MMP_FALSE, &cfg);
            m_ubIsThumbNail = MMP_FALSE;
