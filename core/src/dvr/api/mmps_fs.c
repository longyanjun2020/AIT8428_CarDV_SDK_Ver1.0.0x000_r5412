...
//------------------------------------------------------------------------------
//  Function    : MMPS_FS_MountVolume_Ex
//  Description : HIF_USR_CMD_FS_ACCESS | FS_IS_VOLUME_MOUNTED  SUB(0x21)
//------------------------------------------------------------------------------
/**
 * @brief                 Check if a volume is mounted and has correct file system information. If necessary the volume will be re-mounted.
 *
 * @param[in ] bpVolume    Volume name string.
 * @param[in ] ulNameSize   The length, in bytes, of bpVolume string, including the terminating null character.
 *
 ...
 * @retval others         Fail.
 */
MMP_ERR MMPS_FS_MountVolume_Ex(MMP_BYTE *bpVolume, MMP_ULONG ulNameSize)
{	
    MMP_ERR bResult = MMP_ERR_NONE;
    
    if( MMPF_FS_IsVolumeMounted( bpVolume ) != MMP_ERR_NONE )
    {
        MMP_BYTE i;
        for( i=0; i<10; i++ )
        {
            MMPF_OS_SleepMs(300);
        	bResult = MMPS_FS_MountVolume(bpVolume, ulNameSize);    
