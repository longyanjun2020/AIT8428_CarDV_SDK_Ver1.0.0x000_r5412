...
#if ((VIDEO_R_EN)||(DSC_R_EN))&&(SUPPORT_MDTC)
#include "mmpf_fdtc.h"
#endif
...
__align(32) MMP_ULONG       m_ulHifParam[GRP_IDX_NUM][MAX_HIF_ARRAY_SIZE] = {{0, 0}};
...
void MMPF_HIF_FlowCtlHifISR(MMP_USHORT command)
{
    ...
    {
    	if ((command & VMD_RUN_ON_CPUB) == VMD_RUN_ON_CPUB)
		{
			#if (SCORE_EN)
    		MMPF_OS_SetFlags(CpuB_Algorithm_Flag, SYS_FLAG_VMD, MMPF_OS_FLAG_SET);
			#endif
		}
    	else if ((command & VMD_RUN_ON_CPUA) == VMD_RUN_ON_CPUA)
		{
			#if (MCORE_EN)
    		MMPF_OS_SetFlags(CpuA_Algorithm_Flag, SYS_FLAG_VMD, MMPF_OS_FLAG_SET);
			#endif
		}
    }
	else
    ...
    {
        if ((command & ADAS_RUN_ON_CPUB) == ADAS_RUN_ON_CPUB)
        {
        	#if (SCORE_EN)
			MMPF_OS_SetFlags(CpuB_Algorithm_Flag, SYS_FLAG_ADAS, MMPF_OS_FLAG_SET);
			#endif
        }
		else if ((command & ADAS_RUN_ON_CPUA) == ADAS_RUN_ON_CPUA)
		{
			#if (MCORE_EN)
			MMPF_OS_SetFlags(CpuA_Algorithm_Flag, SYS_FLAG_ADAS, MMPF_OS_FLAG_SET);
			#endif
		}
    }
    else
#endif
#if defined(SUPPORT_SPEECH_RECOG)&&(SUPPORT_SPEECH_RECOG)&& defined(ALL_FW) 
	if ((command & (FUNC_MASK|GRP_MASK)) == HIF_FCTL_CMD_AUDRECOG)
	{
		MMPF_OS_SetFlags(CpuB_Algorithm_Flag, SYS_FLAG_AUD, MMPF_OS_FLAG_SET);//run on cpub only
	}else
#endif
	{
...
MMP_ERR MMPH_HIF_SendCmd(MMP_UBYTE ubGroup, MMP_USHORT usCommand)
{
    ...
    case GRP_IDX_VID:
        ...
        ubRet = MMPF_OS_WaitFlags(DSC_UI_Flag, SYS_FLAG_VID_CMD_DONE,
			                MMPF_OS_FLAG_WAIT_SET_ANY | OS_FLAG_CONSUME, HIF_CMD_TIMEOUT*2/* Too long to parse AVI. TBD */, &flags);
        break;	
    case GRP_IDX_AUD:
