...
*/
MMP_USHORT 							m_usVid2MJpegWidth  = 1920;
MMP_USHORT 							m_usVid2MJpegHeight = 1080;
...
*/
MMP_ULONG	 				        m_ulPrimaryJpegLineStart = 0;
MMP_ULONG 					        m_ulPrimaryJpegCompStart = 0;				    // Capture buffer start address
MMP_ULONG 					        m_ulPrimaryJpegCompEnd = 0;					    // Capture buffer end address
...
static MMP_PIPE_LINK 				m_fctlLinkPlayback 			= {MMP_SCAL_PIPE_0, MMP_ICO_PIPE_0, MMP_IBC_PIPE_0};
static MMP_PIPE_LINK 				m_fctlLinkCapture 			= {MMP_SCAL_PIPE_0,	MMP_ICO_PIPE_0, MMP_IBC_PIPE_0};
static MMP_PIPE_LINK 				m_fctlLinkPreview 			= {MMP_SCAL_PIPE_1, MMP_ICO_PIPE_1, MMP_IBC_PIPE_1};
static MMP_PIPE_LINK 				m_fctlLinkThumb 			= {MMP_SCAL_PIPE_2, MMP_ICO_PIPE_2, MMP_IBC_PIPE_2};
static MMP_PIPE_LINK				m_fctlLinkLdcSrc 			= {MMP_SCAL_PIPE_3, MMP_ICO_PIPE_3, MMP_IBC_PIPE_3};
...
static MMP_ULONG    				m_ulDSCCurMemAddr           = 0;
...
/* For Ring buffer */
static MMP_ULONG					m_ulMaxContiShotRingBufSize	= 15 * 512 * 1024; // 10 -> 7.5MB Reduce for memory not enough
static MMP_ULONG					m_ulMaxVid2MJpegRingBufSize	= 512 * 1024;
static MMP_ULONG                    m_ulVid2MJpegRingBufAddr  	= 0;
...
/* For large capture. IBC store big resolution address. */
static MMP_ULONG 				    m_ulLargeJpegSourceAddr;

MMP_BOOL                            gbDualBayerSnrStartCapture = MMP_FALSE;
...
{
    MMPS_DSC_SetPreviewBuf(PRM_SENSOR, &m_DSCPreviewFctlAttr, 0, 0, MMP_FALSE);  // Release DSC preview buffer.
...
@return MMP_ERR_NONE.
*/
MMP_ERR MMPS_DSC_SetPreviewBuf(	MMP_UBYTE 		ubSnrSel,
								MMPD_FCTL_ATTR 	*pFctlPreviewAttr,
								...
								MMP_BOOL 		bAllocate)
{
    ...
    MMP_ULONG           ulCurAddr, ulPreviewBufSize, ulYSize;
	MMP_UBYTE			i = 0;
	...
	/* Pixel select */
    ulYSize = (ulDispWidth * ulDispHeight);
    ...
    ulCurAddr = ALIGN32(m_ulDSCCurMemAddr);
	for (i = 0; i < m_dscConfig.dispParams.usDscDispBufCnt[m_usDscPreviewMode]; i++) 
	{
        if (m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_YUV420) {

            pFctlPreviewAttr->ulBaseAddr[i] = ulCurAddr;
            pFctlPreviewAttr->ulBaseUAddr[i] = pFctlPreviewAttr->ulBaseAddr[i] +  ALIGN32(ulYSize);
            pFctlPreviewAttr->ulBaseVAddr[i] = pFctlPreviewAttr->ulBaseUAddr[i] + ALIGN32(ulYSize >> 2);
        }
        ...
        else if (m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_YUV422 ||
                 m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_RGB565) {
            
            pFctlPreviewAttr->ulBaseAddr[i] = ulCurAddr;
            pFctlPreviewAttr->ulBaseUAddr[i] = 0;
            pFctlPreviewAttr->ulBaseVAddr[i] = 0;
        }
        else if (m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_RGB888) {
	...
	m_ulDSCPreviewEndAddr = ulCurAddr;
    ...
    {
		// Use ulRotateAddr to accumulate for the rest ulRotateAddr	
        ulCurAddr =	m_ulDSCPreviewEndAddr;
        for (i = 0; i < pFctlPreviewAttr->usRotateBufCnt; i++) {
    		if (m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_YUV420) {
		        
    		    pFctlPreviewAttr->ulRotateAddr[i] = ulCurAddr;
                pFctlPreviewAttr->ulRotateUAddr[i] = pFctlPreviewAttr->ulRotateAddr[i] + ALIGN32(ulYSize);
                pFctlPreviewAttr->ulRotateVAddr[i] = pFctlPreviewAttr->ulRotateUAddr[i] + ALIGN32(ulYSize >> 2);
    	    }
    		else if (m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_YUV420_INTERLEAVE) {

    		    pFctlPreviewAttr->ulRotateAddr[i] = ulCurAddr;
                pFctlPreviewAttr->ulRotateUAddr[i] = pFctlPreviewAttr->ulRotateAddr[i] + ALIGN32(ulYSize);
                pFctlPreviewAttr->ulRotateVAddr[i] = pFctlPreviewAttr->ulRotateUAddr[i];
    	    }
    		else if (m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_YUV422 ||
    				 m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_RGB565) {
		    
    		    pFctlPreviewAttr->ulRotateAddr[i] = ulCurAddr;
                pFctlPreviewAttr->ulRotateUAddr[i] = 0;
                pFctlPreviewAttr->ulRotateVAddr[i] = 0;
    		}
            else if (m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode] == MMP_DISPLAY_COLOR_RGB888) {
    ...
    if (m_DscPreviewPath == MMP_DSC_RAW_PREVIEW)
    {
        ...
        if (gsHdrCfg.bDscEnable) {

            if (rawPreviewBuf.ulRawBufCnt < DSC_MIN_RAWSTORE_BUF_NUM) {
                rawPreviewBuf.ulRawBufCnt = DSC_MIN_RAWSTORE_BUF_NUM;
            }

            /* Plus 256 for buffer address alignment */
            if (gsHdrCfg.ubRawStoreBitMode == HDR_BITMODE_10BIT)
                ulRawBufSize = (2 * usSensorPreviewW * usSensorPreviewH * 4 / 3) + 256;
            else
                ulRawBufSize = (2 * usSensorPreviewW * usSensorPreviewH) + 256;

            /* Use 256Bytes alignment address for HDR store buffer */
            for (i = 0; i < rawPreviewBuf.ulRawBufCnt; i++) {
                rawPreviewBuf.ulRawBufAddr[i] = ALIGN256(m_ulDSCPreviewEndAddr);
                m_ulDSCPreviewEndAddr = rawPreviewBuf.ulRawBufAddr[i] + ulRawBufSize;
            }
            ...
            MMPD_RAWPROC_SetFetchRange(0, 0, usSensorPreviewW * 2, usSensorPreviewH, usSensorPreviewW * 2);

            MMPD_HDR_InitModule(usSensorPreviewW, usSensorPreviewH);
            MMPD_HDR_SetBufEnd(ALIGN32(m_ulDSCPreviewEndAddr));
            ...
            MMPD_RAWPROC_SetStoreBufEnd(ubVifId, &rawPreviewBuf);
    ...
    if (m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_LB_SINGLE   ||
        m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_LB_MULTI    ||
        m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_MULTISLICE) {
    ...
    #if (SUPPORT_MDTC)
    retstatus = MMPS_Sensor_AllocateVMDBuffer(PRM_SENSOR, &m_ulDSCPreviewEndAddr, MMP_TRUE);
	...
	{
		MMP_ULONG	ulWiFiH264CurAddr = m_ulDSCPreviewEndAddr;
		MMP_ULONG	ulWiFiMJPEGCurAddr = m_ulDSCPreviewEndAddr;
		
		#if (SUPPORT_H264_WIFI_STREAM)
    	if (MMPS_3GPRECD_GetConfig()->bH264WifiStreamEnable) {
    		MMP_ERR status;

            // Allocate memory for MV (Motion Vector) buffer
            MMPS_3GPRECD_AllocateMVBuffer(  &ulWiFiH264CurAddr,
                                            MMPS_3GPRECD_GetConfig()->ulMaxH264WifiStreamWidth,
                                            MMPS_3GPRECD_GetConfig()->ulMaxH264WifiStreamHeight);
        	...
        	if (status != MMP_ERR_NONE) {
        		printc("Allocate H264 Streaming Buffer failed!\r\n");	
    	    }
    	}
    	#endif
        
        #if 0
    	MMPS_DSC_SetVid2MJpegBuf(&ulWiFiMJPEGCurAddr,
							     m_usVid2MJpegWidth,
							     m_usVid2MJpegHeight,
							     MMP_TRUE);
		#endif
		
		m_ulDSCPreviewEndAddr = ulWiFiMJPEGCurAddr > ulWiFiH264CurAddr ? ulWiFiMJPEGCurAddr : ulWiFiH264CurAddr;
	}
    #endif
								...
								MMP_BOOL 	bAllocate)
{
    ...
    if (m_usCaptureMode == MMPS_DSC_MULTI_SHOTMODE) {
	    ...
	    else
	    {
    		...
    		/* Allocate ring buffer memory */ 
    		ulDramCurPos += ALIGN_X(m_ulMaxContiShotRingBufSize - ulBufSize, DSC_BUF_ALIGN_BASE);

            MMPD_DSC_SetRingBuffer(m_ulPrimaryJpegCompStart, m_ulMaxContiShotRingBufSize, 
            					   (m_ulPrimaryJpegCompEnd - m_ulPrimaryJpegCompStart),
            					   m_ulContiShotInterval);
        }
    ...
    #if (EN_AUTO_TEST_LOG)
    printc("MMPS_DSC_SetCaptureBuf mod:%d pos:%x~%x (sz:%x)\r\n", m_usCaptureMode, 
                                                                  m_ulPrimaryJpegCompStart, 
                                                                  m_ulPrimaryJpegCompEnd,
                                                                  ulBufSize);
    printc("W:%x H:%x ShotEn:%x MaxDramAdr:%x\r\n", m_usJpegWidth, 
                                                    m_usJpegHeight,
                                                    m_dscConfig.encParams.bStableShotEn,
                                                    m_ulDSCMaxDramAddr);
    #endif
	...
	#if (HANDLE_JPEG_EVENT_BY_QUEUE) // Need Remove
	/*
	...
	1.gulFrontCamTmpCompBuf - Will be assigned in function MMPF_DSC_StreamJPEGInfo
	...
	*/
    ...
    /* Allocate current buffer for capture jpeg size > scaler delay line */
	if (m_usJpegWidth > MMPD_Scaler_GetMaxDelayLineWidth(m_fctlLinkCapture.scalerpath)) {
		m_ulLargeJpegSourceAddr = ulDramCurPos;
		ulDramCurPos += m_usJpegWidth * m_usJpegHeight * 3 / 8; // NV12 format.
		MMPD_DSC_SetLargeCaptureStoreAddr(m_ulLargeJpegSourceAddr);
	}
    ...
    if (ulDramCurPos > MMPS_System_GetMemHeapEnd()) {
        printc("\t= [HeapMemErr] CaptureBuf = %x, %x\r\n", ulDramCurPos, MMPS_System_GetMemHeapEnd());
        return MMP_DSC_ERR_MEM_EXHAUSTED;
    }
...
//------------------------------------------------------------------------------
MMP_ERR MMPS_DSC_GetCaptureBuf(MMP_ULONG *pulLineBuf, MMP_ULONG *pulCompBuf, MMP_ULONG *pulCompBufEnd)
{
    if (pulLineBuf)
    ...
    if (pulCompBufEnd)
        *pulCompBufEnd = m_ulPrimaryJpegCompEnd;
...
{ 
	...
	//YUV sensor->ISP bypass->m_BayerScalerInfo uninit->sFitRangeBayer =0
	...
	if (MMPS_DSC_SetPreviewBuf(ubSnrSel, &m_DSCPreviewFctlAttr, ulPreviewBufW, ulPreviewBufH, MMP_TRUE)) {
        return MMP_DSC_ERR_MEM_EXHAUSTED;
    }
        ...
        m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_MULTISLICE)
	{
        ...
        ldcFctlAttr.bRtModeOut      = MMP_FALSE;
        ldcFctlAttr.colormode      	= MMP_DISPLAY_COLOR_YUV420_INTERLEAVE;
        #if (CCIR656_FORCE_SEL_BT601)
        ldcFctlAttr.eScalColorRange = MMP_SCAL_COLRMTX_FULLRANGE_TO_BT601;
        #else
        ldcFctlAttr.eScalColorRange = MMP_SCAL_COLRMTX_YUV_FULLRANGE;
        #endif
        ...
        ldcFctlAttr.sScalDelay      = m_sFullSpeedScalDelay;
        ldcFctlAttr.bSetScalerSrc	= MMP_TRUE;
        ldcFctlAttr.ubPipeLinkedSnr	= ubSnrSel;
    ...
    if (m_DscPreviewPath == MMP_DSC_RAW_PREVIEW) {
        ...
        if (m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_LB_SINGLE   ||
            m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_LB_MULTI    ||
            m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_MULTISLICE) {
            fitrange.ulInWidth	= m_ulDscLdcMaxSrcW;
            fitrange.ulInHeight	= m_ulDscLdcMaxSrcH;
        }
        else {
			if(CAM_CHECK_PRM(PRM_CAM_YUV_SENSOR)){
				fitrange.ulInWidth	= ulScalInW;//YUV sensor->ISP bypass->m_BayerScalerInfo uninit->sFitRangeBayer =0
            	fitrange.ulInHeight	= ulScalInH;
			}
			else{
    ...
    m_DSCPreviewFctlAttr.bRtModeOut     = MMP_FALSE;
    m_DSCPreviewFctlAttr.colormode      = m_dscConfig.dispParams.DispColorFmt[m_usDscPreviewMode];
    if (m_DSCPreviewFctlAttr.colormode == MMP_DISPLAY_COLOR_RGB565 || 
        m_DSCPreviewFctlAttr.colormode == MMP_DISPLAY_COLOR_RGB888) {
        m_DSCPreviewFctlAttr.eScalColorRange = MMP_SCAL_COLRMTX_FULLRANGE_TO_RGB;
    }
    else {
        #if (CCIR656_FORCE_SEL_BT601)
        m_DSCPreviewFctlAttr.eScalColorRange = MMP_SCAL_COLRMTX_FULLRANGE_TO_BT601;
        #else
        m_DSCPreviewFctlAttr.eScalColorRange = MMP_SCAL_COLRMTX_YUV_FULLRANGE;
        #endif
    }
    ...
    if (m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_LB_SINGLE   ||
        m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_LB_MULTI    ||
        m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_MULTISLICE)
        m_DSCPreviewFctlAttr.scalsrc        = MMP_SCAL_SOURCE_LDC;
    else
        m_DSCPreviewFctlAttr.scalsrc        = MMP_SCAL_SOURCE_ISP;
    
    m_DSCPreviewFctlAttr.sScalDelay         = m_sFullSpeedScalDelay;
    m_DSCPreviewFctlAttr.bSetScalerSrc      = MMP_TRUE;
    m_DSCPreviewFctlAttr.ubPipeLinkedSnr    = ubSnrSel;
    ...
    if (m_sAhcDscPrevInfo.bUserDefine) {
...
MMP_ERR MMPS_DSC_EnablePreviewDisplay(MMP_UBYTE ubSnrSel, MMP_BOOL bEnable, MMP_BOOL bCheckFrameEnd)
{
    ...
    if (bEnable) {
        ...
        switch(m_dscConfig.dispParams.sPreviewPath) {
    		case MMP_DSC_PATH_2PIPE:
    		...
    		break;
    		case MMP_DSC_PATH_LDC_LB_SINGLE:
    		case MMP_DSC_PATH_LDC_LB_MULTI:
    		case MMP_DSC_PATH_LDC_MULTISLICE:
...
//------------------------------------------------------------------------------
//  Function    : MMPS_DSC_SetVid2MJpegBuf
//  Description : Allocate/Release memory for MJPEG line/compress buffer.
//------------------------------------------------------------------------------
MMP_ERR MMPS_DSC_SetVid2MJpegBuf(MMP_ULONG *pulCurBufPos, MMP_USHORT usEncW, MMP_USHORT usEncH, MMP_BOOL bAllocate)
{
    MMP_DSC_CAPTURE_BUF  	JpgCaptureBuf;
    MMP_ULONG				ulBufSize;
	...
	JpgCaptureBuf.ulCompressEnd		= JpgCaptureBuf.ulCompressStart + ulBufSize;
   	...
   	MMPD_DSC_SetRingBuffer(JpgCaptureBuf.ulCompressStart, m_ulMaxVid2MJpegRingBufSize, 
            			   (JpgCaptureBuf.ulCompressEnd - JpgCaptureBuf.ulCompressStart),
            			   0);

	ulDramCurPos += m_ulMaxVid2MJpegRingBufSize;
	
	m_ulVid2MJpegRingBufAddr = JpgCaptureBuf.ulCompressStart;
...
//------------------------------------------------------------------------------
//  Function    : MMPS_DSC_GetVid2MJpegCompBuf
//  Description : Get MJPEG compress buffer info.
//------------------------------------------------------------------------------
MMP_ERR MMPS_DSC_GetVid2MJpegCompBuf(MMP_ULONG *pulBufAddr, MMP_ULONG *pulBufSize)
{
    *pulBufAddr = m_ulVid2MJpegRingBufAddr;
    *pulBufSize = m_ulMaxVid2MJpegRingBufSize;
...
MMP_ERR MMPS_DSC_ConfigVideo2Mjpeg(MMP_SHORT usInputW, MMP_USHORT usInputH)
{
    ...
    MMP_SCAL_GRAB_CTRL			grabctl;
    MMP_SCAL_FIT_RANGE			fitrange;
    MMP_PIPE_LINK				fctllink = {MMP_SCAL_PIPE_0, MMP_ICO_PIPE_0, MMP_IBC_PIPE_0};
    ...
    MMPS_Display_PureAdjustScaleInSize(	MMP_FALSE, 
                                        usInputW, usInputH, 
                                        m_usVid2MJpegWidth, m_usVid2MJpegHeight,
                                        &ulSuggestWidth, &ulSuggestHeight);

    /* Aligned with JEPG HW */
    m_usVid2MJpegWidth = ALIGN16(ulSuggestWidth);
    m_usVid2MJpegHeight = ALIGN8(ulSuggestHeight);

    fitrange.fitmode        = MMP_SCAL_FITMODE_OUT;
    fitrange.scalerType 	= MMP_SCAL_TYPE_SCALER;
    fitrange.ulInWidth      = rect.usWidth;
    fitrange.ulInHeight     = rect.usHeight;
    fitrange.ulOutWidth     = m_usVid2MJpegWidth;
    fitrange.ulOutHeight    = m_usVid2MJpegHeight;
	...
	error = MMPS_MJPEG_OpenStream(MMP_MJPEG_STREAM_FRONTCAM_VIDEO, MMP_MJPEG_UI_MODE_WIFI_VR, &handle);
    ...
    MMPD_Streaming_SetCompBuf(VIDEO1_RING_STREAM, MMP_MJPEG_UI_MODE_WIFI_VR, ulDramAddr, ulBufSize, MMP_STREAM_JPEG);
...
MMP_ERR MMPS_DSC_SetJpegEncParam(MMP_USHORT usJpegResol, MMP_USHORT usW, MMP_USHORT usH, MMP_UBYTE ubRcIdx)
{
    ...
    if (m_dscConfig.dispParams.sPreviewPath == MMP_DSC_PATH_LDC_LB_SINGLE   ||
        ...
        if (m_usJpegWidth > m_ulDscLdcMaxSrcW)
        	m_usJpegWidth = FLOOR16(m_ulDscLdcMaxSrcW);
        if (m_usJpegHeight > m_ulDscLdcMaxSrcH)
        	m_usJpegHeight = FLOOR8(m_ulDscLdcMaxSrcH);
    }
                                        ...
                                        MMP_DSC_JPEG_QUALITY 	qualitylevel)
{
    ...
    if (!m_bUseCustomJpegQT) {
	    MMPD_DSC_SetJpegQTableEx(ubRcIdx, 
                                 (MMP_UBYTE *)m_dscConfig.encParams.ubDscQtable[qualitylevel],
	                             (MMP_UBYTE *)m_dscConfig.encParams.ubDscQtable[qualitylevel] + DSC_QTABLE_ARRAY_SIZE,
	                             (MMP_UBYTE *)m_dscConfig.encParams.ubDscQtable[qualitylevel] + DSC_QTABLE_ARRAY_SIZE, 
                                 qualitylevel);
	} 
							    ...
							    MMP_BOOL                        bRecording)
{
	...
	{		
    	MMPS_DSC_SetJpegEncParam(DSC_RESOL_IDX_UNDEF, pVRCaptInfo->usWidth, pVRCaptInfo->usHeight, MMP_DSC_JPEG_RC_ID_NONE);
	}
    ...
    if (bRecording) {
        MMP_UBYTE      ubEncIdx;
        MMP_IBC_PIPEID  ubRecPipe = MMP_IBC_PIPE_0;
        MMP_ULONG       ulBufW, ulBufH;
            
		if (CAM_CHECK_USB(USB_CAM_SONIX_MJPEG2H264))
		{
	        if(ScalerSrc == MMP_SCAL_SOURCE_ISP){
	            ubEncIdx = MMPS_3GPRECD_FILESTREAM_NORMAL;
	        }else if(ScalerSrc == MMP_SCAL_SOURCE_JPG){
	            ubEncIdx = MMPS_3GPRECD_FILESTREAM_DUAL;
	        }else{
	            MMP_PRINT_RET_ERROR(0, 0, "Scaler Src illegal\r\n",gubMmpDbgBk);
	            return MMP_3GPRECD_ERR_STILL_CAPTURE;
	        }

	        MMPS_3GPRECD_GetRecordPipe(ubEncIdx, &ubRecPipe);
	        MMPS_3GPRECD_GetRecordPipeBufWH(ubEncIdx, &ulBufW, &ulBufH);		
		}
		else
		{
	        MMPS_3GPRECD_GetRecordPipe(MMPS_3GPRECD_FILESTREAM_NORMAL, &ubRecPipe);
	        MMPS_3GPRECD_GetRecordPipeBufWH(MMPS_3GPRECD_FILESTREAM_NORMAL, &ulBufW, &ulBufH);
		}
		...
		if (CAM_CHECK_USB(USB_CAM_SONIX_MJPEG2H264))
		{
	        if(ubEncIdx == MMPS_3GPRECD_FILESTREAM_NORMAL){
	        	MMPD_Fctl_LinkPipeToGra2JPEGQ(ubRecPipe, pVRCaptInfo->usWidth, pVRCaptInfo->usHeight, MMP_FALSE);
	        }
	        else if(ubEncIdx == MMPS_3GPRECD_FILESTREAM_DUAL){
	        	MMPD_Fctl_LinkPipeToGra2JPEGQ(ubRecPipe, pCaptureFitrange->ulInWidth, pCaptureFitrange->ulOutHeight, MMP_TRUE);
	        }
		}
		else
		{
	        //MMPF_JPEGCTL_UseH264Cur2StillJpeg((MMP_UBYTE)ubRecPipe, ulBufW, ulBufH, pVRCaptInfo->usWidth, pVRCaptInfo->usHeight);
	        MMPD_Fctl_LinkPipeToGra2JPEGQ(ubRecPipe, pVRCaptInfo->usWidth, pVRCaptInfo->usHeight, MMP_FALSE);
		}
    ...
    #endif
    {
        ...
        /* Because of changing pipe from pipe3 (preview) to pipe0 (record) */
        ...
        fctlAttr.eScalColorRange    = MMP_SCAL_COLRMTX_FULLRANGE_TO_BT601; // CHECK
		...
		{		
	        if(ScalerSrc == MMP_SCAL_SOURCE_ISP){
	            bStillCapture = MMPD_FCTL_GRA2JPEG_STILLCAP_FRONTCAM;
	        }
	        else if(ScalerSrc == MMP_SCAL_SOURCE_JPG){
	            bStillCapture = MMPD_FCTL_GRA2JPEG_STILLCAP_SONIXREARCAM;
	        }        
		}
		else
		{
        ...
        MMPD_Fctl_LinkPipeToGra2JPEG(pipeID, winID, previewDev, bStillCapture, MMP_FALSE);
        
        /* Change the path to the frame source */
        MMPD_IBC_SetInterruptEnable(pipeID, MMP_IBC_EVENT_FRM_RDY, MMP_TRUE);
        ...
        MMPD_IBC_SetInterruptEnable(pipeID, MMP_IBC_EVENT_FRM_RDY, MMP_FALSE);
        MMPD_Scaler_SetPath(pipeID, MMP_SCAL_SOURCE_LDC, MMP_TRUE);
    }
...
//------------------------------------------------------------------------------
//  Function    : MMPS_DSC_CaptureAfterDualBayerSnrReady
...
@return It reports the status of the operation.
*/
MMP_ERR MMPS_DSC_CaptureAfterDualBayerSnrReady(	MMP_IBC_PIPEID 			pipeID,
                                                MMPS_DSC_CAPTURE_INFO 	*pCaptureInfo,
                                                MMP_SCAL_SOURCE			ScalerSrc,
                                                MMP_SCAL_FIT_RANGE 		*pCaptureFitrange,
                                                MMP_SCAL_GRAB_CTRL 		*pCaptureGrabctl,
                                                ...
                                                MMP_BOOL                bLdcSrc)
{
    ...
    if (!bLdcSrc) {
        ...
        MMPF_IBC_UpdateStoreAddress(pipeID, 0, 0, 0);
    }
    #endif
    ...
    gbDualBayerSnrStartCapture = MMP_TRUE;
    
    if (bLdcSrc) {
        err = MMPD_DSC_TakeJpegAfterLdcReady();
    }
    else {
        err = MMPD_DSC_TakeJpegNotTrigSrc();
    }
    
    if (err) {
        printc("Still capture failed\r\n");
        goto L_Restore;
    }
    ...
    *pCaptureSize = pCaptureInfo->ulJpegFileSize;
    ...
    if (m_MultiShotInfo.ulUsedEndAddr[m_MultiShotInfo.usShotIndex] > m_ulPrimaryJpegCompEnd) {
        ...
        printc("The Compressed Data is over JPEG compressed buffer\r\n");        
        err = MMP_DSC_ERR_CAPTURE_BUFFER_OVERFLOW;
        goto L_Restore;
    }
    ...
    gbDualBayerSnrStartCapture = MMP_FALSE;
...
//------------------------------------------------------------------------------
//  Function    : MMPS_DSC_CaptureAfterLdcReady
//  Description : For video snapshot
//------------------------------------------------------------------------------
/** @brief preview and capture at the same pipe.
...
@return It reports the status of the operation.
*/
MMP_ERR MMPS_DSC_CaptureAfterLdcReady(	MMP_IBC_PIPEID 			pipeID,
                                        MMPS_DSC_CAPTURE_INFO 	*pCaptureInfo,
                                        MMP_SCAL_SOURCE			ScalerSrc,
                                        MMP_SCAL_FIT_RANGE 		*pCaptureFitrange,
                                        MMP_SCAL_GRAB_CTRL 		*pCaptureGrabctl,
								 		...
								 		MMP_BOOL                bDualCamCapture)
{
	...
	/* Back up the Scaler/Icon/IBC setting and close the IBC of preview path */
    MMPD_Scaler_GetPath((MMP_SCAL_PIPEID) pipeID, &ScalerSrcOrig);
    ...
    #endif
    {
        ...
        err = MMPD_DSC_TakeJpegAfterSrcReady(0/*m_ub1stVRStreamSnrId*/);
    }
...
L_Restore:
    ...
    /* Reset display rotate DMA buffer index */
	if (rotateDir != MMP_DISPLAY_ROTATE_NO_ROTATE) {
        MMPD_Display_ResetRotateDMABufIdx(MMP_DISPLAY_SRC_FRONTCAM);
        MMPD_Display_ResetRotateDMABufIdx(MMP_DISPLAY_SRC_REARCAM);
    }
    
    /* Disable IBC store and reset it */  
	MMPD_IBC_SetStoreEnable(pipeID, MMP_FALSE);
	...
	MMPD_IBC_ClearFrameEnd(pipeID);
    MMPD_Scaler_SetPath(pipeID, ScalerSrcOrig, MMP_TRUE);
    MMPD_IBC_CheckFrameEnd(pipeID);
...
MMP_ERR MMPS_DSC_CaptureTakeJpeg(MMP_UBYTE ubSnrSel, MMPS_DSC_CAPTURE_INFO *pCaptureInfo)
{
    ...
    /* PreLink IBC frame done handler for large size capture */ 
	MMPD_Fctl_LinkPipeToMultiRunJpeg(m_fctlLinkCapture.ibcpipeID);
    ...
    if (m_MultiShotInfo.ulUsedEndAddr[m_MultiShotInfo.usShotIndex] > m_ulPrimaryJpegCompEnd) {
    	RTNA_DBG_Str0("The Compressed Data is over JPEG compressed buffer\r\n");
        RTNA_DBG_Str0(FG_RED("Allocate Sz:"));
        RTNA_DBG_Long0((m_ulPrimaryJpegCompEnd - m_ulPrimaryJpegCompStart));
        RTNA_DBG_Str0(" EncSz");
        RTNA_DBG_Long0(pCaptureInfo->ulJpegFileSize);
        RTNA_DBG_Str0("\r\n");
    ...
    if (pCaptureInfo->bExif) {
        ...
        if (m_MultiShotInfo.usShotIndex) {
        	MMPD_DSC_KillExifForMultiShot(EXIF_NODE_ID_PRIMARY); // CHECK
        }
										...
										MMP_SCAL_PIPEID        	pipeID)
{
		...
		ubThumbnailInputMode == MMP_DSC_THUMB_SRC_THUMB_PIPE)
	{
	    ...
	    #if 0 // Mark for fill up the thumb without black part use optimal mode	
		/* Step 2: prepare raw image */
		...
		if (ubThumbnailInputMode == MMP_DSC_THUMB_SRC_DECODED_JPEG)
		{
			decodebuf.ulLineBufStart    = bufAttr.ulBaseAddr + ALIGN_X(ulThumbWidth * 2, DSC_BUF_ALIGN_BASE) * ALIGN_X(ulThumbHeight, DSC_BUF_ALIGN_BASE);
...
//------------------------------------------------------------------------------
//  Function    : MMPS_DSC_SetLargeCaptureInfo
//  Description :
//------------------------------------------------------------------------------
/** @brief The function use for raw fetch multi-times. And capture big resolution (Bigger than scaler delay line)

The function use for raw fetct multi times. And capture big resolution (Bigger than scaler delay line 
@param[in] ubDivideNum 
@return MMP_ERR_NONE
*/
MMP_ERR MMPS_DSC_SetLargeCaptureInfo(MMP_UBYTE ubDivideNum)
{
	MMPD_DSC_SetLargeCaptureInfo(ubDivideNum);
	return MMP_ERR_NONE;
}
...
MMP_ERR MMPS_DSC_SetJpegDisplay(MMP_ULONG ulGrabX, MMP_ULONG ulGrabY, MMP_ULONG ulGrabW, MMP_ULONG ulGrabH)
{
    ...
    MMP_GRAPHICS_ROTATE_TYPE    sGraRotateType = MMP_GRAPHICS_ROTATE_NO_ROTATE;
    MMP_GRAPHICS_RECT       	rect;
    MMP_GRAPHICS_BUF_ATTR 		dstBufAttr;
     ...
     */  
    ratioX = (MMP_ULONG)(m_JpegPlayInfo.usJpegWidth << 12) / m_JpegPlayInfo.usDecodeWidth;
    ratioY = (MMP_ULONG)(m_JpegPlayInfo.usJpegHeight << 12) / m_JpegPlayInfo.usDecodeHeight;

    ulGrabX = (ulGrabX << 12) / ratioX;
    ulGrabY = (ulGrabY << 12) / ratioY;
    ulGrabW = (ulGrabW << 12) / ratioX;
    ulGrabH = (ulGrabH << 12) / ratioY;
    ...
    dispAttr.usDisplayOffsetX   = m_JpegPlayInfo.usDispWinOfstX;
   	...
   	if ((m_JpegPlayInfo.rotateType  == MMP_DISPLAY_ROTATE_NO_ROTATE) ||
        ...
        /* Use DMA to do rotate operation to replace use display module */
        rect.usLeft             = 0;
        rect.usTop              = 0;
        ...
        if (m_JpegPlayInfo.rotateType == MMP_DISPLAY_ROTATE_RIGHT_90)
            sGraRotateType = MMP_GRAPHICS_ROTATE_RIGHT_90;
        else if (m_JpegPlayInfo.rotateType == MMP_DISPLAY_ROTATE_RIGHT_270)    
            sGraRotateType = MMP_GRAPHICS_ROTATE_RIGHT_270;
        ...
        /* Waiting the DMA rotate buf to buf done */
        while (!m_bDMADone);
    ...
    /* Set window scaling function */
   	do {
		...
		if (fitrange.fitmode == MMP_SCAL_FITMODE_IN || 
		    m_JpegPlayInfo.jpeginfo.bPowerOffLogo == MMP_TRUE) {
	        fitrange.ulOutWidth  = dispAttr.usDisplayWidth;
    	    fitrange.ulOutHeight = dispAttr.usDisplayHeight;
		}
		else {
	        fitrange.ulOutWidth  = m_JpegPlayInfo.usDisplayWidth;
    	    fitrange.ulOutHeight = m_JpegPlayInfo.usDisplayHeight;
		}
...
MMP_ERR MMPS_DSC_PlaybackJpeg(MMP_DSC_JPEG_INFO *pJpegInfo)
{
    ...
    DBG_AutoTestPrint(ATEST_ACT_PB_PHOTO_0x0004, ATEST_STS_START_0x0001, 0, 0, gubMmpDbgBk);
    DBG_AutoTestPrint(ATEST_ACT_PB_PHOTO_0x0004, ATEST_STS_RSOL_SIZE_0x0004, pJpegInfo->usPrimaryWidth, pJpegInfo->usPrimaryHeight, gubMmpDbgBk);
    DBG_AutoTestPrint(ATEST_ACT_PB_PHOTO_0x0004, ATEST_STS_END_0x0003, 0, retstatus, gubMmpDbgBk);
	...
	if (pJpegInfo->bPowerOffLogo == MMP_TRUE) {
	...
	if (pJpegInfo->bPowerOffLogo == MMP_FALSE && 
	    fitrange.fitmode == MMP_SCAL_FITMODE_OPTIMAL) {
	    if (m_sAhcDscPlayInfo.bUserDefine) {
		    if ((RotateType  == MMP_DISPLAY_ROTATE_NO_ROTATE) || 
	 			(RotateType  == MMP_DISPLAY_ROTATE_RIGHT_180)) {
	        	m_JpegPlayInfo.usDisplayWidth = m_sAhcDscPlayInfo.ulDecodeOutW;
	        	m_JpegPlayInfo.usDisplayHeight = m_sAhcDscPlayInfo.ulDecodeOutH;
			}
			else {
	        	m_JpegPlayInfo.usDisplayWidth = m_sAhcDscPlayInfo.ulDecodeOutH;
	        	m_JpegPlayInfo.usDisplayHeight = m_sAhcDscPlayInfo.ulDecodeOutW;		
			}
	    }        
	    else {
		    if ((RotateType  == MMP_DISPLAY_ROTATE_NO_ROTATE) || 
	 			(RotateType  == MMP_DISPLAY_ROTATE_RIGHT_180)) {
		        m_JpegPlayInfo.usDisplayWidth = m_dscConfig.decParams.usDecodeOutW[m_usDecodeMode];
	    	    m_JpegPlayInfo.usDisplayHeight = m_dscConfig.decParams.usDecodeOutH[m_usDecodeMode];
			}
			else {
		        m_JpegPlayInfo.usDisplayWidth = m_dscConfig.decParams.usDecodeOutH[m_usDecodeMode];
	    	    m_JpegPlayInfo.usDisplayHeight = m_dscConfig.decParams.usDecodeOutW[m_usDecodeMode];		
			}
	    }
	}
	else {
    ...
    if (fitrange.fitmode == MMP_SCAL_FITMODE_IN) {
        ...
        if ((RotateType  == MMP_DISPLAY_ROTATE_NO_ROTATE) || 
            ...
            if (pJpegInfo->bPowerOffLogo == MMP_TRUE) {
                ...
                m_JpegPlayInfo.usDispWinOfstX   = 0;
                m_JpegPlayInfo.usDispWinOfstY   = 0;
			}
        }
        else {
            if (pJpegInfo->bPowerOffLogo == MMP_TRUE) {
                                  ...
                                  MMP_DISPLAY_COLORMODE  	outputformat)
{
    ...
    MMP_ULONG                   ulDscSramBufAddr;
    MMP_ULONG                   ulDscDramBufAddr;
    MMP_BOOL    				bInPreview      = MMP_FALSE;

    /* Allocate filename buffer, decompressed buffer, line buffer */
    MMPS_3GPRECD_GetPreviewPipeStatus(PRM_SENSOR, &bInPreview);
    
    if (bInPreview) {
        MMPS_3GPRECD_GetStillCaptureEndAddr(&ulDscSramBufAddr, &ulDscDramBufAddr);
        MMPS_DSC_SetSystemBuf(&ulDscDramBufAddr, MMP_FALSE, MMP_TRUE, MMP_TRUE);
    }
    else {
        MMPS_DSC_SetSystemBuf(NULL, MMP_FALSE, MMP_TRUE, MMP_TRUE);
    }
    ...
    if (bInPreview) {
        MMPS_DSC_SetSystemBuf(&ulDscDramBufAddr, MMP_FALSE, MMP_TRUE, MMP_FALSE);
    }
    else {
        MMPS_DSC_SetSystemBuf(NULL, MMP_FALSE, MMP_TRUE, MMP_FALSE);
    }
...
MMP_ERR MMPS_DSC_ThumbDram2Card(MMPS_DSC_CAPTURE_INFO *pCaptureInfo, MMP_ULONG ulStreamType)
{
    ...
    MMPD_DSC_SetThumbFileName(pCaptureInfo->ubFilename, pCaptureInfo->usFilenamelen);

   	ret = MMPD_DSC_ThumbDram2Card(ulStreamType, MMP_TRUE, MMP_TRUE);
    
    if (ret != MMP_ERR_NONE) {
       RTNA_DBG_Str0("MMPS_DSC_ThumbDram2Card ret:");
       RTNA_DBG_Long0(ret);
       RTNA_DBG_Long0(__LINE__);
       RTNA_DBG_Str0("\r\n");
       ret = MMP_DSC_ERR_SAVE_CARD_FAIL;
    }
...
#if 0
void _____MBOOT_Functions_____(){}
#endif
...
/* Playback pipe selection */
static MMP_PIPE_LINK 				m_fctlLinkPlayback;
