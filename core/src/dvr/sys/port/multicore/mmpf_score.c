...
static OS_STK   AUDRECOG_Task_Stk[SCORE_TASK_AUDRECOG_STK_SIZE];
static OS_STK   AUDRECOG_Routine_Task_Stk[SCORE_TASK_AUDRECOG_ROUTINE_STK_SIZE];
#endif
...
//------------------------------------------------------------------------------
//	Function	: MMPF_SCORE_HandleAUDIORECOG
//	Description : 
//------------------------------------------------------------------------------
/** @brief The function allows slave core to handle audio recog request from master core.
...
void MMPF_SCORE_HandleSpeechRecog(void *p_arg)
{
	MMPF_SCORE_HandleRequest(CPU_COMM_ID_AUDIORECOG, MMPF_SpeechRecog_ProcRequest);
}
#endif
#if (MDTC_ON_SCORE)
//------------------------------------------------------------------------------
...
CPU_COMM_ERR MMPF_SCORE_Init(void)
{
	...
	#if (defined(SUPPORT_SPEECH_RECOG)&&(SUPPORT_SPEECH_RECOG))
    /* Register entries for MDTC request ID */
    err = CpuComm_RegisterEntry(CPU_COMM_ID_AUDIORECOG, CPU_COMM_TYPE_DATA);

    /* Create task for MDTC request ID */
    if (err == CPU_COMM_ERR_NONE) {
        task_cfg.ubPriority = SCORE_TASK_RECOG_PRIO;
        task_cfg.pbos = (MMP_ULONG)&AUDRECOG_Task_Stk[0];
        task_cfg.ptos = (MMP_ULONG)&AUDRECOG_Task_Stk[SCORE_TASK_AUDRECOG_STK_SIZE - 1];
        MMPF_OS_CreateTask(MMPF_SCORE_HandleSpeechRecog, &task_cfg, (void *)0);
        MMPF_OS_SetTaskName(task_cfg.ubPriority, (MMP_UBYTE *)"AUDIOREG");
    }
    if (err) {
        SCORE_ERR("#SC_Err: SpeechRecog task\r\n");
        return err;
    }

    /* Create task for Routine request ID */
    if (err == CPU_COMM_ERR_NONE) {
        task_cfg.ubPriority = SCORE_TASK_RECOG_ROUTINE_PRIO;
        task_cfg.pbos = (MMP_ULONG)&AUDRECOG_Routine_Task_Stk[0];
        task_cfg.ptos = (MMP_ULONG)&AUDRECOG_Routine_Task_Stk[SCORE_TASK_AUDRECOG_ROUTINE_STK_SIZE - 1];
        MMPF_OS_CreateTask(MMPF_SpeechRecog_Routine, &task_cfg, (void *)0);
        MMPF_OS_SetTaskName(task_cfg.ubPriority, (MMP_UBYTE *)"AUDIOREG_ROUTINE");
    }
    if (err) {
        SCORE_ERR("#SC_Err: SpeechRecog Routine task\r\n");
        return err;
    }
	#endif
