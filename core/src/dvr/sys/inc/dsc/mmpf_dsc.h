...
typedef enum _MMPF_DSC_IMAGE_SOURCE
{
    ...
    MMPF_DSC_SRC_TRIG_OTHERWHERE            ///< Image source is triggered otherwhere.
...
typedef struct _MMPF_DSC_LARGECAPTURE_INFO {
	MMP_ULONG   ulOutStoreYAddr;
	MMP_ULONG   ulOutStoreUAddr;
	MMP_ULONG   ulOutStoreVAddr;
	MMP_ULONG   ulOutFrameW;
	MMP_ULONG   ulOutFrameH;
	MMP_UBYTE   ubCurrentRun;
	MMP_UBYTE   ubSnrSel;
	MMP_BOOL    ubIconEn;
} MMPF_DSC_LARGECAPTURE_INFO;
...
#if (HANDLE_JPEG_EVENT_BY_QUEUE)
MMP_ERR MMPF_MJPEG_SetDualStreamingEnable(MMP_BOOL bFrontCamEn, MMP_BOOL bRearCamEn);
MMP_UBYTE MMPF_MJPEG_GetEncPipeForDualStreaming(void);
MMP_ULONG MMPF_MJPEG_GetFrontCamRsvdBufSizeForDualStreaming(void);
MMP_ULONG MMPF_MJPEG_GetRearCamRsvdBufSizeForDualStreaming(void);
#endif

/* Internal Function */
int VRThumbnailJpegEncDoneCBFunc(void* pArg);
...
MMP_ERR MMPF_JPG_SetDecodeMode(MMPF_JPEG_DECODE_MODE ubMode);
...
MMP_UBYTE MMPF_DSC_GetCapturePath(void);
...
void MMPF_DSC_ConfigSensorEXIF(void);
...
MMP_ERR MMPF_DSC_GetJpegBitStream(MMP_ULONG *ulJpegbufAddr, MMP_ULONG *ulJpegbufSize);
...
MMP_ERR MMPF_DSC_ScanJpegMarker(MMP_ULONG 			JpegStartOffset,
								...
								MMP_USHORT 			*pusJpgWidth,
								MMP_USHORT 			*pusJpgHeight,
...
/* Large Capture Function */
MMP_ERR MMPF_DSC_SetLargeCaptureInfo(MMP_UBYTE ubDivideNum);
MMP_ERR MMPF_DSC_SetLargeCaptureStoreAddr(MMP_ULONG ulAddr);
MMP_ERR MMPF_DSC_InitLargeCaptureAttr(MMP_UBYTE ubSnrSel, MMP_ULONG ulJpegWidth, MMP_ULONG ulJpegHeight, MMP_ULONG ulAddr);
MMP_ERR MMPF_DSC_UpdateLargeCapture(void);
MMP_ERR MMPF_DSC_TriggerLargeEncode(void);
