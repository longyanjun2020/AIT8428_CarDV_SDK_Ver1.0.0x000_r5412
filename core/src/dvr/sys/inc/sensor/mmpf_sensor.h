...
/* ISP Queue */
typedef struct _MMPF_ISP_CTL_ATTR 
{
    MMP_UBYTE           ubSnrId;                        ///< Sensor ID
    MMP_UBYTE           ubRawId;                        ///< Raw ID
    MMP_UBYTE           ubDummy[2];
} MMPF_ISP_CTL_ATTR;

typedef struct _MMPF_ISP_CTL_QUEUE 
{
    MMPF_ISP_CTL_ATTR   attr[MMPF_ISP_MAX_QUEUE_SIZE];  ///< Queue for ready to raw fetch
...
typedef struct _MMPF_SENSOR_CUSTOMER {
    ...
    void (*MMPF_Cust_CheckSnrTVSrcType)(MMP_UBYTE ubSnrSel, MMP_SNR_TVDEC_SRC_TYPE *TVSrcType);
...
typedef struct _MMPF_SENSOR_FUNCTIION 
{
    ...
    MMP_ERR (*MMPF_Sensor_CheckSnrTVSrcType)(MMP_UBYTE ubSnrSel, MMP_SNR_TVDEC_SRC_TYPE *TVSrcType);
...
MMP_ERR MMPF_Sensor_RegisterCallback(MMPF_SENSOR_EVENT ulEvent, void *pCallback);

void MMPF_Sensor_GetTVDecSrcType(MMP_SNR_TVDEC_SRC_TYPE *sSnrTVSrc);
void MMPF_Sensor_CheckTVDecSrcType(MMP_UBYTE ubSnrSel, MMP_SNR_TVDEC_SRC_TYPE *sSnrTVSrc);
void MMPF_Sensor_EnableTVDecSrcTypeDetection(MMP_BOOL bEnable);

extern MMP_ERR MMPF_SensorDrv_Register(MMP_UBYTE ubSnrSel, MMPF_SENSOR_CUSTOMER* pCust);

__weak void MMP_SensorDrv_PostInitISP(void);
__weak void MMP_SensorDrv_FirstInitISPCB(void);
__weak void MMP_SensorDrv_FirstInitSensorCB(void);
