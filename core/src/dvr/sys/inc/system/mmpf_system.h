...
#define TASK_MONITOR_TIMEOUT        						(10000)	//ms
#endif
...
#define TASK_MONITOR_EVALUROUNDS    (100)
...
typedef enum _MMPF_SYS_FLOW_MONITOR_STATES
{
    MMPF_SYS_FLOW_MONITOR_STATES_WAITING = 0,
    MMPF_SYS_FLOW_MONITOR_STATES_RUNNING    
} MMPF_SYS_FLOW_MONITOR_STATES;

typedef MMP_BOOL (SYS_FLOW_MONITOR_CheckFunction) (void);
typedef void (SYS_FLOW_MONITOR_FailureCallBack) (void);
#endif
...
#if (SYSTEM_FLOW_MONITOR_ENABLE)
typedef struct _MMPF_SYS_FLOW_MONITOR {
    MMP_UBYTE                       SystemFlowName[SYSTEM_FLOW_MONITOR_MAX_NAME_LEN];
    MMPF_SYS_FLOW_MONITOR_STATES    sSystemMonitorStates;
    MMP_ULONG                       ulExecTime;
    MMP_ULONG                       ulTimeout; //If ulTimeout is 0, system flow checking routine will call pCheckFunc directly.
    SYS_FLOW_MONITOR_CheckFunction  *pCheckFunc;
    SYS_FLOW_MONITOR_FailureCallBack *pFailureCB;
} MMPF_SYS_FLOW_MONITOR; 
#endif
...
#if (SYSTEM_FLOW_MONITOR_ENABLE)
MMP_ERR MMPF_SystemFlowMonitor_Register(MMPF_SYS_FLOW_MONITOR *pSystemFlowMonitor, MMPF_SYSTEM_FLOW_MONITOR_GROUP sSystemFlowGroup);
MMP_ERR MMPF_SystemFlowMonitor_UnRegister(MMPF_SYS_FLOW_MONITOR *pSystemFlowMonitor, MMPF_SYSTEM_FLOW_MONITOR_GROUP sSystemFlowGroup);
void MMPF_SystemFlowMonitor_TimerCB(void);
#endif
