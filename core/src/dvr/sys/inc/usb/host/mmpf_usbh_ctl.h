...
#define EP0_DATA_PACKET_SIZE                (5120)
...
#define FRM_RECD_RX_LOST_MIN_TH             (4) // Min threshold to check record frame Rx lost count only, not include preview frame
...
typedef __packed struct _UVCH_XFER_CTL {
    ...
    MMP_UBYTE                   ubUVCRecdEn;                    // Update by MMPS_UVCRECD_StartRecd, MMPS_UVCRECD_StopRecd, MMPS_3GPRECD_PreRecord and MMPF_USBH_NotifySlowCardCallBack,
    MMP_UBYTE                   ubUVCWiFiEn;
#if (EN_USBH_ALWAYS_INPUT_FRM)
    MMP_UBYTE                   ubRecdStopReqEx;                // Triggered by MMPS_UVCRECD_StopRecd,
    MMP_UBYTE                   ubRecdStopDone;                 // Update by MMPS_UVCRECD_StopRecd and MMPF_USBH_CheckNotify2StopUVCRecord,
...
typedef __packed struct _MMPF_USBH_ERROR_HANDLE_CFG 
{
    ...
    MMP_UBYTE                   ubRecdFrmRxTimerStart;                              //TRUE,FALSE
    ...
    MMP_UBYTE                   ubRecdFrmRxTimeoutTHValid;                          //TRUE,FALSE
    MMP_UBYTE                   ubUpdFrmRxTimeoutParm;                              //TRUE,FALSE
    MMP_UBYTE                   ubUpdRecdFrmRxTimeoutParm;                          //TRUE,FALSE
    MMP_UBYTE                   ubFrmRxTimeoutCallbackDone[FRM_RX_TIMEOUT_MAX_HDL]; //TRUE,FALSE
    MMP_UBYTE                   ubRecdFrmRxTimeoutCallbackDone;                     //TRUE,FALSE
    ...
    MMP_UBYTE                   ubRecdFrmRxTimeoutTriggered;                        //TRUE,FALSE
    MMP_ULONG                   ulFrmRxLostTH[FRM_RX_TIMEOUT_MAX_HDL];              //#
    MMP_ULONG                   ulRecdFrmRxLostTH;                                  //#
    ...
    MMP_ULONG                   ulRecdFrmRxTimeoutInterval;                         //ms
    ...
    MMP_ULONG                   ulCurWaitRecdFrmRxMsCnt;                            //ms
    #if (EN_REMOVE_TRAN_HW_TIMER)
    ...
    MMPF_USBH_Callback          *DevRecdFrmRxTimeoutCallback;
...
extern MMP_ULONG                glMjpegFrmSize[];
...
MMP_ERR MMPF_USBH_AllocFBMemory(MMP_ULONG plStartAddr, MMP_ULONG *plAllocSize);
...
MMP_ERR MMPF_USBH_StopRecdFrmRxTimer(void);
MMP_ERR MMPF_USBH_RestartRecdFrmRxTimer(void);
MMP_ERR MMPF_USBH_SetRecdFrmRxTimeoutCFG(MMP_ULONG pulRecdFrmRxLostTH, void *pDevRecdFrmRxLostTHCallback);
...
#if (SUPPORT_DEC_MJPEG_TO_PREVIEW)
int MMPF_USBH_DecJpgDoneCallback(void* pArgu);
...
MMP_ERR MMPF_USBH_RstMjpegToWiFiRingBuf(void);
