...
static void MMPF_AVIPSR_GetAudioHeader(MMP_ULONG size)
{
	...
	m_ulAudioBitRate = gVidFileInfo.audio_info.bitrate;
				...
				if (stream_index < 0) {
					...
					case CODEC_TYPE_AUDIO:
						...
						if (gVidFileInfo.audio_info.format == MMP_VID_AUDIO_AAC && pMedia->ulSampleSize != 0
						    ...
						    if (gVidFileInfo.audio_info.bitrate != 0 && pMedia->ulTotalFrame != AVI_UNDEFINESAMPLENUM) {
						        gVidFileInfo.audio_info.total_audio_time = pMedia->ulStreamTime
...
//End_of_Header:
	...
	#if (SPEED_UP_AVI_OPEN)
	#else
	MMPF_AVIPSR_SetIndex(AviCtx.ulMoviList, AviCtx.ulMoviEnd, gVidFileInfo.file_size, pVidMedia->ulTrackID);
