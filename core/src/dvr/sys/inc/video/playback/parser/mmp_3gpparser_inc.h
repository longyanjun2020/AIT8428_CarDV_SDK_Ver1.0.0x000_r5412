...
#endif
MMP_BOOL                        m_ubIsPreDecSts = MMP_FALSE;
    ...
    if (gDemuxOption & DEMUX_VIDEO) 
    {
        ...
        if (!m_3GPPdata.bFillBufferDone) {
            ...
            if (gubIsVidDualTrks) {
                ...
                {
                	//check spending time for different FPS tracks.
                	if ((gVideoMedia[m_ubVideoDmxTrackNo].ulFrameIndex * gVideoMedia[m_ubVideoDmxTrackNo].ulSttsDuration * 100 / gVideoMedia[m_ubVideoDmxTrackNo].ulTimeScale)
							> (gVideoMedia[(m_ubVideoDmxTrackNo + 1) & 0x01].ulFrameIndex * gVideoMedia[(m_ubVideoDmxTrackNo + 1) & 0x01].ulSttsDuration * 100 / gVideoMedia[(m_ubVideoDmxTrackNo + 1) & 0x01].ulTimeScale))
					{
						m_ubVideoDmxTrackNo++;
