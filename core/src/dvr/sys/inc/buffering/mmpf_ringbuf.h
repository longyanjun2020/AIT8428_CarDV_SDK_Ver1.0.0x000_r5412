...
#define FRAME_ADD_ERR_NONE  		(0)
#define FRAME_ADD_ERR_SPACE  		(-1)
#define FRAME_ADD_ERR_BUSY  		(-2)
...
typedef struct _MMPF_RING2_BUF {
    ...
    /* Frame Queue */
    MMP_USHORT 				usWrIdx;			// The write index.
    MMP_USHORT 				usRdIdx;			// The read index
    MMP_USHORT 				usTailIdx;			// The tail index
    MMP_ULONG				ulInterval;
    MMPF_FRAME_STATIST		sStatInfo;			// The frame statistics
    MMPF_FRAME_INFO 		sFrameInfo[RING2_MAX_FRAME_QUEUE_SIZE];       
} MMPF_RING2_BUF;
...
typedef const struct _MMPF_STREAM_RING_FORMAT   *MMPF_STREAM_RING_FORMAT;
...
void 		MMPF_FrameRing_GetNextReadAddr(MMPF_FRAME_RING_BUF_HANDLE pRing, MMP_ULONG *FrameAddr, MMP_ULONG *Seg1Size);
...
MMP_ERR MMPF_Ring2_InitBuf(MMP_ULONG ulAddr, MMP_ULONG ulSize, MMP_ULONG ulRsvdSize, 
						   MMP_USHORT usAlignment, MMP_ULONG ulInterval);
MMP_ERR MMPF_Ring2_GetNextReadAddr(MMPF_FRAME_INFO *pFrameInfo);						   
MMP_ERR MMPF_Ring2_GetNextWriteAddr(MMPF_FRAME_INFO *pCurFrameInfo, MMP_ULONG *pNextWriteAddr);
MMP_ULONG MMPF_Ring2_GetFrameRsvdSize(void);
MMP_ULONG MMPF_Ring2_GetWritedFrameCnt(void);
