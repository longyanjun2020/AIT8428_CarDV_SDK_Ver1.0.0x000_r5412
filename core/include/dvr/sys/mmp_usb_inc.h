...
#define MSDC_R_TOTAL_SIZE       (MSDC_R_FIRSTSIZE + MSDC_R_SECONDSIZE + \
                                 MSDC_R_THIRDSIZE + MSDC_R_FOURTHSIZE)
#define MSDC_W_TOTAL_SIZE       (MSDC_W_FIRSTSIZE + MSDC_W_SECONDSIZE + \
                                 MSDC_W_THIRDSIZE + MSDC_W_FOURTHSIZE)
 ...
 */
typedef __packed struct _USB_REQUEST {
    uByte   bmRequestType;
    uByte   bRequest;
    uWord   wValue;
    uWord   wIndex;
 ...
 */
typedef __packed struct _USB_CONFIG_DESC {
    uByte   bLength;
    uByte   bDescriptorType;
    uWord   wTotalLength;
 ...
 */
typedef __packed struct _USB_INTERFACE_DESC {
    uByte   bLength;
    uByte   bDescriptorType;
 ...
 */
typedef __packed struct _USB_DESC_HEAD {
    uByte   bLength;
    uByte   bDescriptorType;
} USB_DESC_HEAD;
 ...
 */
typedef __packed struct _USB_HUB_DESC {
    uByte   bDescLength;
    uByte   bDescriptorType;
    uByte   bNbPorts;
 ...
 * [USB_20 - 11.24.2.7]
 */
typedef __packed struct _USB_HUB_PORT_STATUS {
    uWord   wPortStatus;
    uWord   wPortChange;
} USB_HUB_PORT_STATUS;
 ...
 */
typedef __packed struct _UVC_INTERFACE_ASSOCIATION_DESC {
    uByte       bLength;
    uByte       bDescriptorType;
...
typedef __packed struct _USB_UVC_DEV_INFO {
    MMP_ULONG   ulVIDPID;
...
typedef __packed struct _USB_UVC_MJPEG_WIFI_CFG {
    MMP_BOOL    bEnWiFi;
...
typedef __packed struct _USBH_UVC_H264_NALU_CFG 
{
