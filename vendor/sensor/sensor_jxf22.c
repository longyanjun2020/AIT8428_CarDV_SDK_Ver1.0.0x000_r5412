...
#define MAX_SENSOR_GAIN     (16)          // max sensor gain 
...
//  Mode0   Mode1
    ...
    {1125,  1134},	/* usVsyncLine */  //Reg 0x22 0x23
    {1,     1},  /* ubWBinningN */
    {1,     1},  /* ubWBinningN */
    {1,     1},  /* ubWBinningN */
    {1,     1},  /* ubWBinningN */
...
const ISP_UINT8 Sensor_IQ_CompressedText[] = 
{
    ...
    #include "isp_8428_iq_data_v3_JXF22_ezmode.xls.ciq.txt"	
#else //Use old IQ table
#if(CHIP == MCR_V2)
	#include "isp_8428_iq_data_v2_JXF02_20160505_Daniel.xls.ciq.txt"	//Adjust Lux reference
#endif
#endif
};
...
// IQ Data
#define ISP_IQ_DATA_NAME "isp_8428_iq_data_v2_JXF02_20160505_Daniel.xls.ciq.txt"
...
MMP_USHORT SNR_JXF22_Reg_Init_Customer[] = 
{
    0x12, 0x80,  // [7]: software reset, [6]: sleep mode, [5]:mirror, [4]: flip, [3]: HDR mode
};

// 1080p 30FPS
MMP_USHORT SNR_JXF22_Reg_1920x1080_Customer[] = 
{
...
0x8F,0x18,
0x91,0x04,
0x0C,0x00, //0x01, Test Pattern wilsonjin
0x59,0x97,
0x4A,0x05,
0x49,0x10,
0x50,0x02,
...
#else
// [INI_Register] for [5]:mirror, [4]: flip,
0x12,0x70,                                                                                    
0x0E,0x11,                                                                                    
0x0F,0x0C,                                                                                    
0x10,0x44,                                                                                    
...
0x80,0x00,                                                                                    
0x13,0x81,                                                                                    
0x12,0x30,  // [7]: software reset, [6]: sleep mode, [5]:mirror, [4]: flip, [3]: HDR mode     
0x93,0x5C,                                                                                    
0x45,0x89,                                                                                    
SENSOR_DELAY_REG, 500, //I2C Sleep 100ms(min)                                                
0x45,0x09,                                                                                    
0x1F,0x01
#endif
...
// 1080p 60FPS
MMP_USHORT SNR_JXF22_Reg_1920x1080_60_Customer[] = 
{
#if (SENSOR_IF == SENSOR_IF_MIPI_1_LANE)
//TODO
...
static void SNR_Cust_InitConfig(void)
{
	...
	RTNA_DBG_Str(0, "SNR_Cust_IniConfig JXF22\r\n");
	
    // Init OPR Table
    SensorCustFunc.OprTable->usInitSize                      = (sizeof(SNR_JXF22_Reg_Init_Customer)/sizeof(SNR_JXF22_Reg_Init_Customer[0]))/2;
    SensorCustFunc.OprTable->uspInitTable                    = &SNR_JXF22_Reg_Init_Customer[0];    
    
    SensorCustFunc.OprTable->usSize[RES_IDX_1920x1080]       = (sizeof(SNR_JXF22_Reg_1920x1080_Customer)/sizeof(SNR_JXF22_Reg_1920x1080_Customer[0]))/2;
    SensorCustFunc.OprTable->uspTable[RES_IDX_1920x1080]     = &SNR_JXF22_Reg_1920x1080_Customer[0];

    SensorCustFunc.OprTable->usSize[RES_IDX_1920x1080_60]    = (sizeof(SNR_JXF22_Reg_1920x1080_60_Customer)/sizeof(SNR_JXF22_Reg_1920x1080_60_Customer[0]))/2;
    SensorCustFunc.OprTable->uspTable[RES_IDX_1920x1080_60]  = &SNR_JXF22_Reg_1920x1080_60_Customer[0];
   ...
   // Init Vif Setting : PowerOn Setting
    ...
    for (i = 0; i < MAX_SENSOR_RES_MODE; i++)
    {
        ...
        SensorCustFunc.VifSetting->colorId.CustomColorId.Rot180d_Id[i] = MMPF_VIF_COLORID_01;
        SensorCustFunc.VifSetting->colorId.CustomColorId.Rot270d_Id[i] = MMPF_VIF_COLORID_UNDEF;
        SensorCustFunc.VifSetting->colorId.CustomColorId.H_Flip_Id[i]  = MMPF_VIF_COLORID_11;
        SensorCustFunc.VifSetting->colorId.CustomColorId.V_Flip_Id[i]  = MMPF_VIF_COLORID_11;
        SensorCustFunc.VifSetting->colorId.CustomColorId.HV_Flip_Id[i] = MMPF_VIF_COLORID_01;
    }
}

//------------------------------------------------------------------------------
//  Function    : SNR_JXF22_DoAE_FrmSt
//  Description :
//------------------------------------------------------------------------------
