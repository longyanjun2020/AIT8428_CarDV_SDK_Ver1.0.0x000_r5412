...
#include "isp_8428_iq_data_v2_H42_20160601_Daniel.xls.ciq.txt"
#endif
#endif
};
...
// IQ Data
#define ISP_IQ_DATA_NAME "isp_8428_iq_data_v2_H42_20160105.xls.ciq.txt"
...
void SNR_Cust_DoAE_FrmSt(MMP_UBYTE ubSnrSel, MMP_ULONG ulFrameCnt)
{
	...
	//patch code !!!!!!!
	#define low_lux 10
	#define lowest_ratio 0x20
	if ((ulFrameCnt % ubPeriod) == 2){
        ISP_UINT32 Lux,luma_ratio ;
        Lux = ISP_IF_AE_GetLightCond();
        
		if( Lux < low_lux )
			luma_ratio = lowest_ratio;
		else if( Lux < (low_lux+100) )
            luma_ratio = lowest_ratio + (0x80-lowest_ratio)*(Lux - low_lux)/100;
        else
			luma_ratio = 128;
...
void SNR_Cust_SetGain(MMP_UBYTE ubSnrSel, MMP_ULONG ulGain)
{
	...
	gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0x0C, VFPN_SW*0x40);     //Total gain = (2^PGA[6:4])*(1+PGA[3:0]/16) //PGA:0x00   
