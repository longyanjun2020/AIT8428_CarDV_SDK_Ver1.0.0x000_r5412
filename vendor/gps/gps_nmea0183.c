...
MMP_BYTE 		recv_data[15]={'\0'};
//MMP_BYTE 		RecvVolume = 0;
...
void Bluetooth_Recieve_data(MMP_BYTE *fifoa); // long 5-4
...
#endif
#if(UART_2_TO_MCU)
void UART2_String_RxCallback(MMP_UBYTE size, volatile MMP_UBYTE *fifo)
{
 	MMP_BYTE chGPSchar;
   	MMP_BYTE 		chCommand[20]={0};
	//printc("size  2=%d !!!!\n",size);
    while(size--)
	{
       		 chGPSchar =  *fifo;
			 //RecvVolume = *fifo;
		//chCommand[0]=chGPSchar;
		#if 0
		if(chGPSchar == 0x55)
		{
			flag_add = 1;
		...
		else if(chGPSchar == 0xaa)
		{
			flag_sub= 1;
			exit_flag = 0;
		}
		#endif
		Bluetooth_Recieve_data(&chGPSchar);
		chCommand[0]=chGPSchar;
		//SetKeyPadEvent(DEVICES_READY);
		//MMPF_Uart_Write(GPS_UART_NUM, chCommand, 1);
		// printc("chGPSchar2   -----0x%02x--------\n",chGPSchar);
		// MMPF_Uart_Write(GPS_UART_NUM, chCommand, 1); // lyj 20181123
		//MMPF_Uart_Write(MMP_UART_ID_3, chCommand, 1);

    }
	//MMPF_Uart_Write(GPS_UART_NUM, chCommand, 1); // lyj 20190118
	//printc("~~~~~~~end~~~~~~end~~~~~~end~~~~~~~~~~~~~\n");
	//SetKeyPadEvent(VIRTUAL_EVENT);
...
MMP_BYTE 		chCommand[20]={0};
    ...
    while(size--)
	{
        ...
        chGPSchar =  *fifo;
		//Bluetooth_Recieve_data(&chGPSchar); // lyj 20190115
			chCommand[0]=chGPSchar;
			//chCommand[size]=chGPSchar;
	//printc();
 //       m_bGPSString[m_ulEndPosition] = chGPSchar;

		// printc("chGPSchar1    -----%c--------\r\n",chGPSchar);

	//printc("--0x%02x--%d",chGPSchar,m_ulEndPosition);
	//printc("--0x%02x--size-%d---",chCommand[size],size);
	#if(UART_2_TO_MCU)
		#if 0
	...
	{}
		...
		#endif // lyj 20181026
	//MMPF_Uart_Write(UART_2_NUM, chCommand, 1);// tou chan
	#endif
...
// 	MMPF_OS_SetFlags(UartCtrlFlag, GPS_FLAG_GETGPSPACK, MMPF_OS_FLAG_SET); // GPS control flow to get GPS package from UART
	//MMPF_Uart_Write(UART_2_NUM, chCommand, 14);
   // printc("\r\n");
}

//=====================================
void Bluetooth_Recieve_data(  MMP_BYTE *fifoa)
{
	MMP_BYTE 		temp_data = 0xac;
	MMP_BYTE 		temp_data1 = 'U';
	MMP_BYTE 		temp_data2 = 0x0a;
	int i;

	int nNextPos = (m_ulEndPosition + 1);

	recv_data[m_ulEndPosition] = *fifoa;
	//printc("%d-->0x%02x---  ",m_ulEndPosition,recv_data[m_ulEndPosition]);
	
	m_ulEndPosition = nNextPos;

	if(recv_data[0] == 0x77 /*&& recv_data[1] == 0x00*/)
	{
			flag_add = 1;
			exit_flag = 0;
			m_ulEndPosition = 0;
	...
	else if(recv_data[0] == 0xbb /*&& recv_data[1] == 0x00*/)
	{
			flag_sub= 1;
			exit_flag = 0;
	...
	if(recv_data[0] != 0xaa/*&& m_ulEndPosition >1*/)
	{
		m_ulEndPosition = 0; // 20181024
	}

	//printc("~~~%d~~\r\n",STRCMP(&recv_data[1],&temp_data1));
     //STRCMP(&recv_data[1],&temp_data1);

	if( m_ulEndPosition == 14)
	{

		#if 0
		for(i = 0;i<14;i++)
		{
			printc("%d-->0x%02x  ",i,recv_data[i]);
		}
		printc("\r\n");
		#endif
...
#if 1
	//recv_data[1] = 0x55; 
	if(recv_data[6] == 0x0A && m_ulEndPosition == 7)
	{
		#if 0
		for(i = 0;i<7;i++)
		{
...
#if 1
	if( recv_data[8] == 0x0A  && m_ulEndPosition == 9)
	{
		#if 0
		for(i = 0;i<9;i++)
		{
...
#if(UART_2_TO_MCU)
void InitPathUART2_To_MCU(MMP_ULONG ulBaudRate)
{
	MMP_UART_ATTR uartattribute;
    MMP_LONG error;
    
	uartattribute.bParityEn 		= MMP_FALSE;
	uartattribute.parity 			= MMP_UART_PARITY_NONE;// lyj MMP_UART_PARITY_ODD
	uartattribute.bFlowCtlEn 		= MMP_FALSE;
	uartattribute.ubFlowCtlSelect 	= 1;	
       uartattribute.padset 		= UART_2_PIN;
       uartattribute.ulMasterclk 		= UART2_MASTER_CLK * 1000 ;
	uartattribute.ulBaudrate 		= UART2_BAUD_RATE;
	
	error = MMPF_Uart_Open(UART_2_NUM, &uartattribute);
	
	if(error)
		printc("Uart2 Open Error : %d\r\n",error);
	printc("-------------UART2--_Initial------------------\r\n");
	MMPF_Uart_EnableRx(UART_2_NUM, 1,(UartCallBackFunc *)&UART2_String_RxCallback);// register Uart2
	
	if(error)
		printc("Uart2 UseRxDMA : %d\r\n",error);
...
void GPS_InitPathUART_NMEA0183(MMP_ULONG ulBaudRate)
{
	...
	printc("-------------UART--GPS_Initial------------------\r\n");
	MMPF_Uart_EnableRx(GPS_UART_NUM, 1,(UartCallBackFunc *)&GPS_String_RxCallback); // long 5-4
...
//通道
void converseChannel(MMP_BYTE channel)
{
	MMP_BYTE 		chCommand10[20]={
		0xaa,0xac,0x03,0x00,
		0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,
		0x53,0x55};

	chCommand10[3]	= channel;
	
	MMPF_Uart_Write(UART_2_NUM, chCommand10, 14);
}
...
// 开机
void uart_to_mcu(void)
{
	MMP_BYTE 		chCommand1[20]={
		0xaa,0x80,0x00,0xff,
		0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0xff,
		0x7f,0x55};

		MMPF_Uart_Write(UART_2_NUM, chCommand1, 14);
		//MMPF_Uart_Write(UART_2_NUM, "\r\n", 2); AA 80 00 FF 00 00 00 00 00 00 00 FF 
...
void uart_to_mcu_off(void)
{

	MMP_BYTE 		chCommand2[20]={0xaa,0x80,0xff,0x00,
								         0x00,0x00,0x00,0x00,
									  0x00,0x00,0xff,0x00,
									  0x7f,0x55};

	MMPF_Uart_Write(UART_2_NUM, chCommand2, 14);
	printc("~~~~~~~send~~~off~~~~finish!\r\n");

}

//关机上传

void uart_to_mcu_off_report(void)
{

	MMP_BYTE 		chCommand3[20]={0xaa,0x90,0x11,0x11,
									0x11,0x11,0x00,0x00,
									0xee,0xee,0xee,0xee,
									0x4f,0x55};

	MMPF_Uart_Write(UART_2_NUM, chCommand3, 14);
	printc("~~~~~~~send~~~off_report~~~~finish!\r\n");

}

void uart_to_mcu_voice(MMP_BYTE typev,MMP_BYTE val,MMP_BYTE mutef)
{
	MMP_BYTE chCommand4[15] = {0xaa,0x11,0xc1,0x32,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xee,0x55};
	//0xAW,0xAC,0x00,0xF2,0x02,0xAF,0x66
	chCommand4[2] = typev;
	chCommand4[3] = val;
	chCommand4[4] = mutef;
	MMPF_Uart_Write(UART_2_NUM, chCommand4, 14);
