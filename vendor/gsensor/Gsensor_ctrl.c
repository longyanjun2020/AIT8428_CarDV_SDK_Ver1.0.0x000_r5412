...
void AHC_Gsensor_OpenInfoFile(INT8 *ubFileName)
{
    ...
    else 
    {
        ...
        STRCPY(fname, (char *)((FS_FILE *)m_ulInfoFileId)->FileNameMark);        
        printc("%s, gsnr field not null.\r\n", __func__);
        printc("new: %s; ori: %s\r\n", ubFileName, fname);
    }
...
MMP_BOOL AHC_Gsensor_RawWrite(MMP_BOOL bWriteFile)//(MMP_BYTE *pBuff, MMP_USHORT usLength)
{
    ...
    //printc("xyz[0]=%5.3f,xyz[1]=%5.3f,xyz[2]=%5.3f\r\n", xyz[0]/100, xyz[1]/100, xyz[2]/100);
			...
			{
			    xyz[0] = (float)(sdata.acc_val[0])/GSENSOR_ACCELERATION_UNIT_SC7A30E; 
			    xyz[1] = (float)(sdata.acc_val[1])/GSENSOR_ACCELERATION_UNIT_SC7A30E;
				xyz[2] = (float)(sdata.acc_val[2])/GSENSOR_ACCELERATION_UNIT_SC7A30E;
			}
			else
			{
				xyz[0] = (abs((signed char)(sdata.acc_val[0])) *100) / GSENSOR_ACCELERATION_UNIT_DMARD06;
	...
	//#endif
	#elif (GSENSOR_MODULE == GSENSOR_MODULE_SC7A30E)
					err = AHC_Gsensor_IOControl(GSNR_CMD_READ_ACCEL_XYZ, (MMP_UBYTE*) &sdata);
	...
	if (bWriteFile)
	{
		if (!uiGetParkingModeEnable() && (uiGetParkingCfg()->bParkingModeFuncEn == AHC_TRUE))
		{
