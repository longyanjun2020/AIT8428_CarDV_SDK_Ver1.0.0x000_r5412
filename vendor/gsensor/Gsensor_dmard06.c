...
MMP_ERR Gsensor_Read_XYZ(GSENSOR_DATA *data) 
{
	...
	data->acc_val[X_AXIS] = (MMP_UBYTE)x_value;
	data->acc_val[Y_AXIS] = (MMP_UBYTE)y_value;
	data->acc_val[Z_AXIS] = (MMP_UBYTE)z_value;
...
MMP_ERR Gsensor_Read_XYZ_WithCompensation(GSENSOR_DATA *sd) 
{
	...
	//Offset Compensation
	v = ((signed char)sd->acc_val[X_AXIS] - (signed char)m_GSensorOffset.acc_val[X_AXIS]);
	if (v > 0x7F) 
		v = 0x7F;
	else if (v < -0x7F) 
	 	v = 0x80;
	...
	v = ((signed char)sd->acc_val[Y_AXIS] - (signed char)m_GSensorOffset.acc_val[Y_AXIS]);
	if (v > 0x7F) 
		v = 0x7F;
	else if (v < -0x7F) 
		v = 0x80;
	...
	v = ((signed char)sd->acc_val[Z_AXIS] - (signed char)m_GSensorOffset.acc_val[Z_AXIS]);
	if (v > 0x7F) 
		v = 0x7F;
	else if (v < -0x7F) 
		v = 0x80;
	...
	switch(cmd)
	{
		case GSNR_CMD_RESET: 
			...
			if (status == MMP_ERR_NONE)
			{
				...
				gGsensor_Attribute.ubGroundAxis = AXIS_UNKNOWN;
				Gsensor_Initial();
		...
		case GSNR_CMD_SET_INT_THD: 
		{
			...
			Gsensor_EnableIntXYZ(gGsensor_Attribute.ubGroundAxis != X_AXIS,
								 gGsensor_Attribute.ubGroundAxis != Y_AXIS,
								 gGsensor_Attribute.ubGroundAxis != Z_AXIS);
			break;
		}
		...
		case GSNR_CMD_WRITE_REG:
			status |= Gsensor_WriteRegister(*arg , *(arg+1));
		break;
		
		case GSNR_CMD_READ_REG:
			status |= Gsensor_ReadRegister(*arg , arg+1);
		break;
		...
		case GSNR_CMD_GROUND_AXIS_STATUS: 
		{
			...
			if (gGsensor_Attribute.ubGroundAxis == AXIS_UNKNOWN)
