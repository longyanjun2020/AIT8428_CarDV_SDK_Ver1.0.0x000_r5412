...
void RTNA_LCD_InitMainLCD(void)
{
    ...
    m_PanelAttr.usHBPorch     = 7;          // Unit:pixel, Ragne:0~1023  7
    m_PanelAttr.usHBlanking   =10;        // Unit:pixel, Range:0~1023
    m_PanelAttr.usHSyncW      = 2;           // Unit:pixel, Range:0~255 2
    m_PanelAttr.usVBPorch     = 5;          // Unit:line,  Range:0~1023  4
    m_PanelAttr.usVBlanking   = 9;        // Unit:line,  Range:0~1023
    m_PanelAttr.usVSyncW      = 4;           // Unit:line,  Range:0~255   2
    m_PanelAttr.usPRT2HdotClk = 2;      // Unit:Dot Clock, Range:0~255
	//m_PanelAttr.ubDclkPor     = 0;
	//m_PanelAttr.ubHsyncPor	= 1;
	//m_PanelAttr.ubVsyncPor	= 1;
...
void RTNA_LCD_Init(void)
{
    ...
    LCD_ST7796S_INDEX(0x02);//0x42  02
    LCD_ST7796S_INDEX(0x3B);
    
   // LCD_ST7796S_CMD(0xB7) ;//duoyu
    //LCD_ST7796S_INDEX(0xC6) ;//c6
    ...
    LCD_ST7796S_INDEX(0x0C);// 48

	LCD_ST7796S_CMD(0x3A); // Interface Mode Control RGB565
    LCD_ST7796S_INDEX(0x66);//65k 0x50  66
...
LCD_ST7796S_CMD(0xb6);
LCD_ST7796S_INDEX(0xe0) ;  //0XE0  0XA0
LCD_ST7796S_INDEX(0x63) ; 
LCD_ST7796S_INDEX(0x3b) ; 
...
LCD_ST7796S_CMD(0x3a); 
LCD_ST7796S_INDEX(0x66); 
...
LCD_ST7796S_CMD(0x29); //Display ON
   ...
   MMPF_OS_Sleep_MS(120); // Delay 120ms
    ...
    LCD_ST7796S_INDEX(0x01);
#if 0
	 LCD_ST7796S_CMD(0xB5);
	  LCD_ST7796S_INDEX(0x04);
	   LCD_ST7796S_INDEX(0x02);
	    LCD_ST7796S_INDEX(0x00);
		 LCD_ST7796S_INDEX(0x04);
#endif    
    LCD_ST7796S_CMD(0xB6);   // display function control
    LCD_ST7796S_INDEX(0x20);
    LCD_ST7796S_INDEX(0x02);//0x42
    LCD_ST7796S_INDEX(0x3B);
...
LCD_ST7796S_CMD(0x11); // Sleep Out
    ...
    LCD_ST7796S_INDEX(0x96) ;
	LCD_ST7796S_CMD(0x3A); // Interface Mode Control RGB565
    LCD_ST7796S_INDEX(0x66);//65k 0x50
    LCD_ST7796S_CMD(0x36);
    LCD_ST7796S_INDEX(0x48);
    LCD_ST7796S_CMD(0xB4);
    LCD_ST7796S_INDEX(0x01);
