...
//-----------------------------------------------------------------------------
static void insert_preset(u16 frequency, u8 rssi, u8 *seek_preset_rssi,u8 *i)
{
    ...
    if(rssi < min_rssi || frequency == 9600 || frequency == 9900)
        return;
    ...
    for ( i = min_rssi_preset; i < NUM_SEEK_PRESETS; i++ )
    {
	    ...
	    if(frequency == 9600 || frequency == 9900)
	    {
	    	if((*i) != 0)
	    		(*i)--;
			return;
...
u8 si47xxFMRX_autoseek(void) {
    ...
    while((seek_fail == 0)) {
		...
		//printc("~~~current_frequency = %d~~seek_fail = %d~ rssi = %d  Valid = %d\r\n",current_frequency,seek_fail,si47xxFMRX_get_rssi(),Valid);
		if(Valid)
		{
        	insert_preset(current_frequency, si47xxFMRX_get_rssi(), seek_preset_rssi,&i);
			i++;
		}
   ...
   // num_found = (num_found > NUM_SEEK_PRESETS) ? NUM_SEEK_PRESETS : num_found;
	...
	num_found = i;
	//printc("~lyj~~~num_found = %d~~~~\r\n",num_found);
	
	    for ( i = 0; i < num_found; i++ ) {

		printc("~~rssi[%d] = %d~~seek_preset[%d] = %d~\r\n",i,seek_preset_rssi[i],i,seek_preset[i]);
    }
