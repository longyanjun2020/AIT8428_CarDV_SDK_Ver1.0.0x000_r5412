...
SD_UPDATER_ERR SDUpdateCheckFileExisted(MMP_BYTE *bFileName)
{
	...
	//Andy Liu TBD.
		...
		if( STRCMP(SD_FW_UPDATER_BIN_NAME,bFileName)==0) // lyj 20191018
		{
...
//=====================================
	uiError = AHC_FS_FileOpen(SD_FW_UPDATER_BIN_MASS_NAME, STRLEN(SD_FW_UPDATER_BIN_MASS_NAME),"rb", sizeof("rb"),&uiFileId);
	if(uiError==AHC_ERR_NONE)
    {
	    AHC_FS_FileClose(uiFileId);
	    printc("FW Update Bin MASS Existed \n");
        return AHC_TRUE;
    }	else
    	{
    	printc("FW Update Bin MASS  N/A \n");
        return AHC_FALSE;
    	}
