...
#define LCD_BUILT_IN_PATTERN_TEST 0
//==============================================================================
//
//                              CONSTANT
//
...
static void GetMenuStatusInSF(void)
{
    ...
    SF_Err = MMPF_SF_InitialInterface();
        ...
        if( *(buf+ulOffset) == 0xFF) {
            if( *(buf+ulOffset+1) == 0xC0) {
				...
				if ((bSOF.uwWidth *  bSOF.uwHeight) >= (320*240)) {
	                RTNA_DBG_Str0(FG_YELLOW("JPG_LOGO[W:H]/["));
    	            DBG_D0(bSOF.uwWidth,4);
        	        RTNA_DBG_Str0(FG_YELLOW(":"));            
            	    DBG_D0(bSOF.uwHeight,4);            
                	RTNA_DBG_Str0(FG_YELLOW("]\r\n"));                
					bErr = MMP_FALSE;			
					break;
				}
...
int OpenBmp(AIT_STORAGE_INDEX_TABLE2 *bt, IMG *img, unsigned char** pptr, int idx)
{
	...
	MMPF_MMU_FlushDCacheMVA((MMP_ULONG)buf, total);    
...
static int OpenImage(AIT_STORAGE_INDEX_TABLE2 *bt, IMG *img, unsigned char** pptr, int idx)
{
	...
	MMPF_MMU_FlushDCacheMVA((MMP_ULONG)buf, 2);
    MMPF_SF_FastReadData((bt->it[patno].ulStartSec<<9),
    					 (int)buf,
    					 2);
	...
	if ( bwSOIType == BM) {
		...
		MMPF_MMU_FlushDCacheMVA((MMP_ULONG)buf, (bt->it[patno].ulSecSize<<9));		
...
static void CarDV_PowerOn_Check(AIT_STORAGE_INDEX_TABLE2 *bt, DISP_PROP	*dp)
{
	...
	if (KEY_GPIO_POWER != MMP_GPIO_MAX) {
		...
		#if 1 // lyj 20190605
		  RTNA_DBG_Str(0, FG_GREEN("--I-- Power-on by PWER_KEY_LONG:  =="));
	...
	#if defined(USB_IN_DETECT_VBUS_PIN_EX)
    // Get USB status. Set V_BUS_DETECT_GPIO as input mode
	if (USB_IN_DETECT_VBUS_PIN_EX != MMP_GPIO_MAX) {
        MMPF_PIO_PadConfig(USB_IN_DETECT_VBUS_PIN_EX, PAD_NORMAL_TRIG, MMP_TRUE);
	    MMPF_PIO_EnableOutputMode(USB_IN_DETECT_VBUS_PIN_EX, MMP_FALSE, MMP_TRUE);
	    MMPF_PIO_GetData((MMP_GPIO_PIN)USB_IN_DETECT_VBUS_PIN_EX, &bUSBStatus);
...
void Customer_Init_CarDV(void)
{
    	...
    	pIndexTable->ulTail   == INDEX_TABLE_TAIL)
    {
		...
		// Show at Center of SCREEN
		{
			...
			{
			    if (OpenImage(pIndexTable, &theimg, &img, POWER_ON_IDX)) {
			    	#if ((VERTICAL_LCD == VERTICAL_LCD_DEGREE_270) || (VERTICAL_LCD == VERTICAL_LCD_DEGREE_90) )
                    ...
                    #if LCD_BUILT_IN_PATTERN_TEST
                    {
                     MMP_USHORT patternIdx=0;
                     do {
                        /* Default test pattern color format: RGB888 */
                        /* Slide show the test pattern: color checker --> 
                                                        line inverter -->
                                                        luma rump in v dir --> 
                                                        pixel inv --> 
                                                        color bar --> 
                                                        boundary detection  -->  
                                                        Luma step in H dir */
                        DisplayDrawGray((MMP_ULONG)img, dp.disp_w, dp.disp_h, CLR_SCREEN);	
                        DisplayDrawGray((MMP_ULONG)img, dp.disp_w, dp.disp_h, patternIdx++); //patternIdx++);
                        patternIdx = (patternIdx == CLR_SCREEN)? 0: patternIdx;
                    #endif
                    #if ((VERTICAL_LCD == VERTICAL_LCD_DEGREE_270) || (VERTICAL_LCD == VERTICAL_LCD_DEGREE_90) )
                    {
                        ...
                        sRet = MMPD_DMA_RotateImageBuftoBuf(&srcBufAttr, 
                                                            ...
                                                            0,
                                                            0, 
                                                             #if (VERTICAL_LCD == VERTICAL_LCD_DEGREE_270) 
                    ...
                    #if LCD_BUILT_IN_PATTERN_TEST                                              
                        DrawPowerOnImage(&dp, img, dp.disp_w, dp.disp_h, 24, 0, 0);
                        MMPF_OS_Sleep(5000);
                        MMPS_Display_SetWinActive(MMP_DISPLAY_WIN_OVERLAY, 0); 
                        img = (unsigned char*)AIT_SYS_MEDIA_START_ADDR+dp.disp_w*dp.disp_h*3;        
                        }while(1);
                    }   
                    #else
    				DrawPowerOnImage(&dp, img, theimg.img_w, theimg.img_h, theimg.img_clrs, x, y);
