...
extern UINT8    AHC_GetBootingSrc(void);
extern void InitOSD(void);
extern void InitKeyPad(void);
extern void AHC_DCF_Init(void);
...
void HDMI_HPD_TimerCallBack(OS_TMR *ptmr, void *p_arg)
{
    ...
    //pHDMI->HDMI_PHY_INT_CPU_SR & HDMI_PLUGIN_INT)
    {
        if(m_bHDMIPlugIn == MMP_FALSE) {
            ...
            MMPS_AUDIO_InitializeI2SConfig(MMP_I2S_CH_DEFAULT, &m_I2Scfg);
            MMPS_AUDIO_EnableI2SMclk(MMP_I2S_CH_DEFAULT, MMP_TRUE, 44100, 0);
...
void CE_JOB_DISPATCH_Task(void *p_arg)
{
    ...
    MMP_USHORT uwFwBranchVersion;
    MMP_USHORT uwFwTestVersion;
    char**     pszReleaseName;

    AHC_GetFwVersion(&uwFwMajorVersion, &uwFwMediumVersion, &uwFwMinorVersion, &uwFwBranchVersion,  &uwFwTestVersion, pszReleaseName);
    
    RTNA_DBG_Str3("AIT Job Task!\r\n");
    RTNA_DBG_Str(0, "FirmwareCore Version:");
    RTNA_DBG_Short(0, uwFwMajorVersion);
    RTNA_DBG_Str(0, ".");
    RTNA_DBG_Short(0, uwFwMediumVersion);
    RTNA_DBG_Str(0, ".");
    RTNA_DBG_Short(0, uwFwMinorVersion);
    RTNA_DBG_Str(0, "\r\n");
	#if 0 // lyj 20181227
	MMPF_PIO_Enable(MMP_GPIO49, MMP_TRUE);
	//MMPF_PIO_PadConfig(MMP_GPIO51, PAD_OUT_DRIVING(0), MMP_TRUE);// lyj 20181227
	MMPF_PIO_EnableOutputMode(MMP_GPIO49, MMP_TRUE, MMP_FALSE);
	MMPF_PIO_SetData(MMP_GPIO49, 1, MMP_TRUE);

	MMPF_PIO_Enable(MMP_GPIO52, MMP_TRUE);
	//MMPF_PIO_PadConfig(MMP_GPIO52, PAD_OUT_DRIVING(0), MMP_TRUE);// lyj 20181227
	MMPF_PIO_EnableOutputMode(MMP_GPIO52, MMP_TRUE, MMP_FALSE);
	MMPF_PIO_SetData(MMP_GPIO52, 1, MMP_TRUE);

	#endif
	//ºóÀ­¾µÍ·
	MMPF_PIO_PadConfig(MMP_GPIO50, PAD_OUT_DRIVING(0), MMP_TRUE);
	MMPF_PIO_EnableOutputMode(MMP_GPIO50, MMP_TRUE, MMP_TRUE);
	MMPF_PIO_SetData(MMP_GPIO50, MMP_TRUE, MMP_TRUE);// lyj 20190226
	...
	MMPF_PIO_SetData(USB_EN_SELECT,0,MMP_TRUE);

	// BT×´Ì¬¼ì²â
//	MMPF_PIO_SetData(MMP_GPIO34, GPIO_HIGH, MMP_TRUE);
	//MMPF_PIO_EnableOutputMode(MMP_GPIO34, MMP_FALSE, MMP_TRUE); // BT×´Ì¬¼ì²â sd card
	...
	MMPF_PIO_EnableOutputMode(MMP_GPIO63, MMP_FALSE, MMP_TRUE);// BT status damp
	
	MMPF_PIO_EnableOutputMode(MMP_GPIO53, MMP_TRUE, MMP_TRUE);
	MMPF_PIO_SetData(MMP_GPIO53, MMP_TRUE, MMP_TRUE);// lyj 20181030 charge
	//MMPF_PIO_EnableOutputMode(MMP_GPIO52, MMP_FALSE, MMP_TRUE); // lyj 20181030
	//MMPF_PIO_EnableOutputMode(MMP_GPIO39, MMP_FALSE, MMP_TRUE); // lyj 20181026
	...
	light_Bar_fun(MMP_GPIO48,1);// lyj 20190112
	
	#if 1
	MMPF_PIO_PadConfig(MMP_GPIO69, PAD_OUT_DRIVING(0), MMP_TRUE);
	MMPF_PIO_EnableOutputMode(MMP_GPIO69, MMP_TRUE, MMP_TRUE);
	MMPF_PIO_PadConfig(MMP_GPIO68, PAD_OUT_DRIVING(0), MMP_TRUE);
	MMPF_PIO_EnableOutputMode(MMP_GPIO68, MMP_TRUE, MMP_TRUE);
	MMPF_PIO_PadConfig(MMP_GPIO67, PAD_OUT_DRIVING(0), MMP_TRUE);
	MMPF_PIO_EnableOutputMode(MMP_GPIO67, MMP_TRUE, MMP_TRUE); // lyj 20181114
...
*/
	{
 		//InitOSD(); //Move to CE_JOB_DISPATCH_Task for fast boot. // 9-13
		#ifndef	NO_PANEL
			// InitOSD will make LCD off, Shutdown LCD backlight must before,
			// Otherwise, Some kind of LCD will show ugly white lines
...
#if 0 // lyj 20190626
		{
			//	UINT8 bID0 ;// 20170927
				...
				AHC_OSDDrawBitmap(bID0, &bmwelogo1, 0, 0);// ¿ª»úlogo
    ...
    UartCommandInit();
#if WIFI_PORT == 1
    init_config_manager();//in case that other task uses NET CONF before initialization
#endif
	...
	//printc("~~~~~MMMMMMMMMMMMMMMMMM~~~\r\n");
	//RTNA_DBG_Str3("~~~~~c~~~AIT Job Task!\r\n");
    // Never run to here.
    while (1) {
        MMPF_OS_Sleep(1000);
		//printc("~~~~~MMMMMMMMMMMMMMMMMM222~~~\r\n");
		// RTNA_DBG_Str3("~~~~~d~~~AIT Job Task!\r\n");
    }
}
