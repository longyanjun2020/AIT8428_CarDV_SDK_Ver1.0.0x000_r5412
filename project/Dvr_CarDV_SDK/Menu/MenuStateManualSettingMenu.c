...
SMENUSTRUCT sSubGammaAdjust =
{
    ...
    sizeof(sMenuListSaturation)/sizeof(PSMENUITEM),
    sMenuListGamma,
    SubMenuEventHandler,// SubMenuEventHandler_Gamma
    MenuGetDefault_ManualSetting,
    0,
...
AHC_BOOL MenuItemExpoMetering(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
	...
	{
	#if (MENU_MANUAL_METERING_MULTI_EN)
		case ITEMID_METERING_MULTI:				AHC_SetAeMeteringMode(AHC_AE_METERING_MATRIX);		break;
	#endif
	#if (MENU_MANUAL_METERING_WEIGHTED_EN)
		case ITEMID_METERING_CENTER_WEIGHTED:	AHC_SetAeMeteringMode(AHC_AE_METERING_CENTER);		break;
	#endif
	#if (MENU_MANUAL_METERING_CENTER_SPOT_EN)
		case ITEMID_METERING_CENTER_SPOT:		AHC_SetAeMeteringMode(AHC_AE_METERING_SPOT);		break;
	#endif
		case ITEMID_METERING_TOUCH_SPOT:		printc("Not supported now. \n");					break;
		default:								AHC_SetAeMeteringMode(AHC_AE_METERING_MATRIX);  	break;
	}
...
AHC_BOOL MenuItemWB(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
#if 0//( MENU_MANUAL_EFFECT_EN)
   if ( MenuSettingConfig()->uiEffect != EFFECT_NORMAL)
   {
       printc("Try to set WB when menu_effect != EFFECT_NORMAL\r\n");
       return AHC_FALSE;
   }
#endif
...
AHC_BOOL MenuItemColor(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
   ...
   if ( MenuSettingConfig()->uiEffect != EFFECT_NORMAL)
   {
       printc("Try to set Saturation when menu_effect != EFFECT_NORMAL\r\n");
       return AHC_FALSE;
   }
#endif
...
AHC_BOOL MenuItemEffect(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
    ...
    if (MenuSettingConfig()->uiColor != COLOR_NATURAL)    {
        ...
        Menu_SetColor(COLOR_NATURAL);
        printc("Try to set effect mode when menu_color != COLOR_NATURAL\r\n");
        printc("Set menu_color = COLOR_NATURAL first\r\n");
    }
#endif
...
AHC_BOOL MenuItemContrastAdjust(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
   ...
   if ( MenuSettingConfig()->uiEffect != EFFECT_NORMAL)
   {
       printc("Try to set Contrast when menu_effect != EFFECT_NORMAL\r\n");
       return AHC_FALSE;
   }
#endif
    ...
    printc("New contrast = %d\r\n", s32Contrast);

    pf_General_EnSet(COMMON_KEY_CONTRAST, s32Contrast);
 //   Menu_SetContrast( s32Contrast);
...
AHC_BOOL MenuItemSaturationAdjust(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
   ...
   if ( MenuSettingConfig()->uiColor != COLOR_NATURAL)
   {
       printc("Try to set Saturation when menu_color != COLOR_NATURAL\r\n");
       return AHC_FALSE;
   }
#endif
#if ( MENU_MANUAL_EFFECT_EN)
   if ( MenuSettingConfig()->uiEffect != EFFECT_NORMAL)
   {
    ...
    Menu_SetSaturation( s32Saturation);
...
AHC_BOOL MenuItemSharpnessAdjust(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
   ...
   if ( MenuSettingConfig()->uiEffect != EFFECT_NORMAL)
   {
       printc("Try to set Sharpness when menu_effect != EFFECT_NORMAL\r\n");
       return AHC_FALSE;
   }
#endif
    ...
    Menu_SetSharpness( s32Sharpness);
...
AHC_BOOL MenuItemGammaAdjust(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
   ...
   if ( MenuSettingConfig()->uiEffect != EFFECT_NORMAL)
   {
       printc("Try to set Gamma when menu_effect != EFFECT_NORMAL\r\n");
       return AHC_FALSE;
   }
#endif
		...
		if(sub_flag == 0)
		{
			...
			if(mainVol >= 0 && mainVol <= 10 )
			{
				...
				if(mainVol == 0)
					...
					//uart_to_mcu_voice(MAINVOICE,(MMP_BYTE)mainVol,MUTE_OFF);
	...
	case MENU_RIGHT         :
		...
		else if(sub_flag == 1)
		{
			...
			if(subVol >= 0 && subVol <= 10)
			{
				subVol++;
				Draw_sub_vicon(subVol);
				//uart_to_mcu_voice(SUBVOICE1,(MMP_BYTE)subVol,MUTE_OFF);// add voice
				//uart_to_mcu_voice(SUBVOICE2,(MMP_BYTE)subVol,MUTE_OFF);// add voice
				volume_conver_size(SUBVOICE1,subVol);
				volume_conver_size(SUBVOICE2,subVol);
			}
			if(subVol >10)
