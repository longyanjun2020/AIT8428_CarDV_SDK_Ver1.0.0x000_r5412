...
UINT8 QuickFileOperation(UINT8 Op)
{
    ...
    if ((MMP_IsScdCamExist() || MMP_IsUSBCamExist()) &&
        (m_ubPlaybackRearCam == AHC_TRUE && (VIDRECD_MULTI_TRACK == 0 || (AHC_UF_GetCurrentDBFlag() & DCFDT_DB_FLAG_HAS_REAR_CAM)))){ 
...
//use only in format free 
//the max size is 256
AHC_BOOL CleanDataInHiddenFile(char* szFileName, UINT32 uiSize)
{
	UINT32             uiFileID;
    AHC_ERR            err;  
	...
	err = AHC_FS_FileOpen((INT8 *)szFileName, AHC_StrLen(szFileName), "a+b", AHC_StrLen("a+b"), &uiFileID);
	if (err != MMP_ERR_NONE){
	...
	if(err == AHC_ERR_NONE)
    {
		...
		//write
        AHC_FS_FileWrite(uiFileID, (UINT8*)Buf, uiSize, &uiWriteCount);
		//close
...
AHC_BOOL QuickMediaOperation(UINT8 Op)
{
	...
	{		
        ...
        if( ret == AHC_TRUE )
        {
			...
			char szSrcFullPath[DCF_MAX_FILE_NAME_SIZE] = DCFDT_FILEINDEX_RENAMESRC_FIlENAME;
			char szDstFullPath[DCF_MAX_FILE_NAME_SIZE] = DCFDT_FILEINDEX_INDEXSAVED_FIlENAME;
			//rename file
			if(DCFDT_DB0_FILENAME_NAMING_TYPE == DCFDT_FILENAME_NAMING_BYINDEX)
			{
				if(AHC_IsFileExist(szSrcFullPath))
				{
					//clean data in hidden file
					CleanDataInHiddenFile(szSrcFullPath, DCF_DB_COUNT * 4 * 2);//DCF_DB_COUNT * size of SFILE_INDEX_STRUCT 
        			AHC_FS_FileDirRename(szSrcFullPath, sizeof(szSrcFullPath), szDstFullPath, sizeof(szDstFullPath));
				}
				else
					printc("Rename src file does not exist %s\r\n", szSrcFullPath);
			}
			#endif
			//
