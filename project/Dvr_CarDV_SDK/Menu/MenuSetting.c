...
#define MENUSETTING_DEBUG 	(0) //lyj 20190114
...
#define REARCAM_MIRROR_KEY			COMMON_KEY_REARCAM_MIRROR
...
{  
    //Default as none.
    MenuSettingConfig()->uiMOVSize = MOVIE_SIZE_NUM;
    ...
    if((*w  <= 1280) && (*h  <= 720)){
        if (*fps == 60) {
			...
			printc("VR:720P@60FPS\r\n");
        } else 
        #if MENU_MOVIE_SIZE_720_24P_EN
        if (*fps == 24)
        {
            MenuSettingConfig()->uiMOVSize = MOVIE_SIZE_720_24P;
            printc("VR:720P@24FPS\r\n");
        } else
        #endif
        {
            MenuSettingConfig()->uiMOVSize = MOVIE_SIZE_720P;
            *fps = 30;
			printc("VR:720P@30FPS\r\n");
        }
    }
    else if((*w <= 1600) && (*h <= 912)){ //912p
#if (MENU_MOVIE_SIZE_900P_30P_EN)
        if (*fps == 30) {
            MenuSettingConfig()->uiMOVSize = MOVIE_SIZE_900P_30P;
            printc("VR:900P@30FPS\r\n");
        }
#endif        
    }
    else if((*w <= 1280) && (*h <= 960)){ //960p
#if (MENU_MOVIE_SIZE_960P_30P_EN)
        if (*fps == 30) {
            MenuSettingConfig()->uiMOVSize = MOVIE_SIZE_960P_30P;
            printc("VR:960P@30FPS\r\n");
        }
#endif        
    }
        ...
        if (*fps == 60) {
			...
			printc("VR:1080P@60FPS\r\n");
        } else
		#endif
        #if MENU_MOVIE_SIZE_1080_24P_EN
        if (*fps == 24) {
            MenuSettingConfig()->uiMOVSize = MOVIE_SIZE_1080_24P;
            printc("VR:1080P@24FPS\r\n");
        } else
        #endif
        {   
        	#ifdef MENU_MOVIE_SIZE_1080P_30_HDR_EN
        	#if MENU_MOVIE_SIZE_1080P_30_HDR_EN
        	if(MenuSettingConfig()->uiHDR == COMMON_HDR_EN_ON)
        	{
        		MenuSettingConfig()->uiMOVSize = MOVIE_SIZE_1080_30P; //MOVIE_SIZE_1080_30P_HDR
        	}else
        	#endif
        	#endif
        	{
        ...
        *h = 1080;
        *fps = 30;
		printc("VR:Default set to 1080P@30FPS\r\n");

        return AHC_FALSE;
    }

    //No match case.
    if(MOVIE_SIZE_NUM == MenuSettingConfig()->uiMOVSize){
...
AHC_BOOL Menu_SetMovieMode(UINT8 val)
{
    switch(val)
    {
	    ...
	    case MOVIE_SIZE_4K_24P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_3200x1808);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 24);
	    break;
	    #endif    
        #if(MENU_MOVIE_SIZE_1440_30P_EN)
	    case MOVIE_SIZE_1440_30P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_2560x1440);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 30);
	    break;
	    #endif		
        #if(MENU_MOVIE_SIZE_SHD_30P_EN)
	    case MOVIE_SIZE_SHD_30P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_2304x1296);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 30);
	    break;
	    #endif
	    ...
	    case MOVIE_SIZE_SHD_25P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_2304x1296);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 25);
	    break;
        #endif
	    ...
	    case MOVIE_SIZE_1080_60P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_1920x1088 /*VIDRECD_RESOL_1920x1080*/);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 60);
	    break;
	    #endif
        #if(MENU_MOVIE_SIZE_1080_24P_EN)
        ...
        case MOVIE_SIZE_900P_30P:
            AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_1600x900);
            AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 30);
            break;    
        #endif        
        #if (MENU_MOVIE_SIZE_960P_30P_EN)
        case MOVIE_SIZE_960P_30P:
            AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_1280x960);
            AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 30);
            break;    
        #endif        
	    #if (MENU_MOVIE_SIZE_720P_EN)
	    case MOVIE_SIZE_720P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_1280x720);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 30);
	    break;
	    #endif
	    ...
	    case MOVIE_SIZE_VGA30P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_640x480);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 30);
	    break;
        #endif
	    ...
	    case MOVIE_SIZE_VGA120P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_640x480);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 120);
	    break;
        #endif
        #if (MENU_MOVIE_SIZE_VGA240P_EN)
	    case MOVIE_SIZE_VGA_240P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_640x480);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 240);
	    break;
        #endif
	    case MOVIE_SIZE_360_30P:
	        AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_RESOLUTION, AHC_VIDRECD_RESOL_640x360);
	        AHC_VIDEO_SetMovieConfig(0, AHC_FRAME_RATE, 30);
	    break;
    }
    return AHC_TRUE;
...
{
    AHC_VIDEO_SetMovieConfig(0, AHC_VIDEO_COMPRESSION_RATIO, val);
    return AHC_TRUE;
}
...
AHC_BOOL Menu_WriteSetting(void)// menu_ok键按下进入
{
	extern const char txtSetFile[];

	printc(FG_GREEN("Menu_WriteSetting\r\n"));
	//MenuSettingConfig()->uiSpeedRGB = SelfAllFlag.Speedmode;
	//MenuSettingConfig()->uiRGBmode = SelfAllFlag.onceFlag; // lyj 20191021
	
#if 0 // lyj 20190215
	{

	   printc("~~~MenuSettingConfig()->uiMOVQuality = %d~~~~~\r\n",MenuSettingConfig()->uiMOVQuality);
	   printc("~~~MenuSettingConfig()->uiVMDRecTime  = %d~~~~~\r\n",MenuSettingConfig()->uiVMDRecTime );
	   printc("~~~MenuSettingConfig()->uiEV  = %d~~~~~\r\n",MenuSettingConfig()->uiEV);
	   printc("~~~MenuSettingConfig()->uiSpeedStamp  = %d~~~~~\r\n",MenuSettingConfig()->uiSpeedStamp);
	   printc("~~~MenuSettingConfig()->uiEffect  = %d~~~~~\r\n",MenuSettingConfig()->uiEffect);
		 printc("~~~MenuSettingConfig()->uiBatteryVoltage  = %d~~~~~\r\n",MenuSettingConfig()->uiBatteryVoltage);
		  printc("~~~MenuSettingConfig()->uiVolume  = %d~~~~~\r\n",MenuSettingConfig()->uiVolume);
		  	  printc("~~~MenuSettingConfig()->uiMOVPowerOffTime  = %d~~~~~\r\n",MenuSettingConfig()->uiMOVPowerOffTime);
			   printc("~~~MenuSettingConfig()->uiIMGSize  = %d~~~~~\r\n",MenuSettingConfig()->uiIMGSize);
			   printc("~~~MenuSettingConfig()->uiBeep  = %d~~~~~\r\n",MenuSettingConfig()->uiBeep);
			    printc("~~~MenuSettingConfig()->uiMOVClipTime  = %d~~~~~\r\n",MenuSettingConfig()->uiMOVClipTime);
...
void MenuSettingInit(void)
{
    ...
    gCurMenuInfo.uiLCDBrightness    	= 6;
    ...
    gCurMenuInfo.uiHDMIOutput       	= /*HDMI_OUTPUT_1080P*/HDMI_OUTPUT_720P;
    ...
    gCurMenuInfo.uiBatteryVoltage   	= /*BATTERY_VOLTAGE_FULL*/BATTERY_VOLTAGE_LEVEL_2;
	...
	//============================lyj==============================
	//gCurMenuInfo.Volume1				= 5;
	//gCurMenuInfo.Volume2				= 5;
}
...
void DefaultMenuSettingInit(void)
{
    ...
    gDefaultMenuInfo.uiLCDBrightness    	= 6;//lyj
...
#else
    gDefaultMenuInfo.uiAutoPowerOff     	= /*AUTO_POWER_OFF_NEVER*/AUTO_POWER_OFF_1MIN;
#endif
    ...
    gDefaultMenuInfo.uiHDMIOutput       	= /*HDMI_OUTPUT_1080P*/HDMI_OUTPUT_720P;
	...
	gDefaultMenuInfo.uiGsensorSensitivity 	= /*GSENSOR_SENSITIVITY_OFF*/GSENSOR_SENSITIVITY_L2;
	gDefaultMenuInfo.uiMotionDtcSensitivity = MOTION_DTC_SENSITIVITY_OFF;
    gDefaultMenuInfo.uiTimeZone         	= TIMEZONE_GMT_P_8;
    gDefaultMenuInfo.uiBatteryVoltage   	= /*BATTERY_VOLTAGE_FULL*/BATTERY_VOLTAGE_LEVEL_2;
    gDefaultMenuInfo.uiBootUpMode   		= BOOT_UP_MODE_DVR;
    gDefaultMenuInfo.uiWNR   		        = WNR_DEFAULT;
    gDefaultMenuInfo.uiNightMode   		    = MOVIE_NIGHT_MODE_DEFAULT;

	//=================lyj==========================================
	//gDefaultMenuInfo.Volume1				= 5;
	//gDefaultMenuInfo.Volume2				= 5;
}
...
//Reste TimeZone
{
#if 0//(GPS_CONNECT_ENABLE) //lyj 201190114
...
AHC_BOOL CheckMenuSetting(MenuInfo* CurMenu)
{
	...
	MenuSettingInRange(&(CurMenu->uiMOVQuality), 		0, 	/*QUALITY_NUM-1*/4			);
	...
	MenuSettingInRange(&(CurMenu->uiMOVClipTime), 		0, 	MOVIE_CLIP_TIME_NUM -1);//lyj
	// Not to check the range for MENU_TXT, MenuSettingInRange(&(CurMenu->uiMOVClipTime), 		0, 	MOVIE_CLIP_TIME_NUM);
	MenuSettingInRange(&(CurMenu->uiMOVPowerOffTime), 	0,  16/*MOVIE_POWEROFF_TIME_NUM-1*/);
	...
	MenuSettingInRange(&(CurMenu->uiEV), 				0, 	16/*EVVALUE_NUM-1*/			);
	...
	MenuSettingInRange(&(CurMenu->uiEffect), 			0, 	16			);
	...
	MenuSettingInRange(&(CurMenu->uiVolume), 			0, 	16/*VOLUME_NUM-1*/			);
	...
	//General
	MenuSettingInRange(&(CurMenu->uiBeep), 				0, 	4			);
	MenuSettingInRange(&(CurMenu->uiLCDBrightness), 	0, 	50	);
	...
	MenuSettingInRange(&(CurMenu->uiSpeedStamp), 		0, 	16		);
	...
	MenuSettingInRange(&(CurMenu->uiBatteryVoltage), 	0, 	11	);
    ...
    MenuSettingInRange(&(CurMenu->uiNightMode),		    0, 	MOVIE_NIGHT_MODE_NUM-1		);
...
static MENU_ATOMS  menu_atoms =
{
    ...
    NUMBER_SETTING,//菜单ID
    {
        ...
        {/* 009 */COMMON_KEY_MOVIE_QUALITY,0, MENU_MOVIE_QUALITY_EN,           0, 4,                 ParamFun, NULL,     "0:Super fine, 1:fine"},
        ...
        {/* 014 */COMMON_KEY_VR_CLIP_TIME, 0, MENU_MOVIE_CLIP_TIME_EN,  0, MOVIE_CLIP_TIME_NUM - 1,         ParamFun, Menu_Setting_Get_VR_ClipTime,     "Video clip time, 0: Off loop record, 1:1min, 2:3min 3:5min, 4:10min"},
        {/* 015 */COMMON_KEY_POWER_OFF_DELAY, 0, MENU_MOVIE_POWER_OFF_DELAY_EN,   0, 16/*MOVIE_POWEROFF_TIME_NUM - 1*/,     ParamFun, NULL,     "0: 0sec 1:5sec, 2:1min, 3:5min, 4:30min, 5:60min"},
        ...
        {/* 021 */COMMON_KEY_EV,    6, MENU_MANUAL_EV_EN,               0, 16/*EVVALUE_NUM - 1*/,                 ParamFun, Menu_Setting_Get_EV,     "0:-2.0, 1:-1.66, 2:-1.33, 3:-1.00, 4:-0.66, 5:-0.33, 6:0, 7:+0.33, 8:+0.66, 9:+1.00, 10:+1.33, 11:+1,66, 12:+2.0"},
        ...
        {/* 025 */"Effect",         0, MENU_MANUAL_EFFECT_EN,           0, 16,                  ParamFun, NULL,     "Effect:0:Normal"},
        ...
        {/* 033 */"PlaybackVolume",     4, MENU_PLAYBACK_VOLUME_EN,             0, 16/*VOLUME_NUM - 1*/,          ParamFun, NULL,     "Volume:0~9, 0 is Min, 9 is Max"},
        // Edit Tool
        ...
        // General
        {/* 037 */COMMON_KEY_BEEP,  0, MENU_GENERAL_BEEP_EN,            0, 4,                    ParamFun, NULL,     "KeyBeep:0:On, 1:Off"},
        {/* 038 */"LCDBrightness",  0, 1,  0, 50,          ParamFun, NULL,     szReserved},
        ...
        {/* 047 */"BatteryVoltage", 0, 1,                            0, /*BATTERY_VOLTAGE_NUM*/11,         ParamFun, NULL,     szReserved},
        ...
        {/* 054 */"TimeZone",       0, MENU_GENERAL_TIME_ZONE_EN,      0, TIMEZONE_GMT_NUM - 1,            ParamFun, NULL,     szReserved},//lyj 13----0
        ...
        {/* 063 */"SpeedStamp",     0, MENU_GENERAL_SPEED_STAMP_EN,     0, 16,             ParamFun, NULL,     "0:On, 1:Off"},
        ...
        {/* 066 */"PowerOnFlag",			0, 1,								0,255,								ParamFun, NULL, 	szReserved},// lyj 20190606
        {/* 066 */"uiRGBstatus",			0, 1,								0,255,								ParamFun, NULL, 	szReserved},// lyj 201901021
         {/* 066 */"uiBiglangStauts",			0, 1,								0,255,								ParamFun, NULL, 	szReserved},// lyj 201901021
           {/* 066 */"uiSpeedRGB",			0, 1,								0,255,								ParamFun, NULL, 	szReserved},// lyj 201901021
         {/* 066 */"uiRGBmode",			0, 1,								0,255,								ParamFun, NULL, 	szReserved},// lyj 201901021
        ...
        {/* 088 */NIGHT_MODE_EN_KEY,0, MENU_MOVIE_NIGHT_MODE_EN,        0, 1,                                               pf_General, Menu_Setting_Get_General,   "0:On, 1:Off"},
		{/* 089 */REARCAM_MIRROR_KEY,  0, MENU_GENERAL_REARCAM_MIRROR_EN,		0, REARCAM_MIRROR_NUM - 1,				pf_General, Menu_Setting_Get_General, 	szReserved},
        	  {/* 090 */COMMON_KEY_B_VOL,       0, 1,               0, 4096,   pf_General, Menu_Setting_Get_General,  szReserved},// lyj
...
AHC_BOOL Menu_Setting_CheckMenuAtoms(void)
{
    ...
    if (gbAhcDbgBrk) {
        RTNA_DBG_Str0(FG_RED("$MenuChanged\r\n"));
    }        
    else {
        RTNA_DBG_Str0(FG_RED("Menu setting is changed\r\n"));      
    }
    return AHC_FALSE;
}
...
int pf_RearCamMirror_EnSet(int value)
{
	UINT8 i;

	for (i = 0; menu_atoms.sAtom[i].szSKey != NULL; i++)
	{
		if (strcmpi(REARCAM_MIRROR_KEY, menu_atoms.sAtom[i].szSKey) == 0)
		{
			if (IN_RANGE(value, &menu_atoms.sAtom[i]))
			{
				menu_atoms.sAtom[i].nSVal = value;
        ...
        case MENU_SETTING_TYPE_CAPTURE_SETTING:
            switch( byOption )
            {
                ...
                case CAPTURE_SETTING_STATE_WHITE_BALANCE:
                    ...
                    AHC_SetAwbMode( MenuSettingConfig()->uiWB );
                    break;
                ...
                case CAPTURE_SETTING_STATE_HDR:
                    ...
                    {
                        AHC_SetAeWDRMode( AHC_AE_WDR_ENB );
                    }
                    else
                    {
                        AHC_SetAeWDRMode( AHC_AE_WDR_DSB );
                    }
                    break;
            }
            break;
