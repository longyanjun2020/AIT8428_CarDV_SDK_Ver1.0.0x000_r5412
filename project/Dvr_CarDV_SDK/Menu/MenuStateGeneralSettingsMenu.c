...
AHC_BOOL MenuItemRearCamMirror(PSMENUITEM pItem, AHC_BOOL bIsHover);
...
SMENUITEM sItemBeep_AM1     		= { ITEMID_AM1, &BMICON_OPTION_CLOSE, IDS_DS_AM1,NULL, MenuItemBeep, 0, AHC_TRUE, NULL };
SMENUITEM sItemBeep_AM2     		= { ITEMID_AM2, &BMICON_OPTION_CLOSE, IDS_DS_AM2,NULL, MenuItemBeep, 0, AHC_TRUE, NULL };
#endif
#endif
...
SMENUITEM sItemStampOFF				= { ITEMID_STAMP_OFF,       NULL, IDS_DS_STAMP_OFF, 		NULL, MenuItemDateLogoStamp, 0, AHC_TRUE, NULL };
#endif
#endif
...
SMENUITEM sItemFlickerHz          	= { ITEMID_FLICKER, 		&BMICON_MENU_FLICK,   IDS_FLICKER_HZ,	&sSubFlickerHz, 		0, 		0, AHC_TRUE, NULL };
...
SMENUITEM sItemRearCamMirror            = { ITEMID_REARCAM_MIRROR, 		&BMICON_MENU_FLIP,    IDS_DS_MIRROR,	&sSubRearCamMirror, 	0, 		0, AHC_TRUE, NULL };
#if (MENU_GENERAL_REARCAM_MIRROR_ON_EN)
SMENUITEM sItemRearCamMirror_On         = { ITEMID_REARCAM_MIRROR_ON, 	&BMICON_OPTION_OPEN,  IDS_DS_ON,	NULL, MenuItemRearCamMirror, 	0, AHC_TRUE, NULL };
#endif
#if (MENU_GENERAL_REARCAM_MIRROR_OFF_EN)
SMENUITEM sItemRearCamMirror_Off    	= { ITEMID_REARCAM_MIRROR_OFF,  &BMICON_OPTION_CLOSE, IDS_DS_OFF,  	NULL, MenuItemRearCamMirror, 	0, AHC_TRUE, NULL };
#endif
#endif
...
#endif
#if (MENU_GENERAL_REARCAM_MIRROR_EN)
PSMENUITEM   sMenuListRearCamMirror[] =
{
#if (MENU_GENERAL_LCD_ROTATE_ON_EN)
    &sItemRearCamMirror_On,
#endif
#if (MENU_GENERAL_LCD_ROTATE_OFF_EN)
    &sItemRearCamMirror_Off,
#endif
};
#endif
...
SMENUSTRUCT sSubRearCamMirror =
{
    MENUID_SUB_MENU_REARCAM_MIRROR,
    NULL,
    IDS_DS_MIRROR,
    NULL,
    sizeof(sMenuListRearCamMirror)/sizeof(PSMENUITEM),
    sMenuListRearCamMirror,
    SubMenuEventHandler,
    MenuGetDefault_GeneralSetting,
    0,
    IDS_DS_MIRROR,
...
{
	int id;
	if((pItem->iItemId - ITEMID_REARCAM_MIRROR-1) == REARCAM_MIRROR_ON)
	{
		bMirrorEnable = TRUE;
		id = REARCAM_MIRROR_ON;
	}
	else if((pItem->iItemId - ITEMID_REARCAM_MIRROR-1) == REARCAM_MIRROR_OFF)
	{
		bMirrorEnable = FALSE;
		id = REARCAM_MIRROR_OFF;
	}
	
	pf_RearCamMirror_EnSet(id);
...
AHC_BOOL MenuItemTimeZone(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
#if 0//(GPS_CONNECT_ENABLE) lyj 20190114
...
UINT32 MenuGetDefault_GeneralSetting(PSMENUSTRUCT pMenu)
{
    ...
    switch( pMenu->iMenuId )
    {
...
#endif
#if (MENU_GENERAL_REARCAM_MIRROR_EN)
      	case MENUID_SUB_MENU_REARCAM_MIRROR:
        	{
			INT32 s32Setting = 0;
			pf_General_EnGet(COMMON_KEY_REARCAM_MIRROR, &s32Setting);
			DefaultValue = (UINT32) s32Setting;
			if(DefaultValue == REARCAM_MIRROR_ON)
				bMirrorEnable = TRUE;
			else if(DefaultValue == REARCAM_MIRROR_OFF)
				bMirrorEnable = FALSE;
        	}
        	break;
#endif
    	...
    	case MENUID_SUB_MENU_MOTION_DTC_SENSITIVITY:
        	...
        	break;
#endif
#if 0//(MENU_GENERAL_TIME_ZONE_EN)
...
void MenuItemDateTimeFormatToAhcTimeStampSetting(void)
{
    ...
    AHC_GetCaptureConfig(ACC_DATE_STAMP, &m_TimeStampConfig);
    ...
    AHC_ConfigCapture(ACC_DATE_STAMP, m_TimeStampConfig);
}
