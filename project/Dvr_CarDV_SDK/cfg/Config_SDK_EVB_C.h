 ...
 * PCBA config file
 */
#ifndef	__CONFIG_SDK_SDK_EVB_C__
#define	__CONFIG_SDK_SDK_EVB_C__
...
//Use below defintions to avoid unstable images when enter Preview mode
#define DISP_PREVIEW_DELAY_COUNT_PRM_SENSOR    5  //Delay 5 frames
#define DISP_PREVIEW_DELAY_COUNT_SCD_SENSOR    5  //Delay 5 frames
...
#define BIND_SENSOR_AR0330_OTPM 		(1)// 2
...
#define BIND_SENSOR_IMX326 				(0)
#define BIND_SENSOR_OV2710              (0)
#define BIND_SENSOR_OV2710_MIPI         (0)
#define BIND_SENSOR_OV2718              (0)
#define BIND_SENSOR_OV2718_2A           (0)
...
#define BIND_SENSOR_BRV0200             (0)
#define BIND_SENSOR_BRV0500             (0)
...
#define BIND_SENSOR_GM7150              (1)// 1

// AHD
#define BIND_SENSOR_NVP6124B            (0)
#define BIND_SENSOR_RN6750              (0)
...
//#define SNR_OV4689_SID_PIN              (1)   // default is 0

//#define SENSOR_GPIO_ENABLE              (MMP_GPIO_MAX)
//#define SENSOR_GPIO_ENABLE_ACT_LEVEL    (PIO_HIGH)

// For Primary Sensor
/* For REAR_CAM_TV_DECODER/REAR_CAM_YUV_SENSOR/REAR_CAM_BAYER_SENSOR, PrmSensor Only support 2 lane */
...
#define PRM_SENSOR_VIF_ID (MMPF_VIF_MDL_ID0)//(MMPF_VIF_MDL_ID0)
 ...
 */
 /* 6-Level Setting */
#define	GSNR_SENS_5LEVEL0			GR(3.0)
#define	GSNR_SENS_5LEVEL1			GR(2.5)
#define	GSNR_SENS_5LEVEL2			GR(2.0)
#define	GSNR_SENS_5LEVEL3			GR(1.5)
#define	GSNR_SENS_5LEVEL4			GR(1.2)
#define	GSNR_SENS_5LEVEL5			GR(0.5)
...
#define POWER_ON_BY_GSENSOR_EN      (SUPPORT_GSENSOR & 0)
...
//  #define GSENSOR_SLAVE_ADDR      (0x28)          // LIS3DE, 7-bits mode
    #define	GSNR_GROUND_AXIS        (X_AXIS)
...
#define DSC_MODE_ENABLE				(0)

/* Zoom Setting */
#define DSC_DIGIT_ZOOM_ENABLE		(DSC_MODE_ENABLE & 1) 
#define DSC_DIGIT_ZOOM_RATIO_MAX	(4)

#define DSC_SELFSHOT_EN				(0)

#define DSC_BURSTSHOT_EN			(0)
...
#define VR_VIDEO_TYPE_SETTING       (COMMON_VR_VIDEO_TYPE_SETTING_H264ENC_BASELINE_PROFILE)
#if (DUALENC_SUPPORT) //Note: dual encode must be frame mode.
#define VR_VIDEO_CURBUF_ENCODE_MODE (COMMON_VR_VIDEO_CURBUF_FRAME)
#else
#define VR_VIDEO_CURBUF_ENCODE_MODE (COMMON_VR_VIDEO_CURBUF_RT)
#endif

#else   // MCR_V2_32MB

#define VR_VIDEO_TYPE_SETTING       (COMMON_VR_VIDEO_TYPE_SETTING_H264ENC_BASELINE_PROFILE)		// HDR

//Note: dual encode or WIFI H264 must be frame mode.
...
#if (VR_VIDEO_TYPE_SETTING == COMMON_VR_VIDEO_TYPE_SETTING_H264ENC_HIGH_PROFILE) && (VR_VIDEO_CURBUF_ENCODE_MODE == COMMON_VR_VIDEO_CURBUF_RT)
#error "AIT does not support H264 high profile with realtime encode!"
#endif
...
#define AHC_MP3_AUDIO_OPTION        MMPS_3GPRECD_AUDIO_MP3_32K_128K
#define AHC_ADPCM_AUDIO_OPTION      MMPS_3GPRECD_AUDIO_ADPCM_32K_22K
#define AHC_AMR_AUDIO_OPTION        MMPS_3GPRECD_AUDIO_AMR_12d2K
...
#define AHC_EMERGENTRECD_SUPPORT 	(0)
...
#define AUDIO_REC_PLAY_EN				(0)// 0   20180226
...
#define MENU_BLUETOOTH_PROGRESS_BAR         (1)
...
#define DEVICE_GPIO_SD_PLUG				(MMP_GPIO38)	// CGPIO6 --> SD_PR // lyj 20181026 MMP_GPIO38
...
#define SD_FW_REMOVE_SD_UPDATE_FILE		(0)// lyj 20191204

// SD PAD
//#define CFG_SD_PIN_PAD_NUM				(SD_SOCKET0_MAP_ID)  ///SD3_PIN_PAD_NUM
//#define CFG_SD1_PIN_PAD_NUM				(SD_SOCKET1_MAP_ID)

#define ENABLE_POWER_OFF_PICTURE		(0) // lyj 20190711 去掉开机logo
...
#define DIR_KEY_CYCLIC_LPRESS_EN	(0)
...
//For FM
//#define FM_PATH_SELECT_GPIO              (MMP_GPIO61)//long 4-14
...
#define WAIT_CHECK_BDEVICE_CONN_INTERVAL    (100)
#define WAIT_CHECK_BDEVICE_DISCONN_DELAY_CNT    (8)

#define USB_IN_DETECT_VBUS_PIN              (MMP_GPIO_MAX)//B15 MMP_GPIO25 lyj 20181029
#define USB_IN_DETECT_VBUS_PIN_EX		(MMP_GPIO25) // lyj 20181029
#define USB_IN_DETECT_VBUS_PIN_ACTIVE       (GPIO_LOW)
#define USB_DETECTION_CUSTOM (0) //TBD
#define USB_OUT_PWR_SW_PIN                  (MMP_GPIO_MAX)//B8 MMP_GPIO18 lyj
...
#define HDMI_ENABLE					(0) // lyj 20190929 
...
#define HDMI_PC_GPIO				(MMP_GPIO_MAX) // BGPIO13 -->> HDMI_PC
...
#define TV_OUT_DETECT_GPIO          (MMP_GPIO26)
...
#define	MVD_HI_MVTH					(100)//(90) // sensitivity
#define	MVD_HI_CNTH					(1)//(5) // size_perct_thd_min
...
#define POWER_EN_GPIO				(MMP_GPIO41) // CGPIO9 -->> PWR_ENB
...
#define LED_GPIO_POWER							(MMP_GPIO_MAX)//(MMP_GPIO3) // AGPIO1 -->> PWR_LED  long
...
#define USE_PWM_FOR_LCD_BACKLIGHT           	1
...
#define TOUCH_PANEL_INT_GPIO                    (MMP_GPIO_MAX)
#endif
...
#define KEY_BOARD_IS	 KEY_LCD_SENSOR_V18
/* 
...
*/
#if (KEY_BOARD_IS == KEY_LCD_SENSOR_V18)
#define ADC_KEY_EN					(1)
...
#define ADC_KEY_VOLT_L              (559)// 上一曲
#define ADC_KEY_VOLT_R              (482)// 下一曲

#define ADC_KEY_VOLT_ENTER			(213)// OK
#if (MORE_ADC_KEY==1)
#define ADC_KEY_VOLT_REC  			(310)// AM/FM  /// 181
#define ADC_KEY_VOLT_MENU   		(ADC_KEY_VOLT_DEFAULT)  	
#define ADC_KEY_VOLT_PLAY   		(ADC_KEY_VOLT_DEFAULT) 
#define ADC_KEY_VOLT_MODE   		(628)// USB/BT
...
#define ADC_KEY_VOLT_MUTE           (118)// VOL
#define ADC_KEY_VOLT_CAPTURE        (390)// RGB
#endif

#define ADC_STATE_ID_U  			(ADCPRESS_NONE) 
#define ADC_STATE_ID_D   			(ADCPRESS_NONE)  	
#define ADC_STATE_ID_L   			(ADCPRESS_L) 
#define ADC_STATE_ID_R   			(ADCPRESS_R) 
#define ADC_STATE_ID_ENTER			(ADCPRESS_ENTER)
#if (MORE_ADC_KEY==1)
#define ADC_STATE_ID_REC  			(ADCPRESS_REC) //(ADCPRESS_REC) 
#define ADC_STATE_ID_MENU   		(ADCPRESS_NONE)	
...
#define ADC_STATE_ID_CAPTURE        (ADCPRESS_CAPTURE)
#endif

#define ADC_KEY_VOLT_MARGIN         (23)// 32
#define ADC_KEY_VOLT_MARGIN_REC     (23)// 32
...
#define USB_EN_SELECT                           (MMP_GPIO42)//long 4-20 MMP_GPIO33
//---------------------------
#define ENCODE_GPIO_ISR         (1)// 编码器
#if(ENCODE_GPIO_ISR)
#define ENCODE_GPIO_RIGHT                   (MMP_GPIO64)// A
#define ENCODE_GPIO_LEFT                      (MMP_GPIO65)// b
#define ENCODE_BUTTON_RIGHT                      (/*BUTTON_DOWN_PRESS*/BUTTON_DOWN_REL)
#define ENCODE_BUTTON_LEFT                      (/*BUTTON_UP_PRESS*/BUTTON_UP_REL)
#endif
//------------------------no used---
#else

#define ADC_KEY_EN					(0)
#define MORE_ADC_KEY				(0)
#define ADC_KEY_SARADC_CHANNEL      (1)
#define ADC_KEY_BOARD_TYPE          (1) //[0/1:V16/V17]

#define ADC_KEY_VOLT_U              (1019)
#define ADC_KEY_VOLT_D              (436)
...
#if (MORE_ADC_KEY==1)
#define ADC_KEY_VOLT_REC  			(879)
#define ADC_KEY_VOLT_MENU   		(585)  	
#define ADC_KEY_VOLT_PLAY   		(ADC_KEY_VOLT_DEFAULT) 
#define ADC_KEY_VOLT_MODE   		(726)
#define ADC_KEY_VOLT_FUNC1			(ADC_KEY_VOLT_DEFAULT)
#define ADC_KEY_VOLT_FUNC2			(ADC_KEY_VOLT_DEFAULT)
#define ADC_KEY_VOLT_FUNC3			(ADC_KEY_VOLT_DEFAULT)
#define ADC_KEY_VOLT_WIDE           (ADC_KEY_VOLT_DEFAULT)
...
#define ADC_STATE_ID_SET			(ADCPRESS_NONE)
#define ADC_STATE_ID_SOS			(ADCPRESS_NONE)
#define ADC_STATE_ID_MUTE           (ADCPRESS_NONE)
...
#define KEY_GPIO_REC 				(MMP_GPIO84) 	// MIPI_RX1_CKP 	-->> OK_KEY
#define KEY_GPIO_MENU 				(MMP_GPIO_MAX) 	// PI2S_SD0 	-->> MENU_KEY
#define KEY_GPIO_PLAY 				(MMP_GPIO_MAX)
#define KEY_GPIO_MODE 				(MMP_GPIO81) 	// MIPI_RX1_DA2P 	-->> B_KKEY
#define KEY_GPIO_POWER 				(MMP_GPIO40)//(MMP_GPIO_MAX) 	// CGPIO8 			---> PWR_KEY_IN
#define KEY_GPIO_CIRCLE 			(MMP_GPIO_MAX)
#if (MORE_GPIO_KEY == 1)
#define KEY_GPIO_UP 				(MMP_GPIO86) 	// MIPI_RX1_DA1P 	-->> UP_KEY
#define KEY_GPIO_DOWN 				(MMP_GPIO85) 	// MIPI_RX1_DA1N 	-->> DOWN_KEY
#define KEY_GPIO_LEFT 				(MMP_GPIO_MAX)
#define KEY_GPIO_RIGHT 				(MMP_GPIO_MAX)
#define KEY_GPIO_OK 				(MMP_GPIO_MAX)
#define KEY_GPIO_FUNC1 				(MMP_GPIO_MAX)
#define KEY_GPIO_FUNC2 				(MMP_GPIO_MAX)
#define KEY_GPIO_FUNC3 				(MMP_GPIO_MAX)
#define KEY_GPIO_SOS 				(MMP_GPIO_MAX)
#define KEY_GPIO_MUTE 				(MMP_GPIO82) 	// MIPI_RX1_DA2N 	-->> A_KEY
#define KEY_GPIO_CAPTURE 			(MMP_GPIO_MAX)
#endif
#endif	//KEY_BOARD_IS
...
#define BATTERY_DETECT_ADC_CHANNEL      (0)     // 0: SARADC0, ...
...
//-----------------------------
#define MENU_OFF   (0)
#define MENU_MAIN_FLAG		(1)
#define BLUETOOH_FLAG		(2)
#define RADIO_FM_FLAG		(3)
#define RADIO_AM_FLAG		(4)
#define VOLUME_FLAG 			(5)
//#define RADIO_WB_FLAG 		(6)  lyj 20180515
#define LIGHT_BAR_FLAG 		(7)
#define AUX_FLAG 			(8)
#define USB_FLAG                       (9)
#define RGB_FLAG 			(10)
#define BRIGHTNESS_FLAG 	(11)
#define PREVIEW_FLAG		(12)


//===============lyj=========
#define volume_front		(0)
#define volume_rear			(1)
#define volume_output		(2)


//================================blue
#define RGB_DATA_EN_DEBUG	(1)
...
#define ENABLE_DETECT_FLOW			(1)//lyj 0
...
#define TASK_AHC_WORK_STK_SIZE       (512)//(2048)
#define TASK_AHC_WORK_STK_SIZE       (512)
...
//Wifi
#define TASK_STREAMING_STK_SIZE      (2048)
 ...
 *  ADAS setting
 *===========================================================================*/
#define FRONT_MOTION_DETECTION_EN	(SUPPORT_MDTC & 1)
#define	REAR_MOTION_DETECTION_EN 	(SUPPORT_MDTC & 0)
#define MOTION_DETECTION_EN			(FRONT_MOTION_DETECTION_EN | REAR_MOTION_DETECTION_EN)

#define MDT_REC_ALWAYS_BACKLIGHT_OFF (SUPPORT_MDTC & 1)

#define FRONT_ADAS_EN	(SUPPORT_ADAS & 1)
#define	REAR_ADAS_EN 	(SUPPORT_ADAS & 0)
#define ADAS_EN			(FRONT_ADAS_EN | REAR_ADAS_EN)

#define ENABLE_ADAS_LDWS  (SUPPORT_ADAS_LDWS & 0)
#define ENABLE_ADAS_FCWS  (SUPPORT_ADAS_FCWS & 0)
#define ENABLE_ADAS_SAG   (SUPPORT_ADAS_SAG  & 0)

#define RTC_CHECK						(0)
#define RTC_MAX_INTERVAL			    (100)
#endif // __CONFIG_SDK_SDK_EVB_C__
