...
{
#if ( defined(SD_BOOT) || defined(MBOOT_SRAM_FW) )
//In order to use FS in bootloader add defined(MBOOT_SRAM_FW) 
...
void RTNA_LCD_Backlight(MMP_BOOL bEnable)// 通过PWM输出调屏幕亮度   liao
{
#if (USE_PWM_FOR_LCD_BACKLIGHT)
	MMPF_PWM_Initialize();
//printc("-------------2222222222---------------------\r\n");
RTNA_DBG_Str(0, "-------------2222222222---------------------\r\n");
	MMPF_PWM_OutputPulse(LCD_BACKLIGHT_PWM_UNIT_PIN, bEnable, 50000,50, MMP_TRUE, MMP_FALSE, NULL, 0);
#else
