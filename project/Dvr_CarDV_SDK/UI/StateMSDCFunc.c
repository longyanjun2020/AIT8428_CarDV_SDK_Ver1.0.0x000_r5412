...
{       
    switch( ulEvent )
    {
		...
		break;
	#endif
#if 0 // move to StateModeDoHandlerEvent
...
AHC_BOOL StateMSDCModeInitTV(void* pData)
{
    AHC_BOOL ahcRet = AHC_TRUE;
...
AHC_BOOL StateMSDCModeInitHDMI(void* pData)
{
    AHC_BOOL ahcRet = AHC_TRUE;
...
AHC_BOOL StatePCCAMModeInitTV(void* pData)
{
    AHC_BOOL ahcRet = AHC_TRUE;
...
AHC_BOOL StatePCCAMModeInitHDMI(void* pData)
{
    AHC_BOOL ahcRet = AHC_TRUE;
...
AHC_BOOL StateSelectFuncUSB_MSDC_PCAM_Mode(void)
{
	...
	if (AHC_IsHdmiConnect()){
		StateModeSetOperation(UI_MSDC_STATE, StateMSDCModeInitHDMI, StateMSDCModeShutDown, StateMSDCModeHandler);
		StateModeSetOperation(UI_PCCAM_STATE, StatePCCAMModeInitHDMI, StatePCCAMModeShutDown, StateMSDCModeHandler);
	}
	else
#endif
#if (TVOUT_ENABLE)
	if (AHC_IsTVConnectEx()){
		StateModeSetOperation(UI_MSDC_STATE, StateMSDCModeInitTV, StateMSDCModeShutDown, StateMSDCModeHandler);
		StateModeSetOperation(UI_PCCAM_STATE, StatePCCAMModeInitTV, StatePCCAMModeShutDown, StateMSDCModeHandler);
	}else
#endif
	{
