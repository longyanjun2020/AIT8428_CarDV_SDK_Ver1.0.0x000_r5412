...
MMP_LONG Driving_Safety_CheckEcoDriveLevel(void)
{
	...
	xyz[0] = ((signed char)sdata.acc_val[0])*100 / GSENSOR_ACCELERATION_UNIT;
	xyz[1] = ((signed char)sdata.acc_val[1])*100 / GSENSOR_ACCELERATION_UNIT;
	xyz[2] = ((signed char)sdata.acc_val[2])*100 / GSENSOR_ACCELERATION_UNIT;

 	lReturn = EcoDrivingCal(((float)(xyz[0]))/100, ((float)(xyz[1]))/100, ((float)(xyz[2]))/100, MMP_FALSE, 0);
