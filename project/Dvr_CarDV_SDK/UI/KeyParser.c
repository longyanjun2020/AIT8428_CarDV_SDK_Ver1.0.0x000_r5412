...
#include "fm_i2c.h"// long 4-23
#include "si47xx.h"// long
#include "FMRXtest.h" //long
#include "AMRXtest.h"
#include "gps_nmea0183.h"

#include "soft_i2c.h"// lyj 201890111
...
extern MMP_UBYTE flagSelect;// long 4-27

extern int 			subVol;

extern int			sub_count_ex;
extern AHC_BOOL Task_lockOn_flag;
...
AHC_BOOL AUX_flag = 0;
AHC_BOOL FM_AM_falg = 0;
AHC_BOOL brgiht_RGB_flag = 0;
AHC_BOOL sub_flag_ex = 0;

AHC_BOOL Task_delay_flag = 0;
AHC_BOOL Task_lockOn = 1;
...
extern void  MenuStateExecutionCommon1(UINT32 ulPosition);
...
#if 0
void FM_AMFLagSet(AHC_BOOL value)
{
			FM_AM_falg = value;

}

AHC_BOOL FM_AMFlagGet(void)
{
		return FM_AM_falg;
}
#endif
...
UINT32 KeyParser_VideoRecEvent(UINT32 ulMsgId, UINT32 ulEvent, UINT32 ulParam )
{
...
#endif
		case BUTTON_UP_REL         :     return EVENT_KEY_UP;//for debug speech recog~
   ...
   //     case BUTTON_SET_PRESS      :	 return EVENT_KEY_SET;
		case BUTTON_CAPTURE_REL :// À¶ÑÀ¿ªÆô
			return EVENT_SWITCH_WIFI_STREAMING_MODE;
		 case BUTTON_SET_PRESS      : 
			
			return (VideoFunc_RecordStatus()||uiGetParkingModeEnable())?(EVENT_NONE):(EVENT_KEY_MENU);
		...
		case BUTTON_MENU_REL	   :	
			CloseThePreviwVideoPower();// lyj 20190226
			return (VideoFunc_RecordStatus()||uiGetParkingModeEnable())?(EVENT_NONE):(EVENT_KEY_MENU);
		//case BUTTON_MENU_LPRESS	   :	 return (uiGetParkingCfg()->bParkingModeFuncEn == AHC_TRUE)?(EVENT_PARKING_KEY):(EVENT_NONE);
        
		//case BUTTON_MODE_REL	   : 
			//return EVENT_VIDEO_KEY_SWAP_PIP;
			//return (VideoFunc_RecordStatus())?(EVENT_LOCK_VR_FILE):(EVENT_KEY_MODE);
		case BUTTON_MODE_LPRESS	   :     return EVENT_NONE;//EVENT_SWITCH_WIFI_STREAMING_MODE;
 		case BUTTON_MUTE_REL       :	 
			printc("~~~~~~~~~~~~~~~~12~~~~~~\r\n");
			return EVENT_NONE;//EVENT_RECORD_MUTE;
 		case BUTTON_MUTE_LPRESS    :	 return EVENT_NONE;//EVENT_LCD_POWER_SAVE;
 		case BUTTON_POWER_LPRESS   :	 return EVENT_POWER_OFF;
		//case BUTTON_POWER_REL      :     return EVENT_SHOW_GPS_INFO;
		case BUTTON_UP_LPRESS	   :	return EVENT_NONE;//EVENT_VIDEO_KEY_SWAP_PIP;
        ...
        case BUTTON_SD_DETECTED    :     return EVENT_NONE;//EVENT_SD_DETECT;
        case BUTTON_SD_REMOVED     :     return EVENT_NONE;//EVENT_SD_REMOVED;
        case BUTTON_TV_DETECTED    :     return EVENT_TV_DETECT;
        case BUTTON_TV_REMOVED     :     return EVENT_TV_REMOVED;
        case BUTTON_HDMI_DETECTED  :     return EVENT_HDMI_DETECT;
        case BUTTON_HDMI_REMOVED   :     return EVENT_HDMI_REMOVED;
        case BUTTON_DC_CABLE_IN    :	 return EVENT_NONE;//EVENT_DC_CABLE_IN;
        case BUTTON_DC_CABLE_OUT   :	 return EVENT_NONE;//EVENT_DC_CABLE_OUT;
        
        case BUTTON_CUS_SW1_ON     :	 return EVENT_CUS_SW1_ON;
        case BUTTON_CUS_SW1_OFF    :	 return EVENT_CUS_SW1_OFF;

        case BUTTON_LCD_NORMAL     :     return EVENT_NONE;//EVENT_LCD_COVER_NORMAL;
        case BUTTON_LCD_ROTATE     :     return EVENT_NONE;//EVENT_LCD_COVER_ROTATE;
        case BUTTON_COVER_OPEN     :     return EVENT_LCD_COVER_OPEN;
        case BUTTON_COVER_CLOSED   :     return EVENT_LCD_COVER_CLOSE;

        #if (AHC_EMERGENTRECD_SUPPORT) || (AHC_SHAREENC_SUPPORT)
...
{
	//printc("~MenuButtonToMenuOp~~~ulEvent = ~%d~~~~~~\r\n",ulEvent);
    switch( ulEvent ){
    	case BUTTON_UPDATE_MESSAGE :	return EVENT_DROP;
#if(SUPPORT_TOUCH_PANEL)
        ...
        case BUTTON_UP_REL://BUTTON_UP_PRESS       :
        //case BUTTON_UP_LPRESS      :
			//printc("~~~~BUTTON_UP_PRESS~%d~~~~~~\r\n",PowerOff_InProcess);
			//exit_flag = 0;
			printc("~~~~~~~~~~~~~~~~~333333~~\n");
            return (PowerOff_InProcess)?(MENU_NONE):(/*MENU_UP*/MENU_NONE);
		//case BUTTON_UP_REL	   :
		//case BUTTON_DOWN_REL	   :
			//return MENU_NONE;

        case BUTTON_DOWN_REL://BUTTON_DOWN_PRESS     :
       // case BUTTON_DOWN_LPRESS    :
			//printc("~~~~BUTTON_DOWN_PRESS~~~~~~~\n");
			exit_flag = 0;
	   		//IIC_write_data_e2prom(0x12,(void*)0x56,1);
            return (PowerOff_InProcess)?(MENU_NONE):(/*MENU_DOWN*/MENU_RIGHT); //lyj 20180605

        case BUTTON_LEFT_REL://BUTTON_LEFT_PRESS       :
        //case BUTTON_LEFT_LPRESS      :
#if DIR_KEY_TYPE == KEY_TYPE_4KEY
            return (PowerOff_InProcess)?(MENU_NONE):(MENU_LEFT);
#else
/*{
		unsigned char e2promdata[5] = {'\0'};
		MMP_UBYTE i;
		//IIC_write_data_e2prom(0x12,0x56,1);

		iic_read_bytes_e2prom(0x00,e2promdata,5);
		for(i = 0; i < 5; i++)
		printc("~~~iic_read_bytes_e2prom~~~~~data == 0x%02x--->%d~~~~~~\n",e2promdata[i],i);
}*/
            return (PowerOff_InProcess)?(MENU_NONE):(MENU_DOWN);// MENU_UP lyj
#endif

        case BUTTON_RIGHT_REL://BUTTON_RIGHT_PRESS     :
...
#else
/*{
		MMP_UBYTE writeData[5] = {0x11,0x22,0x33,0x44,0x55};
		IIC_write_data_e2prom(0x00,writeData,5);
}*/
	...
	case BUTTON_MUTE_REL :	
		//printc("-----------BUTTON_MUTE_REL-----24key--------\r\n");
		//Key24_switch();
		//print_recvdata();
		Menu_Set_Page(3);
		MenuStateExecutionCommon1(3);		
	...
	case BUTTON_CAPTURE_REL :
		//printc("-----------BUTTON_CAPTURE_REL-------------\r\n");
		...
		if(Task_lockOn)
		{
			if(Main_Get_Page() == MENU_MAIN_FLAG || (Menu_Get_Page() != 8 && Menu_Get_Page() != 7 && Menu_Get_Page() != 4))
			if(SelfAllFlag.SaveTheBrightness)
								brgiht_RGB_flag = 0,SelfAllFlag.SaveTheBrightness= 0;
				else if(SelfAllFlag.SaveTheRGB)
								brgiht_RGB_flag = 1,SelfAllFlag.SaveTheRGB= 0;
				else if(SelfAllFlag.SaveTheLightBar)
								brgiht_RGB_flag = 2,SelfAllFlag.SaveTheLightBar= 0;
		
			if(brgiht_RGB_flag == 0)
			{
				Menu_Set_Page(8);
				MenuStateExecutionCommon1(8);
				brgiht_RGB_flag = 1;
			}
			else if(brgiht_RGB_flag ==1)
			{
				Menu_Set_Page(7);
				MenuStateExecutionCommon1(7);
				brgiht_RGB_flag = 2;
			}
			else if(brgiht_RGB_flag == 2)
			{
				Menu_Set_Page(4);
		...
		case BUTTON_REC_LREL		:
            return MENU_NONE;
	case BUTTON_SET_PRESS      :
		//printc("--------1---BUTTON_SET_PRESS-------------\r\n");
		  return (PowerOff_InProcess)?(MENU_NONE):(MENU_OK);
	
        case BUTTON_REC_REL	   :
			//printc("------2-----BUTTON_REC_REL-------------\r\n");

		if(Task_lockOn)
		{
			if(Main_Get_Page() == MENU_MAIN_FLAG || (Menu_Get_Page() != 1 && Menu_Get_Page() != 2))
				if(SelfAllFlag.SaveTheFM)
									AUX_flag = 0,SelfAllFlag.SaveTheFM= 0;
					else if(SelfAllFlag.SaveTheAM)
									AUX_flag = 1,SelfAllFlag.SaveTheAM= 0;
		...
		else
		{
            ...
            return (PowerOff_InProcess)?(MENU_NONE):(MENU_NONE);/*MENU_OK*/

        case BUTTON_MENU_REL       :
			//printc("-----------menu---PowerOff_InProcess=%d-------------\r\n",PowerOff_InProcess);
			
            return (PowerOff_InProcess)?(MENU_NONE):(MENU_MENU);

			   case BUTTON_MENU_LREL       :
			printc("-----------menu---longpress---\r\n");
			//Key24_switch();
            return (PowerOff_InProcess)?(MENU_NONE):(MENU_NONE);

        case BUTTON_MODE_REL       :
			//printc("-------3----BUTTON_MODE_REL-------------\r\n");
		if(Task_lockOn)
		{
			if(Main_Get_Page() == MENU_MAIN_FLAG || (Menu_Get_Page() != 0 && Menu_Get_Page() != 5 && Menu_Get_Page() != 6))
			if(SelfAllFlag.SaveTheUSB)
								AUX_flag = 2,SelfAllFlag.SaveTheUSB= 0;
				else if(SelfAllFlag.SaveTheAUX)
								AUX_flag = 1,SelfAllFlag.SaveTheAUX= 0;
				else if(SelfAllFlag.SaveTheBluetooth)
		...
		else
		{
            ...
            return (PowerOff_InProcess)?(MENU_NONE):(MENU_NONE);/*MENU_RIGHT*/
	...
	case BUTTON_POWER_REL     :  // BUTTON_POWER_REL
		if(mcu_flag)
		{
			//uart_to_mcu();
			//Key24_switch();
		...
		return MENU_NONE; // long 4-26 uart_to_mcu();MENU_NONE
	case BUTTON_POWER_LPRESS   :	uart_to_mcu_off(); return MENU_POWER_OFF;
