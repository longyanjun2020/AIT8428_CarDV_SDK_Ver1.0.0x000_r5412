...
static UINT8    NetFwUpdateTimerID       = 0xFF;
...
AHC_BOOL NetFwUpdateTimer_Stop(void)
{
	...
	if(NetFwUpdateTimerID < 0xFE){

		printc(FG_PURPLE("NetFwUpdateTimer_Stop: %d \r\n"), NetFwUpdateTimerID);	  
        
	    ret = AHC_OS_StopTimer( NetFwUpdateTimerID, OS_TMR_OPT_NONE );

		NetFwUpdateTimerID = 0xFF;
