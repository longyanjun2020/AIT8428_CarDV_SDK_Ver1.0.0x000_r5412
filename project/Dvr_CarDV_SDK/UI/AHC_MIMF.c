...
AHC_BOOL AHC_MIMF_MemPoolFSInit(UINT32 uiMaxMemSize)
{
    ...
    if( !bInit )
    {
        ...
        hMIMFFileNodeMemPool = AHC_MemPoolFS_InitPool((PBYTE)AHC_MIMF_TEMP_BASE_ADDR, uiMaxMemSize, sizeof(SINDEXMAPNODE) );
    ...
    switch(byOperation)
    {
        case DCFDT_NODE_INSERT:
        {
            ...
            else
            {
                ...
                {                 
                    ...
                    //duplicate node
                    if(nDateTime == psNodeCurr->nDateTime && byDB == psNodeCurr->byDB)
                    {
                        //do nothing
                        printc("%s, Duplicate Node Insert!\r\n", __func__);
                        return AHC_TRUE;
                    }
                    //if same date time, sort by DB count 
                    else if((nDateTime > psNodeCurr->nDateTime) ||
                            (nDateTime == psNodeCurr->nDateTime && byDB > psNodeCurr->byDB))
                    {
                        PSINDEXMAPNODE psNode;
                        psNode = (PSINDEXMAPNODE)AHC_MIMF_FileNodeAlloc();
                        if(psNode == NULL)
        ...
        case DCFDT_NODE_DELETE_PARENT:
        {
            ...
            else
            {
                ...
                while(psNodeCurr)
                {
                    if(psNodeCurr->nDateTime == nDateTime && psNodeCurr->byDB == byDB)
                    {
                        //unlink node
    ...
    while(psNodeCurr && ulFileCount <= ThumbIndex)
    {
        ...
        if(ulFileCount == ThumbIndex)
        {
            ...
            *byDB = psNodeCurr->byDB;
            AHC_DCFDT_GetIndexbyDBandDateTime(psNodeCurr->byDB, psNodeCurr->nDateTime, DCFIndex);
            bFound = AHC_TRUE;
        }
        ulFileCount++;
        psNodeCurr = psNodeCurr->pNext;
